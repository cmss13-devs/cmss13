...
/*			if ("Search Fingerprints")
				...
				if ((!( t1 ) || usr.stat || !( authenticated ) || usr.is_mob_restrained() || (!interactable()) && (!issilicon(usr))))
					return
				active1 = null
				active2 = null
				t1 = lowertext(t1)
...
//RECORD DELETE
			...
			if ("Add Entry")
				...
				if ((!( t1 ) || !( authenticated ) || usr.stat || usr.is_mob_restrained() || (!interactable() && (!issilicon(usr))) || active2 != a2))
					return
				var/counter = 1
				while(active2.fields[text("com_[]", counter)])
					counter++
				...
				switch(href_list["field"])
					if("name")
						if (istype(active1, /datum/data/record))
							...
							if ((!( t1 ) || !length(trim(t1)) || !( authenticated ) || usr.stat || usr.is_mob_restrained() || (!interactable() && (!issilicon(usr)))) || active1 != a1)
								return
							active1.fields["name"] = t1
					if("id")
						if (istype(active2, /datum/data/record))
							var/t1 = copytext(trim(sanitize(input("Please input id:", "Secure. records", active1.fields["id"], null)  as text)),1,MAX_MESSAGE_LEN)
							if ((!( t1 ) || !( authenticated ) || usr.stat || usr.is_mob_restrained() || (!interactable() && (!issilicon(usr))) || active1 != a1))
								return
