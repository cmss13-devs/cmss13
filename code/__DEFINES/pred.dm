#define PRED_TECH_MODERN "Modern"
#define PRED_TECH_RETRO "Retro"
#define PRED_TECH_COMBO "Combo"

#define PRED_MATERIALS list("ebony", "silver", "bronze", "crimson", "bone")
#define PRED_RETRO_MATERIALS list("retro", "ebony", "silver", "bronze", "crimson", "bone")
#define PRED_TRANSLATORS list(PRED_TECH_MODERN, PRED_TECH_RETRO, PRED_TECH_COMBO)
#define PRED_INVIS_SOUNDS list(PRED_TECH_MODERN, PRED_TECH_RETRO)
#define PRED_LEGACIES list("None", "Dragon", "Swamp", "Enforcer", "Collector", "Shadow", "Knight", "Seeker")
#define PRED_UNIQUES list("None", "Anubys", "Cleopatra", "Plated", "Ronin", "Seeker Initiate", "Seeker Adept")
#define PRED_UNIQUES_LEAD list()
#define PRED_UNIQUES_ALL list("None", "Anubys", "Cleopatra", "Plated", "Ronin", "Defender", "Warrior", "Seeker Initiate", "Seeker Adept", "Praetorian")
#define PRED_SKIN_COLOR list("tan", "green", "purple", "blue", "red", "black")

#define PRED_YAUTJA_CAPE "yautja cape"
#define PRED_YAUTJA_CEREMONIAL_CAPE "yautja ceremonial cape"
#define PRED_YAUTJA_THIRD_CAPE "yautja third-cape"
#define PRED_YAUTJA_HALF_CAPE "yautja half-cape"
#define PRED_YAUTJA_QUARTER_CAPE "yautja quarter-cape"
#define PRED_YAUTJA_PONCHO "yautja poncho"
#define PRED_YAUTJA_DAMAGED_CAPE "yautja damaged cape"
#define PRED_YAUTJA_BONE_CAPE_FULL "yautja bone cape"
#define PRED_YAUTJA_BONE_CAPE_THIRD "yautja bone third-cape"
#define PRED_YAUTJA_BONE_CAPE_CEREMONIAL "yautja ceremonial bone cape"

#define SD_TYPE_BIG   0
#define SD_TYPE_SMALL 1

#define PRED_MASK_TYPE_MAX 20
#define PRED_MASK_ACCESSORY_TYPE_MAX 3
#define PRED_ARMOR_TYPE_MAX 8
#define PRED_GREAVE_TYPE_MAX 4

#define YAUTJA_REPAIR_DAMAGED 0
#define YAUTJA_REPAIR_REPAIRED 1
#define YAUTJA_REPAIR_REINFORCED 2
