...
#define ABILITYCHECK_NONE				0
#define ABILITYCHECK_BUILD				(1<<0)	// Checks for dense tiles
#define ABILITYCHECK_DOWNED				(1<<1)  // Checks for any stuns,downed,dazed, etc
#define ABILITYCHECK_TARGETCHECKS		(1<<2)	// checks target passed to it if it is valid
#define ABILITYCHECK_TURF				(1<<3)	// checks current turf
#define ABILITYCHECK_WEED				(1<<4)	// checks if weed is under, if target passed, then check that for weeds
#define ABILITYCHECK_NEEDHAND			(1<<5)	// check if they have free hands
...
#define INTERRUPT_LCLICK            (1<<10)  s
