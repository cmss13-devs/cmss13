...
/proc/getFromPool(var/type, ...)
	...
	if(length(masterdatumPool[type]) <= 0)
		...
		if(ticker)
			to_chat(world, "DEBUG_DATUM_POOL: new proc has been called ([type] | [list2params(B)]).")
		#endif
	...
	#ifdef DEBUG_DATUM_POOL
	to_chat(world, "DEBUG_DATUM_POOL: getFromPool([type]) [length(masterdatumPool[type])] left arglist([list2params(B)]).")
	#endif
...
/proc/returnToPool(const/datum/D)
	...
	if(istype(D, /atom/movable) && length(masterdatumPool[D.type]) > MAINTAINING_OBJECT_POOL_COUNT)
		#ifdef DEBUG_DATUM_POOL
		to_chat(world, "DEBUG_DATUM_POOL: returnToPool([D.type]) exceeds [MAINTAINING_OBJECT_POOL_COUNT] discarding...")
		#endif
	...
	if(D in masterdatumPool[D.type])
		to_chat(world, "returnToPool has been called twice for the same datum of type [D.type] time to panic.")
	#endif

	masterdatumPool[D.type] |= D

	#ifdef DEBUG_DATUM_POOL
	to_chat(world, text("DEBUG_DATUM_POOL: returnToPool([D.type]) [] left.", length(masterdatumPool[D.type])))
	#endif
...
/client/proc/debug_pooling()
	...
	var/list/L = list()
	L += "<em>Stored Variables for Pooling for this type</em><br>"
	for(var/key in pooledvariables[type])
