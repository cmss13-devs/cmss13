...
/proc/get_alien_candidates(datum/hive_status/hive = null, sorted = TRUE, abomination = FALSE)
	...
	for(var/mob/dead/observer/cur_obs as anything in GLOB.observer_list)
		...
		if(deathtime < XENO_JOIN_DEAD_TIME && ( !cur_obs.client.player_data.admin_holder || !check_client_rights(cur_obs.client, R_ADMIN, FALSE)) && !cur_obs.bypass_time_of_death_checks)
			continue
