...
/proc/handle_toxic_states(in_icon)			//adds duplicate states for toxic water turfs so we can handle toxic states
...
actually for more coasts we only generate a very shallow overlay of their fulltile varient, this greatly shrinks the amount we need to make
...
//currently it generates icons only for turfs on loaded maps, not counting nightmares -- working on this
		...
		for(var/found_type in handle_toxic_states(starting_open_turf))	//if the water turf can be toxic, we need to add overlays for each possiblity
			...
			for(var/texture_size in SSwater_overlays.texture_sizes)
				...
				if(starting_open_turf.depth == DEPTH_DEEP)
					SSwater_overlays.water_overlay_icons["[texture_size]_[working_T.water_type][usable_tox]_[starting_open_turf.depth]_u"] = icon(sized_water_texture)	// for the full water overlay

				//	V V V V	construct depthed overlay for mob texture size	V V V V
				var/icon/culled_water = icon(sized_water_texture)
				var/texture_height = sized_water_texture.Height()
				subtraction_texture.Shift(SOUTH, (texture_height - abs(starting_open_turf.depth)-3), FALSE)         //we move it down to "water level" if we're not using a custom mob culling mask
				culled_water.AddAlphaMask(subtraction_texture)
				SSwater_overlays.water_overlay_icons["[texture_size]_[working_T.water_type][usable_tox]_[starting_open_turf.depth]"] = culled_water	//this is the default overlays, made according to depth
				...
				for(var/mob_type in special_mob_masks)
					...
					if(mob_type == /mob/living/carbon/human)
						...
						resting_east.AddAlphaMask(icon(SSwater_overlays.water_overlay_icon_paths["[texture_size]"], "culling_[resting_key]_rest_e"))
						resting_west.AddAlphaMask(icon(SSwater_overlays.water_overlay_icon_paths["[texture_size]"], "culling_[resting_key]_rest_w"))
						SSwater_overlays.water_overlay_icons["32_[working_T.water_type][usable_tox]_[starting_open_turf.depth]_[mob_type]_e"] =  resting_east
						SSwater_overlays.water_overlay_icons["32_[working_T.water_type][usable_tox]_[starting_open_turf.depth]_[mob_type]_w"] =  resting_west
					...
					if(special_mob_masks[mob_type])
						...
						var/icon/special_mask = icon(SSwater_overlays.water_overlay_icon_paths["[texture_size]"], special_mob_masks[mob_type])
						special_icon.AddAlphaMask(special_mask)
						SSwater_overlays.water_overlay_icons["[texture_size]_[working_T.water_type][usable_tox]_[starting_open_turf.depth]_[mob_type]"] = special_icon
