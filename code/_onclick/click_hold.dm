...
/client/MouseDown(var/atom/A, var/turf/T, var/skin_ctl, var/params)
	...
	if(prefs && prefs.toggle_prefs & TOGGLE_COMBAT_CLICKDRAG_OVERRIDE)
		...
		if(ismob(target_mob) && target_mob.faction == mob.faction && !(isCarbonSizeXeno(mob) && !mob.get_active_hand())) //Don't attack your allies or yourself, unless you're a xeno with an open hand.
			return
