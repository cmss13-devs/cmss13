...
/datum/hud/human/persistent_inventory_update(mob/viewer)
	...
	if(H.hud_used)
		if(H.hud_used.hud_shown)
			...
			if(H.get_wear_id())
				var/obj/item/id = H.get_wear_id()
				id.screen_loc = ui_datum.hud_slot_offset(id, ui_datum.ui_id)
				screenmob.client.add_to_screen(id)
			if(H.belt)
			...
			if(H.get_wear_id())
				screenmob.client.remove_from_screen(H.get_wear_id())
			if(H.belt)
