/datum/hud/robot
	var/datum/custom_hud/robot/ui_datum

/datum/hud/robot/New(mob/living/silicon/robot/owner)
	..()
	var/obj/screen/using

	ui_datum = custom_huds_list["robot"]
...
//Module select
	...
	using.screen_loc = ui_datum.ui_inv1
	using.layer = ABOVE_HUD_LAYER
	owner.inv1 = using
	static_inventory += using
	...
	using.screen_loc = ui_datum.ui_inv2
	using.layer = ABOVE_HUD_LAYER
	owner.inv2 = using
	static_inventory += using
	...
	using.screen_loc = ui_datum.ui_inv3
	using.layer = ABOVE_HUD_LAYER
	owner.inv3 = using
	static_inventory += using
...
//Cell
	...
	owner.cells.icon = ui_datum.ui_style_icon
	owner.cells.icon_state = "charge-empty"
	owner.cells.name = "cell"
	owner.cells.screen_loc = ui_datum.ui_toxin
	infodisplay += owner.cells

//Health
	healths = new /obj/screen/healths()
	healths.icon = ui_datum.ui_style_icon
	healths.screen_loc = ui_datum.ui_borg_health
	infodisplay += healths

//Installed Module
	owner.hands = new /obj/screen()
	owner.hands.icon = ui_datum.ui_style_icon
	owner.hands.icon_state = "nomod"
	owner.hands.name = "module"
	owner.hands.screen_loc = ui_datum.ui_borg_module
...
//Module Panel
	...
	using.screen_loc = ui_datum.ui_borg_panel
	using.layer = HUD_LAYER
	static_inventory += using

//Store
	module_store_icon = new /obj/screen()
	module_store_icon.icon = ui_datum.ui_style_icon
	module_store_icon.icon_state = "store"
	module_store_icon.name = "store"
	module_store_icon.screen_loc = ui_datum.ui_borg_store
	static_inventory += module_store_icon

//Temp
	bodytemp_icon = new /obj/screen/bodytemp()
	bodytemp_icon.screen_loc = ui_datum.ui_borg_temp
	...
	pull_icon.screen_loc = ui_datum.ui_borg_pull
...
/datum/hud/robot/persistant_inventory_update()
	...
	if(hud_shown)
		if(R.module_state_1)
			R.module_state_1.screen_loc = ui_datum.ui_inv1
			R.client.screen += R.module_state_1
		if(R.module_state_2)
			R.module_state_2.screen_loc = ui_datum.ui_inv2
			R.client.screen += R.module_state_2
		if(R.module_state_3)
			R.module_state_3.screen_loc = ui_datum.ui_inv3
