...
/datum/controller/subsystem/minimaps/proc/load_new_z(datum/dcs, datum/space_level/z_level)
	...
	minimaps_by_z["[level]"].scaled_x_min = xmin
	minimaps_by_z["[level]"].scaled_y_min = ymin
...
/datum/controller/subsystem/minimaps/proc/pregenerate_ceiling_overlay(z_level)
	...
	var/map_xmin = world.maxx
	var/map_ymin = world.maxy
	var/map_xmax = 1
	var/map_ymax = 1

	for(var/xval = 1 to world.maxx)
		for(var/yval = 1 to world.maxy)
			var/turf/location = locate(xval, yval, z_level)
			if(location.density)
				if(!istype(location, /turf/closed/wall/almayer/outer))
					map_xmin = min(map_xmin, xval)
					map_ymin = min(map_ymin, yval)
					map_xmax = max(map_xmax, xval)
					map_ymax = max(map_ymax, yval)
			else if(!istype(location, /turf/open/space))
				map_xmin = min(map_xmin, xval)
				map_ymin = min(map_ymin, yval)
				map_xmax = max(map_xmax, xval)
	...
	for(var/xval = 1, xval <= world.maxx, xval++)
		for(var/yval = 1, yval <= world.maxy, yval++)
			...
			if(ceiling_color && !istype(turfloc, /turf/open/space))
				ceiling_overlay.DrawBox(ceiling_color, xval, yval)

	var/xmin = hud_data.scaled_x_min
	var/ymin = hud_data.scaled_y_min
