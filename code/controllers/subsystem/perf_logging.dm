...
/datum/controller/subsystem/perf_logging/proc/start_logging()
	...
	if(!ground) return
	ord = 0
	round = GLOB.SSentity_manager.select(/datum/entity/mc_round)
	round.map_name = ground.map_name
	round.save()
	var/datum/entity/mc_controller/C
	for(var/datum/controller/subsystem/SS in Master.subsystems)
		C = GLOB.SSentity_manager.select_by_key(/datum/entity/mc_controller, "[SS.type]")
		if(!C) continue
		C.wait_time = SS.wait
		C.save()
...
/datum/controller/subsystem/perf_logging/proc/new_record(datum/controller/subsystem/SS, datum/entity/mc_controller/C)
	...
	var/datum/entity/mc_record/record = GLOB.SSentity_manager.select(/datum/entity/mc_record)
...
/datum/controller/subsystem/perf_logging/proc/update_mc_timing()
	SHOULD_NOT_SLEEP(TRUE)
	var/datum/entity/mc_timing_info/timing_info = GLOB.SSentity_manager.select(/datum/entity/mc_timing_info)
