			...
			for(var/texture_size in configs_by_size)
				...
				for(var/datum/water_overlay_config/config as anything in configs_of_this_size)	//could be streamlined by removing configs thatd result in the same overlays being generated
					...
					if(config.resting_behavior == "resting_some")
						...
						SSwater_overlays.water_overlay_icons["32_[found_type]_[toxic]_[found_depth]_[config.icon_state_key]_resting_e"] = resting_east
						SSwater_overlays.water_overlay_icons["32_[found_type]_[toxic]_[found_depth]_[config.icon_state_key]_resting_w"] = resting_west
