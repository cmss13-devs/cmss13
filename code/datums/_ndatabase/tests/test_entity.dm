...
/proc/test_read(id)
	var/datum/entity/test_entity/ET = GLOB.SSentity_manager.select(/datum/entity/test_entity, id)
	ET.sync_then(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(log_sync)))

/proc/test_insert(name, desc, value)
	var/datum/entity/test_entity/ET = GLOB.SSentity_manager.select(/datum/entity/test_entity)
	ET.name = name
	ET.description = desc
	ET.value = value
	ET.save()
...
/proc/test_filter(value)
	GLOB.SSentity_manager.filter_then(/datum/entity/test_entity, DB_COMP("value", DB_EQUALS, value), CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(log_filter)))
