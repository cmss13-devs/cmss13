...
/datum/ammo/proc/setup_faction_clash_values()
	...
	penetration = min(penetration, 30) //more ap overpenetrates anyway but makes next calculation cleaner
	accurate_range = min(accurate_range, 10 - penetration/5) //this makes AP ammo better at close range (and technically super far range when the hitchance gets bottom caped at 5% hitchance)
