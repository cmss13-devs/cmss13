...
/obj/effect/ebeam/laser/Crossed(atom/movable/AM)
	...
	if(rand_laser_power > laser_protection)
		...
		if(HAS_TRAIT(moving_human, TRAIT_BIMEX))
			...
			moving_human.balloon_alert_to_viewers("the laser bounces off [moving_human.p_their()] BiMex© personal shades!", "the laser bounces off your BiMex© personal shades!")
		else
