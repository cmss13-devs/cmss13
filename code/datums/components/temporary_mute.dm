...
/datum/component/temporary_mute/proc/on_speak(
	...
	if(!nolog)
		log_say("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to say the following before their spawn mute ended: [message] (CKEY: [user.key]) (JOB: [user.job]) (AREA: [get_area_name(user)])")
	if(on_speak_message)
...
/datum/component/temporary_mute/proc/on_hivemind(mob/user, message)
	...
	log_say("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to hivemind the following before their spawn mute ended: [message] (CKEY: [user.key]) (JOB: [user.job]) (AREA: [get_area_name(user)])")
	if(on_speak_message)
		to_chat(parent, SPAN_BOLDNOTICE(on_speak_message))
...
/datum/component/temporary_mute/proc/on_emote(mob/user, datum/emote/current_emote, act, m_type, param, intentional)
	...
	log_say("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to emote the following before their spawn mute ended: [param] (CKEY: [user.key]) (JOB: [user.job]) (AREA: [get_area_name(user)])")
	if(on_emote_message)
...
/datum/component/temporary_mute/proc/on_point(mob/user, atom/target)
	...
	log_say("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to point at the following before their spawn mute ended: [target] (CKEY: [user.key]) (JOB: [user.job]) (AREA: [get_area_name(user)])")
	if(on_emote_message)
		to_chat(parent, SPAN_BOLDNOTICE(on_emote_message))
