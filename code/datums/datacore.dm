...
/datum/datacore/proc/get_manifest(monochrome, OOC, nonHTML)
	var/list/list/cic = GLOB.ROLES_CIC.Copy()
	var/list/list/auxil = GLOB.ROLES_AUXIL_SUPPORT.Copy()
	var/list/list/misc = GLOB.ROLES_MISC.Copy()
	var/list/list/mp = GLOB.ROLES_POLICE.Copy()
	var/list/list/eng = GLOB.ROLES_ENGINEERING.Copy()
	var/list/list/req = GLOB.ROLES_REQUISITION.Copy()
	var/list/list/med = GLOB.ROLES_MEDICAL.Copy()
	var/list/list/list/marines_by_squad = GLOB.ROLES_SQUAD_ALL.Copy()
