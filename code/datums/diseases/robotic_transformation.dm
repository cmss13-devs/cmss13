...
/datum/disease/robotic_transformation/stage_act()
	...
	switch(stage)
		if(2)
			...
			if (prob(9))
				to_chat(affected_mob, "<span class='danger'>Beep...boop..</span>")
			if (prob(9))
				to_chat(affected_mob, "<span class='danger'>Bop...beeep...</span>")
		if(3)
			if (prob(8))
				to_chat(affected_mob, "<span class='danger'>Your joints feel very stiff.</span>")
				affected_mob.take_limb_damage(1)
			if (prob(8))
			...
			if (prob(4))
				to_chat(affected_mob, "<span class='danger'>You can feel something move...inside.</span>")
		if(4)
			if (prob(10))
				to_chat(affected_mob, "<span class='danger'>Your skin feels very loose.</span>")
				affected_mob.take_limb_damage(8)
			if (prob(20))
				affected_mob.say(pick("beep, beep!", "Boop bop boop beep.", "kkkiiiill mmme", "I wwwaaannntt tttoo dddiiieeee..."))
			if (prob(8))
				to_chat(affected_mob, "<span class='danger'>You can feel... something...inside you.</span>")
		if(5)
