...
/datum/disease/xeno_transformation/stage_act()
	...
	switch(stage)
		if(2)
			...
			if (prob(9))
				to_chat(affected_mob, SPAN_DANGER("Kill..."))
			if (prob(9))
				to_chat(affected_mob, SPAN_DANGER("Kill..."))
		if(3)
			if (prob(8))
		...
		if(4)
			if (prob(10))
				affected_mob << pick(SPAN_DANGER("Your skin feels very tight."), SPAN_DANGER("Your blood boils!"))
		...
		if(5)
			affected_mob <<SPAN_DANGER("Your skin feels impossibly calloused...")
