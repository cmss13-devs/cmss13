...
/datum/effects/bleeding/external/process_mob()
	...
	if(duration % 3 == 0) //Do it every third tick
		if(affected_mob.reagents) // Annoying QC check
			...
			if(affected_mob.bodytemperature < BODYTEMP_CRYO_THRESHOLD && (affected_mob.reagents.get_reagent_amount("cryoxadone") || affected_mob.reagents.get_reagent_amount("clonexadone")))
