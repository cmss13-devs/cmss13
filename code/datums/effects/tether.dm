...
/datum/effects/tethering/New(var/atom/A, var/range, var/icon)
    ..()
    src.range = range
    tether_icon = icon
...
/datum/effects/tethering/proc/set_tethered(var/datum/effects/tethered/T)
    tethered = T
    T.tether = src
    beam_id = affected_atom.Beam(T.affected_atom, tether_icon, 'icons/effects/beam.dmi', BEAM_INFINITE_DURATION, range+1)
...
/proc/apply_tether(var/atom/tetherer, var/atom/tethered, var/two_way = FALSE, var/range = 1, var/resistable = FALSE, var/icon = "chain")
    var/list/ret_list = list()

    var/datum/effects/tethering/TR = new /datum/effects/tethering(tetherer, range, icon)
