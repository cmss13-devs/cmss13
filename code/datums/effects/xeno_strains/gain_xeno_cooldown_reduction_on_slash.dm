...
/datum/effects/gain_xeno_cooldown_reduction_on_slash/Destroy()
	if(affected_atom)
		...
		if(istype(xeno) && show_baloon_alert)
			to_chat(xeno, SPAN_XENOWARNING("Мы чувствуем, что ярость больше не переполняет нас! Наши способности во время боя будут перезаряжаться с обычной скоростью!"))
