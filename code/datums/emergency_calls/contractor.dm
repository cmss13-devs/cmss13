...
/datum/emergency_call/contractors/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(mob.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(mob.client, JOB_SQUAD_LEADER, time_required_for_job))
		leader = mob
		to_chat(mob, role_header("You are a Contractor Team Leader of Vanguard's Arrow Incorporated!"))
		...
		synths++
		to_chat(mob, role_header("You are a Contractor Support Synthetic of Vanguard's Arrow Incorporated!"))
		arm_equipment(mob, /datum/equipment_preset/contractor/duty/synth, TRUE, TRUE)
	else if(medics < max_medics && HAS_FLAG(mob.client.prefs.toggles_ert, PLAY_MEDIC) && check_timelock(mob.client, JOB_SQUAD_MEDIC, time_required_for_job))
		medics++
		to_chat(mob, role_header("You are a Contractor Medical Specialist of Vanguard's Arrow Incorporated!"))
		...
		heavies++
		to_chat(mob, role_header("You are a Contractor Machinegunner of Vanguard's Arrow Incorporated!"))
		...
		engineers++
		to_chat(mob, role_header("You are a Contractor Engineering Specialist of Vanguard's Arrow Incorporated!"))
		arm_equipment(mob, /datum/equipment_preset/contractor/duty/engi, TRUE, TRUE)
	else
		to_chat(mob, role_header("You are a Contractor of Vanguard's Arrow Incorporated!"))
...
/datum/emergency_call/contractors/print_backstory(mob/living/carbon/human/M)
	if(ishuman_strict(M))
		to_chat(M, role_body("You were born [pick(60;"in the United States", 20;"on Earth", 20;"on a colony")] to a [pick(75;"average", 15;"poor", 10;"well-established")] family."))
		to_chat(M, role_body("Joining the USCM gave you a lot of combat experience and useful skills but changed you."))
		to_chat(M, role_body("After getting out, you couldn't hold a job with the things you saw and did, deciding to put your skills to use you joined a Military Contractor firm."))
		to_chat(M, role_body("You are a skilled mercenary, making better pay than in the Corps."))
	else
		to_chat(M, role_body("You were brought online in a civilian factory."))
		to_chat(M, role_body("You were programmed with all of the medical and engineering knowledge a military fighting force support asset required."))
		to_chat(M, role_body("You were soon after bought by Vanguard's Arrow Incorporated(VAI) to act as support personnel."))
		to_chat(M, role_body("Some months after your purchase, you were assigned to the USCSS Inheritor, a VAI transport vessel."))
	to_chat(M, role_body("You are [pick(80;"unaware", 15;"faintly aware", 5;"knowledgeable")] of the xenomorph threat."))
	to_chat(M, role_body("You are employed by Vanguard's Arrow Incorporated(VAI), as a member of VAI Primary Operations(VAIPO)"))
	to_chat(M, role_body("You are stationed on-board the USCSS Inheritor, a part of VAIPO Task-Force Charlie."))
	to_chat(M, role_body("Under the directive of the VAI executive board, you have been assist in riot control, military aid, and to assist USCMC forces wherever possible."))
	to_chat(M, role_body("The USCSS Inheritor is staffed with crew of roughly three hundred military contractors, and fifty support personnel."))
	to_chat(M, role_body("Assist the USCMC Force of the [MAIN_SHIP_NAME] however you can."))
	to_chat(M, role_body("As a side-objective, VAI has been hired by an unknown benefactor to engage in corporate espionage and sabotage against Weyland-Yutani, avoid direct conflict; you aren't VAISO; but attempt to recover Wey-Yu secrets and plans if possible."))
...
/datum/emergency_call/contractors/covert/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, JOB_SQUAD_LEADER, time_required_for_job))    //First one spawned is always the leader.
		leader = H
		to_chat(H, role_header("You are a Covert Contractor Team Leader of Vanguard's Arrow Incorporated!"))
		...
		synths++
		to_chat(H, role_header("You are a Contractor Support Synthetic of Vanguard's Arrow Incorporated!"))
		arm_equipment(H, /datum/equipment_preset/contractor/covert/synth, TRUE, TRUE)
	else if(medics < max_medics && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_MEDIC) && check_timelock(H.client, JOB_SQUAD_MEDIC, time_required_for_job))
		medics++
		to_chat(H, role_header("You are a Covert Contractor Medical Specialist of Vanguard's Arrow Incorporated!"))
		...
		heavies++
		to_chat(H, role_header("You are a Covert Contractor Machinegunner of Vanguard's Arrow Incorporated!"))
		...
		engineers++
		to_chat(H, role_header("You are a Covert Contractor Engineering Specialist of Vanguard's Arrow Incorporated!"))
		arm_equipment(H, /datum/equipment_preset/contractor/covert/engi, TRUE, TRUE)
	else
		to_chat(H, role_header("You are a Covert Contractor of Vanguard's Arrow Incorporated!"))
