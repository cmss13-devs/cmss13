...
/datum/emergency_call/cryo_squad_equipped/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(leaders < cryo_squad.max_leaders && HAS_FLAG(human.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(human.client, JOB_SQUAD_LEADER, time_required_for_job))
		leader = human
		leaders++
		arm_equipment(human, /datum/equipment_preset/uscm/leader_equipped/cryo, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are a Squad Leader in the USCM"))
		to_chat(human, SPAN_ROLE_BODY("Your squad is here to assist in the defence of the [SSmapping.configs[GROUND_MAP].map_name]."))
		...
		heavies++
		arm_equipment(human, /datum/equipment_preset/uscm/specialist_equipped/cryo, TRUE, TRUE)
		...
		smartgunners++
		arm_equipment(human, /datum/equipment_preset/uscm/smartgunner_equipped/cryo, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are a Smartgunner in the USCM"))
		to_chat(human, SPAN_ROLE_BODY("Your squad is here to assist in the defence of the [SSmapping.configs[GROUND_MAP].map_name]."))
		...
		engineers++
		arm_equipment(human, /datum/equipment_preset/uscm/engineer_equipped/cryo, TRUE, TRUE)
		...
		medics++
		arm_equipment(human, /datum/equipment_preset/uscm/medic_equipped/cryo, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are a Hospital Corpsman in the USCM"))
		to_chat(human, SPAN_ROLE_BODY("Your squad is here to assist in the defence of the [SSmapping.configs[GROUND_MAP].map_name]."))
	else
		arm_equipment(human, /datum/equipment_preset/uscm/private_equipped/cryo, TRUE, TRUE)
