...
/datum/emergency_call/deus_vult/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, JOB_SQUAD_LEADER, time_required_for_job))
		...
		to_chat(H, role_header("You are the leader of these holy warriors!"))
		...
		to_chat(H, role_header("You are a champion of the holy warriors!"))
		to_chat(H, role_body("You must clear out any traces of the unholy from this wretched place!"))
		to_chat(H, role_body("Follow any orders directly from the Higher Power!"))
	else
		arm_equipment(H, /datum/equipment_preset/other/gladiator, TRUE, TRUE)
		to_chat(H, role_header("You are a holy warrior!"))
		to_chat(H, role_body("You must clear out any traces of the unholy from this wretched place!"))
		to_chat(H, role_body("Follow any orders directly from the Higher Power!"))
