...
/datum/emergency_call/dutch/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(human.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(human.client, JOB_SQUAD_LEADER, time_required_for_job))
		leader = human
		arm_equipment(human, /datum/equipment_preset/dutch/arnie, TRUE, TRUE)
		...
		heavies++
		arm_equipment(human, /datum/equipment_preset/dutch/flamer, TRUE, TRUE)
		...
		smartgunners++
		arm_equipment(human, /datum/equipment_preset/dutch/minigun, TRUE, TRUE)
		...
		medics++
		arm_equipment(human, /datum/equipment_preset/dutch/medic, TRUE, TRUE)
	else
		arm_equipment(human, /datum/equipment_preset/dutch, TRUE, TRUE)
