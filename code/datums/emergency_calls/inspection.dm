...
/datum/emergency_call/inspection_cmb/New()
	...
	objectives = "Get your instructions from the CMB Office at Anchorpoint Station, and carry out your orders. Ensure that Colonial assets are safe and in your custody. Do not enforce or override Marine Law on a Marine Ship, as it's outside of your juristiction."
...
/datum/emergency_call/CMB/print_backstory(mob/living/carbon/human/M)
	if(spawning_human == leader)
		...
		to_chat(M, SPAN_BOLD("You have interstellar juristiction as a CMB Official, but you cannot and should not override Marine Law on a Marine Ship."))
		...
		to_chat(M, SPAN_BOLD("Your headset is equipped with several frequencies, including a gifted key from Weyland-Yutani to try to incentivize your support. Use it for communication."))
		to_chat(M, SPAN_BOLD("As the ICC Agent attached to the CMB Office at Anchorpoint Station, your job is to observe and ensure fair trade practices. Inspect and document cargo shipments for suspected illict activity as needed."))
	...
	else if(spawning_human == observer)
