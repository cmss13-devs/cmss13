...
/datum/emergency_call/mercs/print_backstory(mob/living/carbon/human/human)
	...
	if(hostility)
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("Despite this, you have been tasked to ransack the [MAIN_SHIP_NAME] and kill anyone who gets in your way.")))
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("Any UPP, CLF or WY forces also responding are to be considered neutral parties unless proven hostile.")))
	else
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("To this end, you have been contacted by Weyland-Yutani of the USCSS Royce to assist the [MAIN_SHIP_NAME]..")))
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("Ensure they are not destroyed.</b>")))
...
/datum/emergency_call/mercs/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(human.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(human.client, JOB_SQUAD_LEADER, time_required_for_job))
		leader = human
		arm_equipment(human, /datum/equipment_preset/other/freelancer/leader, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are the Freelancer leader!"))
		...
		medics++
		arm_equipment(human, /datum/equipment_preset/other/freelancer/medic, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are a Freelancer Medic!"))
	else
		arm_equipment(human, /datum/equipment_preset/other/freelancer/standard, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are a Freelancer Mercenary!"))
...
/datum/emergency_call/heavy_mercs/print_backstory(mob/living/carbon/human/human)
	to_chat(human, SPAN_BOLD("You started off in the Neroid Sector as an experienced miner seeking work at one of the established colonies."))
	to_chat(human, SPAN_BOLD("The withdrawl of United American forces in the early 2180s, the system fell into disarray."))
	to_chat(human, SPAN_BOLD("Taking up arms as a mercenary, the Freelancers have become a powerful force of order in the system."))
	to_chat(human, SPAN_BOLD("While they are motivated primarily by money, many colonists see the Freelancers as the main forces of order in the Neroid Sector."))
	if(hostility)
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("Despite this, you have been specially tasked to ransack the [MAIN_SHIP_NAME] and kill anyone who gets in your way.")))
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("Any UPP, CLF or WY forces also responding are to be considered neutral parties unless proven hostile.")))
	else
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("To this end, you have been contacted by Weyland-Yutani of the USCSS Royce to assist the [MAIN_SHIP_NAME]..")))
		to_chat(human, SPAN_NOTICE(SPAN_BOLD("Ensure they are not destroyed.</b>")))
...
/datum/emergency_call/heavy_mercs/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(human.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(human.client, JOB_SQUAD_LEADER, time_required_for_job))    //First one spawned is always the leader.
		leader = human
		arm_equipment(human, /datum/equipment_preset/other/elite_merc/leader, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are the Elite Mercenary leader!"))
	else if(medics < max_medics && HAS_FLAG(human.client.prefs.toggles_ert, PLAY_MEDIC) && check_timelock(human.client, JOB_SQUAD_MEDIC, time_required_for_job))
		medics++
		arm_equipment(human, /datum/equipment_preset/other/elite_merc/medic, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are an Elite Mercenary Medic!"))
		...
		engineers++
		arm_equipment(human, /datum/equipment_preset/other/elite_merc/engineer, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are an Elite Mercenary Engineer!"))
		...
		heavies++
		arm_equipment(human, /datum/equipment_preset/other/elite_merc/heavy, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are an Elite Mercenary Specialist!"))
	else
		arm_equipment(human, /datum/equipment_preset/other/elite_merc/standard, TRUE, TRUE)
		to_chat(human, SPAN_ROLE_HEADER("You are an Elite Mercenary!"))
	print_backstory(human)
