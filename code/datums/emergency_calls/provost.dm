...
/datum/emergency_call/provost_inspection
	...
	mob_max = 5
	mob_min = 5
	objectives = "Inspect the USS Almayer and ensure Marine Law is being upheld by the security team."
	probability = 0


/datum/emergency_call/provost_inspection/create_member(datum/mind/M, var/turf/override_spawn_loc)
...
/datum/emergency_call/provost_enforcer/create_member(datum/mind/M, var/turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, list(JOB_SHIP_MARINE, JOB_SHIP_MASTER_AT_ARMS), time_required_for_job))       //First one spawned is always the leader.
		leader = H
