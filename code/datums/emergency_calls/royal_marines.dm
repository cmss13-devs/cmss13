...
/datum/emergency_call/royal_marines/create_member(datum/mind/spawning_mind, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(mob.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(mob.client, JOB_SQUAD_LEADER, time_required_for_job))
		leader = mob
		to_chat(mob, role_header("You are an Officer in the Royal Marines Commando. Born in the Three World Empire."))
		...
		switch(specialist_kit)
			if("Sniper")
				to_chat(mob, role_header("You are a skilled marksman in the Royal Marines Commando. Born in the Three World Empire."))
				arm_equipment(mob, /datum/equipment_preset/twe/royal_marine/spec/marksman, TRUE, TRUE)
			if("Smartgun")
				to_chat(mob, role_header("You are a Smartgunner in the Royal Marines Commando. Born in the Three World Empire."))
				arm_equipment(mob, /datum/equipment_preset/twe/royal_marine/spec/machinegun, TRUE, TRUE)
			if("Breach")
				to_chat(mob, role_header("You are a CQB Specialist in the Royal Marines Commando. Born in the Three World Empire."))
				arm_equipment(mob, /datum/equipment_preset/twe/royal_marine/spec/breacher, TRUE, TRUE)
		heavies++
	else
		to_chat(mob, role_header("You are a member of the Royal Marines Commando. Born in the three world empire."))
...
/datum/emergency_call/royal_marines/print_backstory(mob/living/carbon/human/spawning_mob)
	...
	to_chat(spawning_mob, role_body("Joining the Royal Marines gave you a lot of combat experience and useful skills."))
	to_chat(spawning_mob, role_body("You are [pick_weight(list("unaware" = 75, "faintly aware" = 15, "knoledgeable" = 10))] of the xenomorph threat."))
	to_chat(spawning_mob, role_body("You are a citizen of the three world empire and joined the Royal Marines Commando"))
	to_chat(spawning_mob, role_body("You are apart of a jointed UA/TWE taskforce onboard the HMS Patna and Thunderchild."))
	to_chat(spawning_mob, role_body("Under the directive of the RMC high command, you have been assisting USCM forces with maintaining peace in the area."))
	to_chat(spawning_mob, role_body("Assist the USCMC Force of the [MAIN_SHIP_NAME] however you can."))
