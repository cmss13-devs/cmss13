...
/datum/emergency_call/upp/print_backstory(mob/living/carbon/human/M)
	if(isHumanStrict(M))
		...
		to_chat(M, SPAN_BOLD("You were brought online in a FACILITY TYPE facility, knowing only your engineers for the first few weeks for your pseudo-life."))
		...
		to_chat(M, SPAN_BOLD("Despite this, your engineers, and later, your UPP compatriots, treated you like any other organic comrade."))
...
/datum/emergency_call/upp/create_member(datum/mind/M, var/turf/override_spawn_loc)
	...
	if(!leader && (H.client.prefs.toggles_ert & PLAY_LEADER))
		leader = H
	...
	else if(synths < max_synths && RoleAuthority.roles_whitelist[H.ckey] & WHITELIST_SYNTHETIC)
		synths++
