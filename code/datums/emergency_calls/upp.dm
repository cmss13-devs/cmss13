...
/datum/emergency_call/upp/print_backstory(mob/living/carbon/human/M)
	if(ishuman_strict(M))
		to_chat(M, role_body("You grew up in relatively simple family in [pick(75;"Eurasia", 25;"a famished UPP colony")] with few belongings or luxuries."))
		to_chat(M, role_body("The family you grew up with were [pick(50;"getting by", 25;"impoverished", 25;"starving")] and you were one of [pick(10;"two", 20;"three", 20;"four", 30;"five", 20;"six")] children."))
		to_chat(M, role_body("You come from a long line of [pick(40;"crop-harvesters", 20;"soldiers", 20;"factory workers", 5;"scientists", 15;"engineers")], and quickly enlisted to improve your living conditions."))
		to_chat(M, role_body("Following your enlistment to the UPP military at the age of 17, you were assigned to the 17th 'Smoldering Sons' battalion (six hundred strong) under the command of Colonel Ganbaatar."))
	else
		to_chat(M, role_body("You were brought online in a UPP engineering facility, knowing only your engineers for the first few weeks for your pseudo-life."))
		to_chat(M, role_body("You were programmed with all of the medical and combat experience a military fighting force support asset required."))
		to_chat(M, role_body("Throughout your career, your engineers, and later, your UPP compatriots, treated you like a tool, and only that."))
		to_chat(M, role_body("Some weeks after your unit integration, you were assigned to the 17th 'Smoldering Sons' battalion (six hundred strong) under the command of Colonel Ganbaatar."))
	to_chat(M, role_body("You were shipped off with the battalion to one of the UPP's most remote territories, a gas giant designated MV-35 in the Anglo-Japanese Arm, in the Neroid Sector."))
	to_chat(M, role_body("For the past 14 months, you and the rest of the Smoldering Sons have been stationed at MV-35's only facility, the helium refinery, Altai Station."))
	to_chat(M, role_body("As MV-35 and Altai Station are the only UPP-held zones in the Neroid Sector for many lightyears, you have spent most of your military career holed up in crammed quarters in near darkness, waiting for supply shipments and transport escort deployments."))
	to_chat(M, role_body("With the recent arrival of the enemy USCM battalion the 'Falling Falcons' and their flagship, the [MAIN_SHIP_NAME], the UPP has felt threatened in the sector."))
	to_chat(M, role_body("In an effort to protect the vulnerable MV-35 from the encroaching UA/USCM imperialists, the leadership of your battalion has opted this to be the best opportunity to strike at the Falling Falcons to catch them off guard."))
	to_chat(M, role_header("Glory to Colonel Ganbaatar."))
	to_chat(M, role_header("Glory to the Smoldering Sons."))
	to_chat(M, role_header("Glory to the UPP."))
	to_chat(M, role_body(" Use say :3 <text> to speak in your native tongue."))
	to_chat(M, role_body(" This allows you to speak privately with your fellow UPP allies."))
	to_chat(M, role_body(" Utilize it with your radio to prevent enemy radio interceptions."))
...
/datum/emergency_call/upp/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, JOB_SQUAD_LEADER, time_required_for_job))
		...
		to_chat(H, role_header("You are an Officer of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
		...
		synths++
		to_chat(H, role_header("You are a Combat Synthetic of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
		arm_equipment(H, /datum/equipment_preset/upp/synth, TRUE, TRUE)
	else if(medics < max_medics && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_MEDIC) && check_timelock(H.client, JOB_SQUAD_MEDIC, time_required_for_job))
		medics++
		to_chat(H, role_header("You are a Medic of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
		...
		engineers++
		to_chat(H, role_header("You are a Sapper of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
		...
		heavies++
		to_chat(H, role_header("You are a Sergeant of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
		...
		to_chat(H, role_header("You are a soldier of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
