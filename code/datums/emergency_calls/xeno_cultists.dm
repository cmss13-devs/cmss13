...
/datum/emergency_call/xeno_cult/print_backstory(mob/living/carbon/human/H)
	to_chat(H, role_body("The xenos are graced the Neroid Sector with their presence! It's time to spread their glory across the stars!"))
...
/datum/emergency_call/xeno_cult/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, JOB_SQUAD_LEADER, time_required_for_job))
		leader = H
		to_chat(H, role_header("You are the leader of this xeno cult! Bring glory to Queen Mother!"))
		...
		synths++
		to_chat(H, role_header("You are the xeno cult's synthetic! Tend to the Hive and the captured hosts, make sure the Hive grows!"))
		arm_equipment(H, /datum/equipment_preset/synth/survivor/cultist_synth, TRUE, TRUE)
	else
		to_chat(H, role_header("You are a xeno cultist! Follow the orders of the Queen Mother, the Queen, and your cult leader, in that order!"))
		arm_equipment(H, /datum/equipment_preset/other/xeno_cultist, TRUE, TRUE)
	print_backstory(H)

	addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), H, role_body("<b><center>Your objectives are:</center></b></p><p><b>[objectives]</b>")), 1 SECONDS)
