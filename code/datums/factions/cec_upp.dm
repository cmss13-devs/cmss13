/datum/faction/cec_upp
	name = "Cosmos Exploration Corps"
	faction_tag = FACTION_CEC
	base_icon_file = 'icons/mob/hud/factions/cec_upp.dmi'

/datum/faction/cec_upp/modify_hud_holder(image/holder, mob/living/carbon/human/human)
	...
	switch(_role)
		...
		if(JOB_CEC_TRAINEE)
			hud_icon_state = "assistant"
		if(JOB_CEC_DOCENT)
			hud_icon_state = "docent"
		if(JOB_CEC_PROFESSOR)
			hud_icon_state = "professor"
		if(JOB_CEC_VICE_RECTOR)
			hud_icon_state = "vice-rector"
		if(JOB_CEC_RECTOR)
			hud_icon_state = "rector"
	if(hud_icon_state)
