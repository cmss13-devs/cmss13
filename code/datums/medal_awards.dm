...
/proc/give_jelly_award(var/datum/hive_status/hive, var/as_admin = FALSE)
	...
	for(var/mob/living/carbon/Xenomorph/xeno in hive.totalXenos)
		if (xeno.persistent_ckey == usr.persistent_ckey) // Don't award self
			continue
		if (!as_admin && istype(xeno.caste, /datum/caste_datum/king)) // Don't award kings unless admin
			continue
	...
	for(var/mob/living/carbon/Xenomorph/xeno in hive.totalDeadXenos)
		if (xeno.persistent_ckey == usr.persistent_ckey) // Don't award previous selves
			continue
		if (!as_admin && istype(xeno.caste, /datum/caste_datum/king)) // Don't award previous kings unless admin
			continue
	...
	if(as_admin)
		admin_attribution = strip_html(input("Override the jelly attribution? Press cancel for no attribution.", "Jelly Attribution", "King Mother", null) as text|null, MAX_NAME_LEN)
