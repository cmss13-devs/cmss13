/datum/quadtree
	...
	var/list/datum/coords/player/player_coords = list()
	var/z_level
...
/datum/quadtree/Destroy(force, ...)
	QDEL_NULL(nw_branch)
	QDEL_NULL(ne_branch)
	QDEL_NULL(sw_branch)
	QDEL_NULL(se_branch)
	QDEL_NULL(boundary)
	QDEL_NULL_LIST(player_coords)
	return ..()

/datum/coords/player
	var/client/player
	var/list/datum/quadtree/trees

/datum/coords/player/New(client/C)
	. = ..()
	player = C
	C.qtrees += src
	trees = list()

/datum/coords/player/Destroy(force, ...)
	player.qtrees -= src
	player = null
	for(var/i in trees)
		var/datum/quadtree/QT = i
		QT.player_coords -= src
	return ..()
...
/datum/quadtree/proc/query_range(datum/shape/rectangle/range, list/found_players, flags = 0)
	...
	for(var/i in player_coords)
		var/datum/coords/player/P = i
