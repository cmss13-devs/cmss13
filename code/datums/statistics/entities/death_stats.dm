...
/mob/proc/track_mob_death(datum/cause_data/cause_data, turf/death_loc)
	...
	if(isxeno(cause_mob))
		track_statistic_earned(new_death.cause_faction_name, STATISTIC_TYPE_CASTE, new_death.cause_role_name, ff_type ? STATISTICS_KILL_FF : STATISTICS_KILL, 1, new_death.cause_player_id)
	else if(ishuman(cause_mob))
		track_statistic_earned(new_death.cause_faction_name, STATISTIC_TYPE_JOB, new_death.cause_role_name, ff_type ? STATISTICS_KILL_FF : STATISTICS_KILL, 1, new_death.cause_player_id)
		if(new_death.cause_role_name)
			track_statistic_earned(new_death.cause_faction_name, STATISTIC_TYPE_WEAPON, new_death.cause_role_name, ff_type ? STATISTICS_KILL_FF : STATISTICS_KILL, 1, new_death.cause_player_id)

	if(isxeno(src))
		track_statistic_earned(new_death.faction_name, STATISTIC_TYPE_JOB, new_death.role_name, ff_type ? STATISTICS_DEATH_FF : STATISTICS_DEATH, 1, new_death.player_id)
	else if(ishuman(src))
		track_statistic_earned(new_death.faction_name, STATISTIC_TYPE_JOB, new_death.cause_name, ff_type ? STATISTICS_DEATH_FF : STATISTICS_DEATH, 1, new_death.player_id)
		if(new_death.cause_name)
			track_statistic_earned(new_death.faction_name, STATISTIC_TYPE_WEAPON, new_death.cause_name, ff_type ? STATISTICS_DEATH_FF : STATISTICS_DEATH, 1, new_death.player_id)
