...
/datum/entity/statistic_round/process()
	...
	cached_tgui_data = list(
		...
		"game_mode" = game_mode,
		"map_name" = map_name,
		"round_result" = round_result ? round_result : "In Progress",
		"real_time_start" = real_time_start ? time2text(real_time_start) : "Not Started",
		"real_time_end" = real_time_end ? time2text(real_time_end) : "Not Ended",
		"round_length" = round_length ? duration2text(round_length) : "Not Ended",
		"round_hijack_time" = round_hijack_time ? time2text(round_hijack_time) : 0,
