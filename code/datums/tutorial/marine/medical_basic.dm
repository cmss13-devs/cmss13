...
/datum/tutorial/marine/medical_basic/proc/on_burn_inject(datum/source, obj/item/reagent_container/hypospray/injector)
	...
	message_to_player("Great job. Now, the third type of damage is 'oxy' damage. Oxy damage is an indicator your organs are struggling to receive oxygen, either due to blood loss, heart and lung injuries, or breathing in a low oxygen environment. To simulate this, we're forcing you to hold your breath for a very long time.")
...
/datum/tutorial/marine/medical_basic/proc/oxy_tutorial()
	message_to_player("<b>Dexalin Plus</b> is a miraculous medication that instantly removes all oxygen damage. Quickly, inject yourself with the <b>dexalin plus EZ autoinjector</b> before you pass out!")
...
/datum/tutorial/marine/medical_basic/proc/on_oxy_inject(datum/source, obj/item/reagent_container/hypospray/injector)
	...
	message_to_player("Amazing. Feels like you took a breath without taking one. You can breathe, now. The next type of damage is toxin damage, which accumulates from ingesting or breathing toxic chemicals, overdosing on medicine, drinking too much alcohol, and liver damage.")
	update_objective("")
	var/mob/living/living_mob = tutorial_mob
...
/datum/tutorial/marine/medical_basic/proc/on_tox_inject(datum/source, obj/item/reagent_container/hypospray/injector)
	...
	TUTORIAL_ATOM_FROM_TRACKING(/obj/item/reagent_container/hypospray/autoinjector/ez/tutorial/dexalinp, oxy_injector)
	remove_highlight(oxy_injector)
	message_to_player("Great work. Now, you may have noticed that toxins, in addition to other types of damage, cause <b>pain</b>. Pain slows you down and can knock you out if left unchecked.")
	update_objective("")
	var/mob/living/living_mob = tutorial_mob
...
/datum/tutorial/marine/medical_basic/proc/pain_tutorial()
	...
	var/obj/item/reagent_container/hypospray/autoinjector/skillless/tramadol/pain_injector = new(loc_from_corner(0, 4))
...
/datum/tutorial/marine/medical_basic/proc/on_pain_inject(datum/source, obj/item/reagent_container/hypospray/injector)
	...
	if(!istype(injector, /obj/item/reagent_container/hypospray/autoinjector/skillless/tramadol))
		return

	UnregisterSignal(tutorial_mob, COMSIG_LIVING_HYPOSPRAY_INJECTED)
	TUTORIAL_ATOM_FROM_TRACKING(/obj/item/reagent_container/hypospray/autoinjector/skillless/tramadol, pain_injector)
