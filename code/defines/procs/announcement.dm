...
/proc/marine_announcement(message, title = COMMAND_ANNOUNCE, var/sound_to_play = sound('sound/misc/notice2.ogg'), var/faction_to_display = FACTION_MARINE, var/add_PMCs = TRUE, var/signature)
...
/proc/yautja_announcement(message, title = YAUTJA_ANNOUNCE, var/sound_to_play = sound('sound/misc/notice1.ogg'))
	var/list/targets = GLOB.human_mob_list + GLOB.dead_mob_list
	for(var/mob/M in targets)
		if(isobserver(M))		//observers see everything
			continue
...
/proc/shipwide_ai_announcement(message, title = MAIN_AI_SYSTEM, var/sound_to_play = sound('sound/misc/interference.ogg'), var/signature)
	var/list/targets = GLOB.human_mob_list + GLOB.dead_mob_list
