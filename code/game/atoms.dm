...
/atom/vv_get_dropdown()
	...
	if(!ismovable(src))
		...
		if(curturf)
			. += "<option value='?_src_=player_data.admin_holder;[HrefToken(forceGlobal = TRUE)];adminplayerobservecoodjump=1;X=[curturf.x];Y=[curturf.y];Z=[curturf.z]'>Jump To</option>"
