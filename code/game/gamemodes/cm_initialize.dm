...
/datum/game_mode
	...
	var/latejoin_larva_drop = 12 //A larva will spawn in once the tally reaches this level. If set to 0, no latejoin larva drop
...
datum/game_mode/proc/initialize_special_clamps()
	...
	xeno_starting_num = Clamp((ready_players/6), xeno_required_num, INFINITY) //(n, minimum, maximum)
...
/datum/game_mode/proc/transform_predator(mob/pred_candidate)
	if(!pred_candidate.client) //Something went wrong.
		message_admins("<span class='warning'><em>Warning</em>: null client in transform_predator.</span>")
	...
	if(wants_elder)
		...
		spawn(10)
			to_chat(new_predator, "<span class='boldnotice'>Welcome Elder!</span>")
		...
		spawn(12)
			to_chat(new_predator, "<span class='notice'>You are <em>Yautja</em>, a great and noble predator!</span>")
...
/datum/game_mode/proc/initialize_starting_xenomorph_list()
	...
	if(possible_xenomorphs.len < xeno_required_num) //We don't have enough aliens.
		to_chat(world, "<h2 style=\"color:red\">Not enough players have chosen to be a xenomorph in their character setup. <em>Aborting</em>.</h2>")
		return
	...
	*/
	if(xenomorphs.len < xeno_required_num)
		to_chat(world, "<h2 style=\"color:red\">Could not find any candidates after initial alien list pass. <em>Aborting</em>.</h2>")
		return
...
/datum/game_mode/proc/transform_xeno(datum/mind/ghost_mind)
	...
	if(is_queen)
		to_chat(new_xeno, "<em>You are now the alien queen!</em>")
		to_chat(new_xeno, "<em>Your job is to spread the hive.</em>")
	...
	while(current_survivors.len)
		...
		if(current_survivors.len > 1) //If we have another survivor to pick from.
			if(survivor_multi_story.len) //Unlikely.
				...
				spawn(6)
					var/temp_story = "<em>Your story thus far</em>: " + replacetext(story, "{surv}", "[another_survivor.current.real_name]")
					to_chat(survivor.current, temp_story)
					survivor.memory += temp_story //Add it to their memories.
					temp_story = "<em>Your story thus far</em>: " + replacetext(story, "{surv}", "[survivor.current.real_name]")
			...
			if(survivor_story.len) //Shouldn't happen, but technically possible.
				...
				spawn(6)
					var/temp_story = "<em>Your story thus far</em>: " + replacetext(story, "{name}", "[random_name]")
					to_chat(survivor.current, temp_story)
