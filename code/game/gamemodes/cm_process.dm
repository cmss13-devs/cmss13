...
/datum/game_mode/proc/declare_completion_announce_medal_awards()
	...
	if(GLOB.jelly_awards.len)
		...
		for(var/recipient in GLOB.jelly_awards)
			...
			for(var/i in 1 to RA.medal_names.len)
				dat += "<br><b>[recipient]</b> is awarded [RA.posthumous[i] ? "posthumously " : ""]<span class='boldnotice'>[RA.medal_names[i]]</span>: \'<i>[RA.medal_citations[i]]</i>\' by [RA.giver_rank] [RA.giver_name]."
		to_world(dat)
