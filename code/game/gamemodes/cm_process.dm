...
/datum/game_mode/proc/declare_completion_announce_medal_awards()
	...
	if(GLOB.jelly_awards.len)
		...
		for(var/recipient in GLOB.jelly_awards)
			...
			for(var/i in 1 to recipient_award.medal_names.len)
				dat += "<br><b>[recipient]</b> is awarded [recipient_award.posthumous[i] ? "posthumously " : ""]<span class='boldnotice'>[recipient_award.medal_names[i]]</span>: \'<i>[recipient_award.medal_citations[i]]</i>\'[recipient_award.giver_name[i] ? " by [recipient_award.giver_rank[i]] [recipient_award.giver_name[i]]" : ""]."
		to_world(dat)
