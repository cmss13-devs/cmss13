...
/datum/game_mode/proc/spawn_latejoin_larva()
	...
	if(GLOB.pathogen_round)
		new_xeno = new /mob/living/carbon/xenomorph/bloodburster(get_turf(pick(GLOB.xeno_spawns)))
	else
		new_xeno = new /mob/living/carbon/xenomorph/larva(get_turf(pick(GLOB.xeno_spawns)))
