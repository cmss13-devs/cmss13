...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_THE_LAST_BUNKER)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Attention: Incoming intelligence report:\nThis is Fourth Battalion, we've concluded our search and recovery of Bunker C01. All clear here, broken transmission tower. VIP's are clean. We're assisting with local repairs and will depart within the day. Out.\nSeventh Battalion, 'Sweeping Seventh' here. Bunker D61 is empty, no one tried to come here. We're departing to the rendezvous point, ETA, 3 days. Out.\nThis is 'Lucky First', First Battalion, We're engaged in Riot Control, attempted mutiny within Bunker F12 due to food shortage. No casualties thus far, VIPs are safe. No artefacts on them. We'll be here for a few days. Out.", "ARES V3.2", 'sound/AI/commandreport.ogg'), 10 SECONDS)
