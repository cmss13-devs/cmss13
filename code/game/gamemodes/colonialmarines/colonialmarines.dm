...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_NORTHPOINT_HOSPITAL)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(venir_announcement), "To anyone who can hear this. A rescue team is being prepared to deploy to the quarantine area of Northpoint Hospital. All civilians are instructed to barricade themselves in place and await rescue.", "Public Broadcast", 'sound/AI/commandreport.ogg'), 10 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "My children. Our situation on this world is dire. The hosts of this world have diminished our hive and contained us behind these concrete walls. However, through your earlier efforts, we have exhausted these hosts. Take this time to rebuild your strength. No mere wall of concrete will contain us for long.", "everything", QUEEN_MOTHER_ANNOUNCE), 10 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "USS Almayer, this is Third Fleet Command. We are authorized to reveal that intelligence has acertained that the colony of New Leeds is suffering from a Xenomorph outbreak. Our liaison at TWE Imperial High command has informed us that local IASF forces have contained the outbreak within the ground of Northpoint Hospital, the colonies only major medical centre. We have a limited window to eliminate the threat and secure the colony before they breach the quarantine. Your objective is search and destroy. Further orders pending. Good luck Falling Faclons.", "Third Fleet Command", 'sound/AI/commandreport.ogg'), 5 MINUTES)
