...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		if(MAP_TYRARGO_RIFT)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Local command, we're detecting several still living vital signs within the Tyrango Museum evacuation site via our thermal scanners, the bugs may not have gotten to them yet. We're en-route to sector THX-113, we'll see what we can spot when we return to your sector. Out.", "EVAC-3 Recon Flight", 'sound/AI/commandreport.ogg'), 160 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Almayer. We're doing a final sweep of the Civilain Evac site. Our scanners show a single thermal signiture. Its faint, likely a child. We're not seeing anything else. We are bingo for fuel, we're oscar mike, out.", "EVAC-3 Recon Flight", 'sound/AI/commandreport.ogg'), 180 MINUTES)
