...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_LV_624)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Attention. \n\n Planetary scan indicates a localized atmospheric anomaly: a dense fog forming over the river surface. \n Initial assessment predicts dissipation in T-20 minutes.", "ARES V3.2", 'sound/AI/commandreport.ogg'), 5 MINUTES) // 5 minute lobby + 5 minutes into the game means the fog drops 20 minutes from now.
