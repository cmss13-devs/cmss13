...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_NEW_VARADERO)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "My children. Your hive has become separated by a great aquatic barrier, and as such I have divided your hive into two. Your sub-hive will defend the island, you will defeat the sky hosts above and ensure they do not claim this island as it is the hosts only form of escape whilst the other sub-hive overtakes the defenses of this world’s hosts.", "everything", QUEEN_MOTHER_ANNOUNCE), 20 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "US#M fo#$#, this is N$% Var##-- ##&@ion #$%^-------------------#ou r#$#? Rep#*t, ###% i# the New Varadero Gararrison’s Acting Commander Lieute$an# #--#$#-------- #% you rea##.\n\n Base has been overrun by some kinda ’bug men’ about half of our forces are K and or M, I.A.\n\n Me and what remains of the garrison are set up in entrenchements across the coast, long side of a collapsed tunnel.\n\n Monsoon weather right now means all areospace craft are grounded outside UDs, and even then they’ll need a navbeacon, only place with one is at the base. If you want to get us aid, best bet is to secure the base it self. Over", "New Varadero, LACN One-Way Emergency Relay",'sound/AI/commandreport.ogg'), 5 MINUTES)
