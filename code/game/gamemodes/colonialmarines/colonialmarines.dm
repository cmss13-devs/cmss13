...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_NEW_VARADERO)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "My children. Your hive has become seperated by a great auqatic barrier, and as such I have divided your hive into two. Your sub-hive will defend the island, you will defeat the sky hosts above and ensure they do not claim this island as it is the hosts only form of escape whilst the other sub-hive overtakes the defences of this world’s hosts.", "everything", QUEEN_MOTHER_ANNOUNCE), 20 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "US#M fo#$#, this is N$% Var##-- ##&@ion #$%^-------------------#ou r#$#? Rep#*t, ###% i# the New Varadero Gararrison’s Acting Commander Lieute$an# #--#$#-------- #% you rea##.\n\n Base has been overrun by some kinda ’bug men’ about half of our forces are K and or M, I.A.\n\n Me and what remains of the garrison are set up in entrenchements across the coast, long side of a collapsed tunnel.\n\n Monsoon weather right now means all areospacecraft are grounded outside UDs, and even then they’ll need a navbeacon, only place with one is at the base. If you want to get us aid, best bet is to secure the base it self. Over", "New Varadero, LACN One-Way Emergency Relay",'sound/AI/commandreport.ogg'), 5 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Almayer! You still re## &@? We’re trying to g&@ through to you #$# two-way communic##%#ns we got, however seems theres some kinda radio interferance on anything weaker then this emergency relay this far from the base's comms towers during this stormy weath#r.\n\n Regardless, local law enforcement is lending a hand here in coastal quarantine, it should hold for now untill you can secure the base. Over.", "New Varadero, LACN One-Way Emergency Relay",'sound/AI/commandreport.ogg'), 20 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "#@@#%-- %-%#--- #%#%#-? #%^#^--- situ#&@on is worsening, gia#$ lobst#$ like bug me# have cropped up too, morale ain’t the best right now. we’re doing our best to hold however have had to expand the qurantine out northward to the city’s dockyards\n\n God speed marines, turn off this fauscet. Over.", "New Varadero, LACN One-Way Emergency Relay",'sound/AI/commandreport.ogg'), 45 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "USC#, Status report, we're still holding, albeit barely.\n\n Lobster looking bastards found a way past the quarantine, now the city’s doc#yards under siege by f##%^## cra# m#$.\n\n Good news however, seems these crab bastards dont climb well, so I’ve ordered the peacekeeper precinct  in the dockyards to  use cargo containers and trams as gun platforms.\n\n Praying you lot are close to victory. Over.", "New Varadero, LACN One-Way Emergency Relay",'sound/AI/commandreport.ogg'), 70 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Private First #l%ss To#r#@, Lieute$### is KIA. We’ve reclaimed the relay station! \n\n Situation isn’t the best honestlyI’ve designated Saint Ferrer’s Hospital as a rally point, if this monsoon ever lets up, get a dropship lan#%#$ on the roo# sta#.\n\n New Varadero’s citizens are still holding out hope for you lot to s#%%re us a# exit and so am I! Over.", "New Varadero, LACN One-Way Emergency Relay",'sound/AI/commandreport.ogg'), 110 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Marines? You still the#$? Private First #l%ss To#r#^ ER and childrens ward have had their roofs melted by acid. However rel#% %#$ is surrou%#%, I’m c#% o## with a #%^ bu# #i#%#^-#h f##%---#^#@$--#%^-fa#l%##^#-------", "New Varadero, LACN One-Way Emergency Relay",'sound/AI/commandreport.ogg'), 140 MINUTES)
