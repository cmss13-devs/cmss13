...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_THE_LAST_BUNKER)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(venir_announcement), "If anyone is still alive, I've lifted the exterior lockdown. I've setup a fortified position directly west of the main entrance, find me, I've setup sentry guns, make sure you are wearing IFF-ID tags....wait, where's mine, oh fu---", "Facility Transmission", 'sound/AI/commandreport.ogg'), 10 MINUTES)
