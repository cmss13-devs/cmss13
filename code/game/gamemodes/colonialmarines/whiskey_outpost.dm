...
/datum/game_mode/whiskey_outpost/proc/story_announce(time)
	switch(time)
		if(0)
			marine_announcement("This is Captain Hans Naiche, Commander of the 3rd Battailion, 'Dust Raiders' forces on LV-624. As you already know, several of our patrols have gone missing and were likely wiped out by hostile local creatures as we attempted to set up our base.", "Captain Naiche, 3rd Battalion Command, LV-624 Garrison")
		if(1)
...
/datum/game_mode/whiskey_outpost/declare_completion()
	...
	if(finished == 1)
		...
		to_world(SPAN_ROUNDBODY("Eventually, the Dust Raiders secure LV-624 and the entire Neroid Sector in 2182, pacifying it and establishing peace in the sector for decades to come."))
