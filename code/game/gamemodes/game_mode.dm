...
/datum/game_mode/proc/post_setup()
	...
	if(flags_round_type & MODE_SUNSET)
		do_sunset(sunrise=FALSE)

	return TRUE

/datum/game_mode/proc/do_sunset(sunrise)
	var/list/cold_map_light_progression = list(
		"#a8c3cf", "#9db9ca", "#92afc5", "#87a5c0",
		"#7a9abb", "#7592b6", "#708ab1", "#6b82ac", 
		"#6679a8", "#6175a1", "#5c719a", "#576d93", 
		"#516a8b", "#4b6284", "#455a7d", "#3f5276",
		"#38486e", "#354266", "#323c5e", "#2f3656", 
		"#2c2f4d", "#292a47", "#262541", "#23203b",
		"#211b36", "#211b35", "#211b34", "#211b33",
		"#1f1b33", "#1a172d", "#151327", "#100f21", 
		"#0c0a1b", "#090814", "#06060d", "#030406", 
		"#000000", "#000000", "#000000", "#000000",
	)

	var/list/hot_map_light_progression = list(
		"#e3a979", "#e3a471", "#e39f69", "#e39a61",
		"#e29658", "#e09355", "#de9052", "#dc8d4f",
		"#da8b4a", "#ce8147", "#c27744", "#b66d41", 
		"#a9633c", "#a35b38", "#9d5334", "#974b30", 
		"#90422d", "#863e30", "#7c3a33", "#723636", 
		"#68333a", "#613139", "#5a2f38", "#532d37",
		"#4d2b35", "#432735", "#392335", "#2f1f35", 
		"#231935", "#1c1632", "#15132f", "#0e102c",
		"#050c27", "#04091d", "#030613", "#020309",
		"#000000", "#000000", "#000000", "#000000",
	)

	var/list/sunrise_light_progression = list(
		"#000000", "#010205", "#02040a", "#03060f",
		"#040712", "#070a16", "#0a0d1a", "#0d101e",
		"#111322", "#17142a", "#1d1532", "#23163a",
		"#291642", "#2f1940", "#351c3e", "#3b1f3c",
		"#3f2239", "#48253a", "#51283b", "#5a2b3c",
		"#632c3d", "#793d3b", "#8f4e39", "#a55f37",
		"#b97034", "#b97034", "#b97034", "#b97034",
	)
	...
	var/list/light_progression

	if(sunrise)
		light_progression = sunrise_light_progression
	else
		light_progression = is_cold ? cold_map_light_progression : hot_map_light_progression

	var/list/area/affected_areas = list()

	for(var/area/current_area in GLOB.all_areas_once)
		if(!is_ground_level(current_area.z))
			continue

		if(CEILING_IS_PROTECTED(current_area?.ceiling, CEILING_PROTECTION_TIER_2))
			continue

		if(current_area in affected_areas)
			continue

		affected_areas += current_area

	for(var/area/affected_area in affected_areas)
		var/index = 0
		if(!sunrise)
			affected_area.set_base_lighting(affected_area.base_lighting_color, 255)

		for(var/lighting_color in light_progression)
			var/alpha = sunrise ? 255 / length(light_progression) * (index+1) : 255 - 255 / length(light_progression) * (index+1)
			addtimer(CALLBACK(src, PROC_REF(animate_area), affected_area, lighting_color, alpha), index * (3 MINUTES / length(light_progression)))
			index++
