...
/datum/job/civilian/working_joe/announce_entry_message(mob/living/carbon/human/H)
	addtimer(CALLBACK(GLOBAL_PROC, .proc/ai_announcement, "[H.real_name] has been deployed to help with operations."), 1.5 SECONDS)
	addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(all_hands_on_deck), "ARES has deployed [H.real_name] to help with operations."), 1.5 SECONDS)
	return ..()
