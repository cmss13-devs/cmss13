/datum/job/command/tank_crew
	...
	// ROLE_ADD_TO_SQUAD defaults them to Foxtrot, but they're still transferable to any squad afterward.
...
/datum/job/command/tank_crew/get_total_positions(latejoin = FALSE)
	if(SStechtree.trees[TREE_MARINE].get_node(/datum/tech/arc).unlocked)
		return 0
	if(length(GLOB.clients) >= 130 || total_positions_so_far > 0)
		return 2
