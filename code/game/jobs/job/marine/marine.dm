...
/datum/job/marine/generate_entry_message(mob/living/carbon/human/current_human)
	if(current_human.assigned_squad)
		entry_message_intro = "<p>You are a [title]!</p>You have been assigned to: <b><font color=[current_human.assigned_squad.equipment_color]>[current_human.assigned_squad.name] Squad!</font></b>[Check_WO() ? "" : " Make your way to the cafeteria for some post-cryosleep chow, and then get equipped in your squad's prep room." ]"
	return ..()
