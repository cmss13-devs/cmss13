/obj/item/device/ai_tech_pda/ui_data(mob/user)
	...
	if(set_ui == "AresAdmin")
		data["ares_access_text"] = "[link.interface.sudo_holder ? "(SUDO)," : ""] access level [link.interface.authentication], [link.interface.ares_auth_to_text(link.interface.authentication)]."
		data["ares_logged_in"] = link.interface.last_login
