...
/obj/structure/machinery/optable/buckle_mob(mob/living/carbon/human/human, mob/living/user)
	...
	var/obj/item/clothing/mask/breath/medical/mask = new()
	if(!human.equip_if_possible(mask, WEAR_FACE, TRUE))
...
/obj/structure/machinery/optable/process()
	...
	if(human.blood_volume < BLOOD_VOLUME_SAFE)
		if(!(patient_exam & PATIENT_LOW_BLOOD))
			visible_message("[icon2html(src, viewers(src))] <b>\The [src] beeps,</b> Warning: Patient has a dangerously low blood level: [floor(human.blood_volume / BLOOD_VOLUME_NORMAL * 100)]%. Type: [human.blood_type].")
	...
	if(human.nutrition < NUTRITION_LOW)
		if(!(patient_exam & PATIENT_LOW_NUTRITION))
			visible_message("[icon2html(src, viewers(src))] <b>\The [src] beeps,</b> Warning: Patient has a dangerously low nutrition level: [floor(human.nutrition / NUTRITION_MAX * 100)]%.")
...
/obj/structure/machinery/optable/proc/take_victim(mob/living/carbon/humanoid, mob/living/carbon/user)
	if (humanoid == user)
		...
		visible_message(SPAN_NOTICE("[humanoid] has been laid on the operating table by [user]."), null, 4)
	humanoid.resting = 1
	humanoid.forceMove(loc)
