...
/obj/machinery/body_scanconsole/Topic(href, href_list)
	...
	if (href_list["print"])
		if (!src.connected)
			to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Error: No body scanner connected.</span>")
			return
		var/mob/living/carbon/human/occupant = src.connected.occupant
		if (!src.connected.occupant)
			to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>The body scanner is empty.</span>")
			return
		if (!istype(occupant,/mob/living/carbon/human))
			to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>The body scanner cannot scan that lifeform.</span>")
			return
