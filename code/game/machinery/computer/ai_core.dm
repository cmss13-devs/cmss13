				...
				switch(interaction)
					if("AICARD")
						...
						if(C.contents.len)//If there is an AI on card.
							to_chat(U, "<span class='danger'>Transfer failed</span>: \black Existing AI found on this terminal. Remove existing AI to install a new one.")
						else
					...
					if("AICARD")
						...
						if(!T.contents.len)
							...
							if(!C.contents.len && T.occupant && !T.active)
								...
								to_chat(U, "<span class='danger'>ERROR</span>: \black Artificial intelligence detected on terminal.")
							else if (T.active)
								to_chat(U, "<span class='danger'>ERROR</span>: \black Reconstruction in progress.")
							else if (!T.occupant)
								to_chat(U, "<span class='danger'>ERROR</span>: \black Unable to locate artificial intelligence.")
	else
		to_chat(U, "<span class='danger'>ERROR</span>: \black AI flush is in progress, cannot execute transfer protocol.")
	return
