	...
	// sd \\
		...
		if("destroy")
			...
			for(var/client/admin as anything in GLOB.admins)
				if(check_client_rights(admin, R_ADMIN|R_MOD, FALSE))
					admin << 'sound/effects/sos-morse-code.ogg'
			message_admins("[key_name(user)] has requested Self-Destruct! [CC_MARK(user)] (<A HREF='?_src_=player_data.admin_holder;[HrefToken(forceGlobal = TRUE)];destroyship=\ref[user]'>GRANT</A>) (<A HREF='?_src_=player_data.admin_holder;[HrefToken(forceGlobal = TRUE)];sddeny=\ref[user]'>DENY</A>) [ADMIN_JMP_USER(user)] [CC_REPLY(user)]")
