...
/obj/structure/machinery/computer/demo_sim/proc/simulate_detonation(mob/living/user)
	...
	//Mortars
		...
		if(istype(configuration,/obj/item/mortar_shell/custom))//we only need the warhead
			...
			var/obj/item/mortar_shell/custom_explosive = new configuration.type(get_turf(simulation.sim_camera))
			custom_explosive.detonate(get_turf(simulation.sim_camera))
...
/obj/structure/machinery/computer/demo_sim/proc/make_and_prime_explosive(obj/item/explosive/custom_explosive)
	var/obj/item/explosive/explosive = new custom_explosive.type(get_turf(simulation.sim_camera))
