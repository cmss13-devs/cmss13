...
/obj/structure/machinery/computer/message_monitor/attack_hand(var/mob/living/user as mob)
	...
	if(!istype(user))
		return
	//If the computer is being hacked, display the reboot message.
	if(hacking)
		message = rebootmsg
	...
	switch(screen)
		...
		if(0)
			...
			if(ishighersilicon(user) && (user.mind.special_role && user.mind.original == user))
					...
					if("Send")
						...
						if(isnull(PDARec))
							...
							if (!customrecepient.silent)
								...
								for (var/mob/O in hearers(3, customrecepient.loc))
									O.show_message(text("[htmlicon(customrecepient, O)] *[customrecepient.ttone]*"))
								if( customrecepient.loc && ishuman(customrecepient.loc) )
									var/mob/living/carbon/human/H = customrecepient.loc
									to_chat(H, "[htmlicon(customrecepient, H)] <b>Message from [customsender] ([customjob]), </b>\")[custommessage]\" (<a href='byond://?src=\ref[src];choice=Message;skiprefresh=1;target=\ref[src]'>Reply</a>")
						...
						//Sender is faking as someone who exists
							...
							if (!customrecepient.silent)
								...
								if( customrecepient.loc && ishuman(customrecepient.loc) )
									var/mob/living/carbon/human/H = customrecepient.loc
									to_chat(H, "[htmlicon(customrecepient, H)] <b>Message from [PDARec.owner] ([customjob]), </b>\")[custommessage]\" (<a href='byond://?src=\ref[customrecepient];choice=Message;skiprefresh=1;target=\ref[PDARec]'>Reply</a>")
