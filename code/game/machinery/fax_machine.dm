...
/obj/structure/machinery/faxmachine/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("insertpaper")
			var/jammed = FALSE
			var/obj/item/current_item = ui.user.get_active_hand()
			if(istype(current_item, /obj/item/paper_bundle))
				var/obj/item/paper_bundle/bundle = current_item
				if(bundle.amount > 5)
					jammed = TRUE
				// Repeating code? This is not ideal. Why not put this functionality inside of a proc?
			if(istype(current_item, /obj/item/paper) || istype(current_item, /obj/item/paper_bundle) || istype(current_item, /obj/item/photo))
				ui.user.drop_inv_item_to_loc(current_item, src)
				original_fax = current_item
				if(!jammed)
...
/obj/structure/machinery/faxmachine/proc/send_fax(datum/fax/faxcontents)
	for(var/obj/structure/machinery/faxmachine/machine in allfaxes)
		if(machine != src && machine.department == target_department)
			if(!faxcontents)
				return
			if(!(machine.inoperable()))
				...
				spawn(20)
					var/obj/item/paper/new_fax = new(machine.loc,faxcontents.photo_list)
					new_fax.name = "faxed message"
					new_fax.info = "[faxcontents.data]"
					new_fax.update_icon()
					...
					switch(network)
						...
						if("NC4 UA Federal Secure Network - CMB Relay")
							...
							new_fax.stamps += "<HR><i>This paper has been stamped by The Office of Colonial Marshals.</i>"
