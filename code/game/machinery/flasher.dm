...
/obj/structure/machinery/flasher/attackby(obj/item/wirecutters as obj, mob/user as mob)
	if (HAS_TRAIT(wirecutters, TRAIT_TOOL_WIRECUTTERS))
		add_fingerprint(user)
		src.disable = !src.disable
		if (src.disable)
			user.visible_message(SPAN_DANGER("[user] has disconnected's flashbulb!"), SPAN_DANGER("You disconnect's flashbulb!"))
		if (!src.disable)
			user.visible_message(SPAN_DANGER("[user] has connected's flashbulb!"), SPAN_DANGER("You connect's flashbulb!"))
