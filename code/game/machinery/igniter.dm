...
/obj/machinery/sparker/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/tool/screwdriver))
		...
		if (src.disable)
			user.visible_message("<span class='warning'>[user] has disabled the [src]!", "<span class='warning'>You disable the connection to the [src].</span></span>")
			icon_state = "[base_state]-d"
		if (!src.disable)
			user.visible_message("<span class='warning'>[user] has reconnected the [src]!", "<span class='warning'>You fix the connection to the [src].</span></span>")
			if(src.powered())
				icon_state = "[base_state]"
			else
				icon_state = "[base_state]-p"
