	...
	if(istype(user, /mob/living/carbon/human) || istype(user,/mob/living/silicon) )
		...
		switch(screen)
			if(0)
				...
				dat += "<BR><FONT SIZE=1>Property of The Company</FONT>"
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (issilicon(usr)))
		...
		if(href_list["set_channel_name"])
			...
			while (findtext(src.channel_name," ") == 1)
				src.channel_name = copytext(src.channel_name,2,text_length(src.channel_name)+1)
			src.updateUsrDialog()
			//src.update_icon()
			...
			while (findtext(src.msg," ") == 1)
				src.msg = copytext(src.msg,2,text_length(src.msg)+1)
			src.updateUsrDialog()
