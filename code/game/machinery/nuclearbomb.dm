			...
			if(do_after(usr, 50, INTERRUPT_NO_NEEDHAND, BUSY_ICON_HOSTILE))
				...
				if(timing)
					if(!safety)
						...
						start_processing()
						announce_to_players()
						message_admins("[src] has been activated by [key_name(usr, 1)][ADMIN_JMP_USER(usr)]")
					else
						bomb_set = FALSE
				else
					disable()
					message_admins("[src] has been deactivated by [key_name(usr, 1)][ADMIN_JMP_USER(usr)]")
...
/obj/structure/machinery/nuclearbomb/Destroy()
	if(timing != -1)
		message_admins("[src] has been unexpectedly deleted at ([x],[y],[x]). [ADMIN_JMP(src)]")
