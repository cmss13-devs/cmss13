...
/obj/structure/machinery/telecomms/relay/preset/tower/mapcomms/attack_hand(mob/user)
	...
	if(on) //now, if it went on it now uses power
		use_power = 1
		message_admins("[key_name(user)] turned \the [src] in [commarea] ON. ([commloc ? "<a href='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[commloc.x];Y=[commloc.y];Z=[commloc.z]'>JMP</a>" : "null"])")
	else
		use_power = 0
		message_admins("[key_name(user)] turned \the [src] in [commarea] OFF. ([commloc ? "<a href='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[commloc.x];Y=[commloc.y];Z=[commloc.z]'>JMP</a>" : "null"])")
