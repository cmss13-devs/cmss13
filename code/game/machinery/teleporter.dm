		...
		if(istype(L, /obj/effect/landmark/) && istype(L.loc, /turf))
			...
			if(C.data == "Clown Land")
				...
				for(var/mob/O in hearers(src, null))
					O.show_message("<span class='danger'>Incoming bluespace portal detected, unable to lock in.</span>", 2)
...
/obj/machinery/teleport/hub/proc/teleport(atom/movable/M as mob|obj)
	...
	if (!com.locked)
		for(var/mob/O in hearers(src, null))
			O.show_message("<span class='danger'>Failure: Cannot authenticate locked on coordinates. Please reinstate coordinate matrix.</span>")
		return
...
/proc/do_teleport(atom/movable/M as mob|obj, atom/destination, precision)
	...
	if (istype(M, /obj/item/disk/nuclear)) // Don't let nuke disks get teleported --NeoFite
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span class='danger'><B>The [] bounces off of the portal!</B></span>", M.name), 1)
		return
	...
	if (disky)
		for(var/mob/P in viewers(M, null))
			P.show_message(text("<span class='danger'><B>The [] bounces off of the portal!</B></span>", M.name), 1)
		return
