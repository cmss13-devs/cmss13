obj/effect/decal/cleanable/liquid_fuel
	...
	New(turf/newLoc, amt = 1, nologs = 0)
		if(!nologs)
			message_admins("[amt] units of liquid fuel have spilled in [newLoc.loc.name] ([newLoc.x],[newLoc.y],[newLoc.z]) (<A HREF='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[newLoc.x];Y=[newLoc.y];Z=[newLoc.z]'>JMP</a>)")
