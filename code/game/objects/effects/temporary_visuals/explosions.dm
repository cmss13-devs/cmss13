...
/obj/effect/temp_visual/explosion/Initialize(mapload, radius, small = FALSE, large = FALSE, tiny = FALSE, color = LIGHT_COLOR_LAVA)
	. = ..()
	set_light(radius, radius, color)
	generate_particles(radius, small, large, tiny)
	var/turf/turf = get_turf(src)
	if(turf && (turf.turf_flags & TURF_IS_WATER))
		icon_state = null
		return
	var/image/I
	if(tiny)
...
/obj/effect/temp_visual/explosion/proc/generate_particles(radius, small = FALSE, large = FALSE, tiny = FALSE)
	...
	if(turf_type && (turf_type.turf_flags & TURF_IS_WATER))
		if(tiny)
