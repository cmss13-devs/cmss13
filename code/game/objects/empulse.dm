...
/proc/log_emp(mob/living/affected, datum/cause_data/cause_data)
	...
	// One human EMP'd another, be worried about it but do everything basically the same
		...
		var/ff_msg = "[key_name(firing_mob)] EMP'd [key_name(affected)] with \a [source] in [get_area(location)] [ADMIN_JUMP_COORDS(location.x, location.y, location.z)] [ADMIN_JMP_USER(firing_mob)] [ADMIN_PM(firing_mob)]"
		var/ff_living = TRUE
