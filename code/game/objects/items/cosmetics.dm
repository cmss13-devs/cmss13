...
/obj/item/tattoo_gun
	gender = PLURAL
	name = "tattoo gun"
	desc = "A device with attached multi-ink canister used for tattooing, be it self-done or done on someone else. It can even work on good-quality synthetic skin!"
	icon = 'icons/obj/items/paint.dmi'
	icon_state = "camo"
	w_class = SIZE_TINY
...
/obj/item/tattoo_gun/attack(mob/target, mob/user)
	if(user.a_intent == INTENT_HARM)
		return ..()
	...
	if(!COOLDOWN_FINISHED(src, apply_delay)) // Stops players from spamming each other with popups.
		to_chat(user, SPAN_WARNING("You just attempted to do a tattoo, slow down!"))
		return FALSE
	...
	if(ishuman(target))
		...
		if(tattoo_location_check(human_target, tattoo_location))
			to_chat(user, SPAN_WARNING("There is already tattoos where this tattoo will go!"))
			return FALSE

		if(human_target == user)
			apply_tattoo(human_target, user)
			return TRUE
		else
			to_chat(user, SPAN_NOTICE("You start to tattoo [src] on [human_target]..."))
			to_chat(human_target, SPAN_NOTICE("[user] is trying to tattoo [src] on you..."))
			if(alert(human_target,"Will you allow [user] to tattoo you?",,"Sure","No") == "Sure")
				if(user && loc == user && (user in range(1,human_target)) ) //Have to be close and hold the thing.
					apply_tattoo(human_target, user)
					return TRUE

/obj/item/tattoo_gun/proc/tattoo_location_check(mob/living/carbon/human/human_target, tattoo_location)
	var/already_tattooed = FALSE
	switch(tattoo_location)
		if("chest")
			if(human_target.chest_tattoo)
				already_tattooed = TRUE
			if(human_target.back_tattoo)
				already_tattooed = TRUE
		if("l_arm")
			if(human_target.left_arm_tattoo)
				already_tattooed = TRUE
		if("l_leg")
			if(human_target.left_leg_tattoo)
				already_tattooed = TRUE
		if("r_arm")
			if(human_target.right_arm_tattoo)
				already_tattooed = TRUE
		if("r_leg")
			if(human_target.right_leg_tattoo)
				already_tattooed = TRUE
		if("head")
			if(human_target.head_tattoo)
				already_tattooed = TRUE

	return already_tattooed


/obj/item/tattoo_gun/proc/apply_tattoo(mob/living/carbon/human/human_target, mob/user)
	if(!human_target || !user)
		return

	var/obj/limb/target_part
	if(!human_target.has_limb(tattoo_location)) // No limb? No tattoo
		return

	target_part = human_target.get_limb(tattoo_location)
	if((target_part.status & LIMB_ROBOT) && !(human_target.species && human_target.species.flags & IS_SYNTHETIC)) // Prosthetics don't get tats, Synths can
		return

	switch(tattoo_location)
		if("chest")
			if(tattoo_specific_location == "chest")
				human_target.chest_tattoo = tattoo
			if(tattoo_specific_location == "back")
				human_target.back_tattoo = tattoo
		if("l_arm")
			human_target.left_arm_tattoo = tattoo
		if("l_leg")
			human_target.left_leg_tattoo = tattoo
		if("r_arm")
			human_target.right_arm_tattoo = tattoo
		if("r_leg")
			human_target.right_leg_tattoo = tattoo
		if("head")
			human_target.head_tattoo = tattoo

	human_target.update_body()

	user.visible_message(SPAN_NOTICE("[user] starts to apply [src] on [human_target]."), SPAN_NOTICE("You apply [src]."))

/obj/item/tattoo_gun/uscm
	name = "USCM chest tattoo"
	desc = "A tattoo gun with a pre-loaded pattern. This one is the good ol' USCM logo!"
	icon = 'icons/obj/items/paint.dmi'
	icon_state = "camo"
	tattoo = "uscm_chest"
...
/obj/item/tattoo_gun/arm_sleeve
	name = "arm sleeve tattoo"
	desc = "A tattoo gun with a pre-loaded pattern. This one has an arm sleeve!"
	icon = 'icons/obj/items/paint.dmi'
	icon_state = "camo"
