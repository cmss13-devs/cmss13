...
/obj/item/device/binoculars/designator/proc/lasering(mob/living/carbon/human/user, atom/targeted_atom, params)
	...
	message_admins("ALERT: [user] ([user.key]) IS CURRENTLY LASING A TARGET: CURRENT MODE [las_mode], at ([current_turf.x],[current_turf.y],[current_turf.z]) [ADMIN_JMP(current_turf)].") // Alert all the admins to this asshole. Added the jmp command from the explosion code.
