...
/obj/item/device/flashlight/flare/signal/activate_signal(mob/living/carbon/human/user)
	...
	if(faction && cas_groups[faction])
		...
		msg_admin_niche("Flare target [src] has been activated by [key_name(user, 1)] at ([target_turf.x], [target_turf.y], [target_turf.z]). (<A HREF='?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP LOC</a>)")
