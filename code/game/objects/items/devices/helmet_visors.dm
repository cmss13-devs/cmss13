...
/obj/item/device/helmet_visor/proc/toggle_visor(obj/item/clothing/head/helmet/marine/attached_helmet, mob/living/carbon/human/user, silent = FALSE)
	if(attached_helmet != user.head)
		to_chat(user, SPAN_NOTICE("Wear it on your head first")) // This lets you change through the visors WITHOUT the effects, meaning you can still interact with it and whatever you had it on will be applied once you put it on, better then just making the helmet uninteractable.
		return FALSE
