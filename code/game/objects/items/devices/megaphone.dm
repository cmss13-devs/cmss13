...
/obj/item/device/megaphone/attack_self(mob/living/user)
	if (user.client)
		if(user.client.prefs.muted & MUTE_IC)
			...
			return
	if(!ishuman(user))
		to_chat(user, "<span class='danger'>You don't know how to use this!</span>")
		return
	if(user.silent)
		return
	...
	if(spamcheck)
		to_chat(user, "<span class='danger'>\The [src] needs to recharge!</span>")
		return
	...
	if ((src.loc == user && usr.stat == 0))
		if(emagged)
			if(insults)
				...
				to_chat(user, "<span class='danger'>*BZZZZzzzzzt*</span>")
		else
