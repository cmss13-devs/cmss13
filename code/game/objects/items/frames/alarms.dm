...
/obj/item/frame/air_alarm/proc/try_build(turf/on_wall)
	...
	if (!istype(loc, /turf/open/floor))
		to_chat(usr, "<span class='danger'>Air Alarm cannot be placed on this spot.</span>")
		return
	if (A.requires_power == 0 || A.name == "Space")
		to_chat(usr, "<span class='danger'>Air Alarm cannot be placed in this area.</span>")
		return
...
/obj/item/frame/fire_alarm/proc/try_build(turf/on_wall)
	...
	var/area/A = loc.loc
	if (!istype(loc, /turf/open/floor))
		to_chat(usr, "<span class='danger'>Fire Alarm cannot be placed on this spot.</span>")
		return
	if (A.requires_power == 0 || A.name == "Space")
		to_chat(usr, "<span class='danger'>Fire Alarm cannot be placed in this area.</span>")
		return
