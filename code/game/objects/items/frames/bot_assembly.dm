...
/obj/item/frame/bucket_sensor/Initialize(mapload, ...)
	. = ..()
	ADD_TRAIT(src, TRAIT_WRITABLE, TRAIT_SOURCE_INHERENT)

/obj/item/frame/bucket_sensor/write(obj/item/writer, mob/user, mods, color, crayon)
...
/obj/item/frame/toolbox_tiles/Initialize(mapload, ...)
	. = ..()
	ADD_TRAIT(src, TRAIT_WRITABLE, TRAIT_SOURCE_INHERENT)

/obj/item/frame/toolbox_tiles/write(obj/item/writer, mob/user, mods, color, crayon)
	var/t = copytext(stripped_input(user, "Enter new robot name", src.name, src.created_name), 1, MAX_NAME_LEN)
	if (!t)
		return
...
/obj/item/frame/toolbox_tiles_sensor/Initialize(mapload, ...)
	. = ..()
	ADD_TRAIT(src, TRAIT_WRITABLE, TRAIT_SOURCE_INHERENT)

/obj/item/frame/toolbox_tiles_sensor/write(obj/item/writer, mob/user, mods, color, crayon)
...
/obj/item/frame/firstaid_arm_assembly/Initialize(mapload, ...)
	. = ..()
	if(src.skin)
		src.overlays += image('icons/obj/structures/machinery/aibots.dmi', "kit_skin_[src.skin]")
	ADD_TRAIT(src, TRAIT_WRITABLE, TRAIT_SOURCE_INHERENT)

/obj/item/frame/firstaid_arm_assembly/write(obj/item/writer, mob/user, mods, color, crayon)
	var/t = copytext(stripped_input(user, "Enter new robot name", src.name, src.created_name), 1, MAX_NAME_LEN)
	if (!t)
		return
	if (!in_range(src, user) && src.loc != user)
