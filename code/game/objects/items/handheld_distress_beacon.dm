...
/obj/item/handheld_distress_beacon/attack_self(mob/user)
	...
	for(var/current_ert_num in 1 to length(ert_paths))
		beacon_call_buttons += "(<A HREF='?_src_=player_data.admin_holder;[HrefToken(forceGlobal = TRUE)];distress_handheld=\ref[user];ert_name=[ert_paths[current_ert_num]]'>[ert_short_names[current_ert_num]]</A>) "

	for(var/client/admin in GLOB.admins)
		if(check_client_rights(admin, R_ADMIN|R_MOD, FALSE))
			playsound_client(admin, 'sound/effects/sos-morse-code.ogg', 10)
	message_admins("[key_name(user)] has used a [beacon_type] for the reason '[SPAN_ORANGE(reason)]'! [CC_MARK(user)] [beacon_call_buttons](<A HREF='?_src_=player_data.admin_holder;[HrefToken(forceGlobal = TRUE)];deny_distress_handheld=\ref[user]'>DENY</A>) [ADMIN_JMP_USER(user)] [CC_REPLY(user)]")
