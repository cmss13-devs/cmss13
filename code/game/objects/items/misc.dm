...
/obj/item/bananapeel/Crossed(AM as mob|obj)
	if (iscarbon(AM))
		var/mob/living/carbon/current_mob = AM
		current_mob.slip(name, 4, 2)
...
/obj/item/evidencebag/MouseDrop(obj/item/stored_item as obj)
	if (!ishuman(usr))
		return
	...
	if (isturf(stored_item.loc))
		if (!user.Adjacent(stored_item))
			return
	else
		//If it isn't on the floor. Do some checks to see if it's in our hands or a box. Otherwise give up.
		if(istype(stored_item.loc,/obj/item/storage)) //in a container.
			var/depth = stored_item.get_storage_depth_to(user)
			if (!depth || depth > 2)
				return //too deeply nested to access or not being carried by the user.

			var/obj/item/storage/current_storage = stored_item.loc
			user.client.screen -= stored_item
			current_storage.contents.Remove(stored_item)
		else if(user.l_hand == stored_item) //in a hand
			user.drop_l_hand()
		else if(user.r_hand == stored_item) //in a hand
			user.drop_r_hand()
		else
			return

	if(!istype(stored_item) || stored_item.anchored)
		return

	if(istype(stored_item, /obj/item/evidencebag))
		to_chat(user, SPAN_NOTICE("You find putting an evidence bag in another evidence bag to be slightly absurd."))
		return

	if(stored_item.w_class > SIZE_MEDIUM)
		to_chat(user, SPAN_NOTICE("[stored_item] won't fit in [src]."))
		return
