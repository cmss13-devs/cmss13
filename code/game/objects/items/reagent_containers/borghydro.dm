...
/obj/item/reagent_container/borghypo/attack(mob/living/M as mob, mob/user as mob)
	...
	if(M.reagents)
		...
		to_chat(user, "<span class='notice'>[t] units of \red [reagent_ids[mode]] \blue injected for a total of \red [round(M.reagents.get_reagent_amount(reagent_ids[mode]))]\blue. [reagent_volumes[reagent_ids[mode]]] units remaining.</span>")
