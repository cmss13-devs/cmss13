...
/obj/item/reagent_container/food/drinks/cans/proc/crush_can(mob/user)
	...
	if(src == H.get_inactive_hand())
		message = "between [user.gender == MALE ? "his" : user.gender == FEMALE ? "her" : "their"] hands"
		...
		switch(user.zone_selected)
			if("head")
				if(!L)
					...
					return
				message = "against [user.gender == MALE ? "his" : user.gender == FEMALE ? "her" : "their"] head!"
			...
			if("l_foot" , "r_foot")
				if(!L)
					...
					return
				message = "under [user.gender == MALE ? "his" : user.gender == FEMALE ? "her" : "their"] foot!"
