...
/obj/item/reagent_container/food/condiment/attack(mob/M as mob, mob/user as mob, def_zone)
	...
	if(M == user)
		...
		msg_admin_attack("[user.name] ([user.ckey]) fed [M.name] ([M.ckey]) with [src.name] (REAGENTS: [rgt_list_text]) (INTENT: [uppertext(user.a_intent)]) in [get_area(src)] ([src.loc.x],[src.loc.y],[src.loc.z]).", src.loc.x, src.loc.y, src.loc.z)
