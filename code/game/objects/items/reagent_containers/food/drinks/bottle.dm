...
/obj/item/reagent_container/food/drinks/bottle/attack(mob/living/target as mob, mob/living/user as mob)
	...
	if(affecting == "head" && istype(target, /mob/living/carbon/) && !isXeno(target))
		...
		for(var/mob/O in viewers(user, null))
			if(target != user) O.show_message(text("<span class='danger'><B>[target] has been hit over the head with a bottle of [src.name], by [user]!</B></span>"), 1)
		...
		for(var/mob/O in viewers(user, null))
			if(target != user) O.show_message(text("<span class='danger'><B>[target] has been attacked with a bottle of [src.name], by [user]!</B></span>"), 1)
			else O.show_message(text("<span class='danger'><B>[target] has attacked \himself with a bottle of [src.name]!</B></span>"), 1)
