...
/obj/item/reagent_container/food/drinks/bottle/attack(mob/living/target as mob, mob/living/user as mob)
	...
	if(affecting == "head" && istype(target, /mob/living/carbon/) && !isXeno(target))
		...
		for(var/mob/O in viewers(user, null))
			if(target != user) O.show_message(text(SPAN_DANGER("<B>[target] has been hit over the head with a bottle of [src.name], by [user]!</B>")), 1)
			else O.show_message(text(SPAN_DANGER("<B>[target] hit \himself with a bottle of [src.name] on the head!</B>")), 1)
		...
		for(var/mob/O in viewers(user, null))
			if(target != user) O.show_message(text(SPAN_DANGER("<B>[target] has been attacked with a bottle of [src.name], by [user]!</B>")), 1)
			else O.show_message(text(SPAN_DANGER("<B>[target] has attacked \himself with a bottle of [src.name]!</B>")), 1)
	...
	msg_admin_attack("[user.name] ([user.ckey]) attacked [target.name] ([target.ckey]) with a bottle. (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
