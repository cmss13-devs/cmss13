...
/obj/item/reagent_container/syringe/afterattack(obj/target, mob/user, proximity)
	...
	if (user.a_intent == INTENT_HARM && ismob(target))
		var/mob/M = target
		if(M != user && M.stat != DEAD && M.a_intent != INTENT_HELP && !M.is_mob_incapacitated() && (skillcheck(M, SKILL_CQC, SKILL_CQC_SKILLED) || isyautja(M)) && !HAS_TRAIT(M, TRAIT_XENO_BRAINDEAD)) // preds have null skills
