...
/obj/item/weapon/melee/maintenance_jack //put here for unique action
...
/obj/item/weapon/melee/maintenance_jack/verb/use_unique_action()
	set category = "Weapons"
	set name = "Flip Maintenance Jack"
	set desc = "Toggle the mode of the maintenance jack."
	set src in usr
	unique_action(usr)

/obj/item/weapon/melee/maintenance_jack/get_examine_text(mob/user)
	. = ..()
	. += SPAN_NOTICE("Use the Flip action to switch modes.")
	if(mode == "crowbar")
...
/obj/item/weapon/melee/maintenance_jack/unique_action(mob/living/user)
	if(user.get_active_hand() != src)
		return

	playsound(src, 'sound/weapons/punchmiss.ogg', 15, TRUE, 3)
	if(mode == "crowbar") //Wrench mode | Remove bolts if engi 3
	...
	else //Crowbar mode | Pry open doors if super strong trait
...
/obj/item/weapon/melee/maintenance_jack/afterattack(obj, mob/living/user, proximity)
	if(!(mode == "crowbar")) //Otherwise it lets you pry open right after unbolting
		return
	if(!(HAS_TRAIT(user, TRAIT_SUPER_STRONG))) //Only synths can force open doors, otherwise its a regular crowbar
		to_chat(user, SPAN_DANGER("The airlock's motors resist your efforts to force it."))
		return
	...
	if(istype(obj, /obj/structure/machinery/door/airlock))
		...
		if(!door.density)
			return
		if(door.heavy)
			to_chat(usr, SPAN_DANGER("You cannot force [door] open."))
			return FALSE
		if(user.action_busy || user.a_intent == INTENT_HARM)
			return

		user.visible_message(SPAN_DANGER("[user] jams [src] into [door] and starts to pry it open."),
		SPAN_DANGER("You jam [src] into [door] and start to pry it open."))
		playsound(src, "pry", 15, TRUE)
		if(do_after(user, 3 SECONDS, INTERRUPT_ALL, BUSY_ICON_HOSTILE))
			if(!door.density)
				return

			user.visible_message(SPAN_DANGER("[user] forces [door] open with [src]."),
		...
		if(do_after(user, 5 SECONDS, INTERRUPT_ALL, BUSY_ICON_HOSTILE) && door.density)
			user.visible_message(SPAN_DANGER("[user] forces [door] open with [src]."),
			SPAN_DANGER("You force [door] open with [src]."))
			door.Open()
