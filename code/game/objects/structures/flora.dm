...
/obj/structure/flora/jungle/thickbush/Crossed(atom/movable/AM)
	...
	if(!stump)
		if(isliving(AM))
			...
			if(ishuman(living))
				var/mob/living/carbon/human/person = living
				var/stuck = rand(0,10)
				if(HAS_TRAIT(living, TRAIT_HAULED))
					return
				switch(stuck)
					if(0 to 4)
						var/new_slowdown = person.next_move_slowdown + rand(2,3)
						person.next_move_slowdown = new_slowdown
						if(prob(2))
							to_chat(person, SPAN_WARNING("Moving through [src] slows you down."))
					if(5 to 7)
						var/new_slowdown = person.next_move_slowdown + rand(4,7)
						person.next_move_slowdown = new_slowdown
						if(prob(10))
							to_chat(person, SPAN_WARNING("It is very hard to move through [src]..."))
					if(8 to 9)
						var/new_slowdown = person.next_move_slowdown + rand(8,11)
						person.next_move_slowdown = new_slowdown
						to_chat(person, SPAN_WARNING("You got tangled in [src]!"))
					if(10)
						var/new_slowdown = person.next_move_slowdown + rand(12,20)
						person.next_move_slowdown = new_slowdown
						to_chat(person, SPAN_WARNING("You got completely tangled in [src]! Oh boy..."))
