...
/obj/structure/bed/buckle_mob(mob/living/carbon/human/mob, mob/user)
	if(buckled_bodybag)
		return
	if(ishuman(mob))
		if(MODE_HAS_MODIFIER(/datum/gamemode_modifier/disable_stripdrag_enemy) && (mob.stat == DEAD || mob.health < mob.human_health_threshold_crit) && !mob.get_target_lock(user.faction_group) && !(mob.status_flags & PERMANENTLY_DEAD))
