...
/obj/structure/bed/chair/wheelchair/relaymove(mob/user, direction)
	...
	if(ishuman(user))
		...
		if(!left_hand || (left_hand.status & LIMB_DESTROYED))
			...
			working_hands--
		else if((left_hand.status & LIMB_BROKEN) && !(left_hand.status & LIMB_SPLINTED) || (left_hand.status & LIMB_ESCHAR))
			move_delay ++
		if(!right_hand || (right_hand.status & LIMB_DESTROYED))
			move_delay += 4
			working_hands--
		else if((right_hand.status & LIMB_BROKEN) && !(right_hand.status & LIMB_SPLINTED) || (right_hand.status & LIMB_ESCHAR))
			move_delay++
		if(!working_hands)
