...
client/verb/JoinResponseTeam()
	...
	/*	if(admin_emergency_team)
		...
		for (var/obj/effect/landmark/L in landmarks_list) if (L.name == "Commando")
			...
			to_chat(new_commando, SPAN_NOTICE(" You are [!leader_selected?"))a member":"the <B>LEADER</B>"] of an Emergency Response Team, a type of military division, under CentComm's service. There is a code red alert on [station_name], you are tasked to go and fix the problem."
...
proc/trigger_armed_response_team(var/force = 0)
	...
	if(!prob(send_team_chance))
		marine_announcement("It would appear that an emergency response team was requested for [station_name]. Unfortunately, we were unable to send one at this time.", "Central Command")
		can_call_ert = 0 // Only one call per round, ladies.
		return

	marine_announcement("It would appear that an emergency response team was requested for [station_name]. We will prepare and send one as soon as possible.", "Central Command")
