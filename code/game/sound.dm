...
/proc/playsound(atom/source, soundin, vol = 100, vary = FALSE, sound_range, vol_cat = VOLUME_SFX, channel = 0, status , falloff = 1, echo, y_s_offset,x_s_offset)
	if(isarea(source))
		...
		return FALSE
	var/datum/sound_template/sound = new()

	var/sound/SD = soundin
	if(istype(SD))
		sound.file = SD.file
		sound.wait = SD.wait
		sound.repeat = SD.repeat
	else
		sound.file = get_sfx(soundin)
	sound.channel = channel ? channel : get_free_channel()
	sound.status = status
	sound.falloff = falloff
	sound.volume = vol
	sound.volume_cat = vol_cat
	sound.echo = echo
	sound.y_s_offset = y_s_offset
	sound.x_s_offset = x_s_offset
	if(vary != FALSE)
		if(vary > 1)
			sound.frequency = vary
		else
			sound.frequency = GET_RANDOM_FREQ // Same frequency for everybody

	if(!sound_range)
		sound_range = round(0.25*vol) //if no specific range, the max range is equal to a quarter of the volume.
	sound.range = sound_range

	var/turf/turf_source = get_turf(source)
	if(!turf_source || !turf_source.z)
		return FALSE
	sound.x = turf_source.x
	sound.y = turf_source.y
	sound.z = turf_source.z
	...
	if(SSinterior.in_interior(turf_source))
		...
		if(VI?.ready)
			...
			if(VI.exterior)
				...
				sound.x = new_turf_source.x
				sound.y = new_turf_source.y
				sound.z = new_turf_source.z
			else sound_range = 0
	...
	SSsound.queue(sound, null, extra_interiors)
	return sound.channel
...
/proc/playsound_client(client/C, soundin, atom/origin, vol = 100, random_freq, vol_cat = VOLUME_SFX, channel = 0, status, list/echo, y_s_offset, x_s_offset)
	...
	if(origin)
		...
		if(current_turf)
			sound.x = current_turf.x
			sound.y = current_turf.y
			sound.z = current_turf.z
	var/sound/SD = soundin
	if(istype(SD))
		sound.file = SD.file
		sound.wait = SD.wait
	...
	if(random_freq)
		sound.frequency = GET_RANDOM_FREQ
	sound.volume = vol
	sound.volume_cat = vol_cat
...
/proc/playsound_area(area/A, soundin, vol = 100, channel = 0, status, vol_cat = VOLUME_SFX, list/echo, y_s_offset, x_s_offset)
	...
	for(var/mob/living/mob in A.contents)
		if(!mob || !mob.client || !mob.client.soundOutput)
			continue
		hearers += mob.client
	SSsound.queue(sound, hearers)
...
/client/proc/generate_sound_queues()
	...
	var/datum/sound_template/sound
	for(var/i = 1, i <= ammount, i++)
		sound = new
		sound.file = get_sfx("male_warcry") // warcry has variable length, lots of variations
		sound.channel = get_free_channel() // i'm convinced this is bad, but it's here to mirror playsound() behaviour
		sound.range = range
		sound.x = x
		sound.y = y
		sound.z = z
