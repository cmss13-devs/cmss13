			...
			if(SO.ordernum == ordernum)
				...
				if(supply_controller.points >= round(supply_pack.cost) && supply_controller.black_market_points >= supply_pack.dollar_cost)
					...
					if(supply_controller.black_market_heat != -1) //-1 Heat means heat is disabled
						supply_controller.black_market_heat = clamp(supply_controller.black_market_heat + supply_pack.crate_heat + (supply_pack.crate_heat * rand(-0.25,0.25)), 0, 100) // black market heat added is crate heat +- up to 25% of crate heat
					supply_controller.shoppinglist += supply_order
...
/datum/controller/supply/proc/black_market_CMB_investigation()
	black_market_heat = -1
	SSticker.mode.get_specific_call("Inspection - Colonial Marshal Ledger Investigation Team", FALSE, FALSE)
