...
/datum/STUI/ui_static_data(mob/user)
	. = list()
	.["tabs"] = list()
	if(user.client.player_data.admin_holder.admin_rank.rights & R_MOD)
		.["tabs"] += STUI_TEXT_ATTACK
		.["tabs"] += STUI_TEXT_STAFF
		.["tabs"] += STUI_TEXT_STAFF_CHAT
		.["tabs"] += STUI_TEXT_OOC
	if((user.client.player_data.admin_holder.admin_rank.rights & R_MOD) || (user.client.player_data.admin_holder.admin_rank.rights & R_DEBUG))
		.["tabs"] += STUI_TEXT_GAME
	if(user.client.player_data.admin_holder.admin_rank.rights & R_DEBUG)
