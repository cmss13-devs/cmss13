	...
	for(var/line in Lines)
		...
		for(var/i=2, i<=List.len, i++)
			switch(ckey(List[i]))
				...
				if("everything","host","all")	rights |= (R_HOST|R_BUILDMODE|R_ADMIN|R_BAN|R_SERVER|R_DEBUG|R_PERMISSIONS|R_POSSESS|R_STEALTH|R_REJUVINATE|R_COLOR|R_VAREDIT|R_SOUNDS|R_SPAWN|R_MOD|R_MENTOR|R_EVENT)
				...
				if("devleoper")					rights |= G_DEVELOPER
