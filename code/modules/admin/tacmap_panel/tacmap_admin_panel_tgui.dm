	...
	switch(action)
		...
		if("delete")
			...
			if(is_uscm)
				...
				selected_minimap = SSminimaps.fetch_minimap_object(2, MINIMAP_FLAG_USCM, live=FALSE, popup=FALSE, drawing=FALSE)
				selected_minimap.update()
				selected_minimap = SSminimaps.fetch_minimap_object(2, MINIMAP_FLAG_USCM, live=TRUE, popup=FALSE, drawing=FALSE)
				...
				if(xeno_selection == LATEST_SELECTION)
					return TRUE
				selected_minimap = SSminimaps.fetch_minimap_object(2, MINIMAP_FLAG_XENO, live=TRUE, popup=FALSE, drawing=FALSE)
				for(var/turf/label as anything in selected_minimap.labelled_turfs)
					SSminimaps.remove_marker(label)
				selected_drawing_image = SSminimaps.get_drawing_image(2, MINIMAP_FLAG_XENO, drawing=FALSE)
