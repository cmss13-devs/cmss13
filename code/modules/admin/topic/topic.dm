...
//======================================================
	if(href_list["evac_authority"])
		switch(href_list["evac_authority"])
			...
			if("toggle_evac")
				GLOB.EvacuationAuthority.flags_scuttle ^= FLAGS_EVACUATION_DENY
				message_staff("[key_name_admin(usr)] has [GLOB.EvacuationAuthority.flags_scuttle & FLAGS_EVACUATION_DENY ? "forbidden" : "allowed"] ship-wide evacuation.")

			if("force_evac")
				if(!GLOB.EvacuationAuthority.begin_launch())
			...
			if("init_dest")
				if(!GLOB.EvacuationAuthority.enable_self_destruct())
			...
			if("use_dest")
				...
				GLOB.EvacuationAuthority.trigger_self_destruct()

			if("toggle_dest")
				GLOB.EvacuationAuthority.flags_scuttle ^= FLAGS_SELF_DESTRUCT_DENY
				message_staff("[key_name_admin(usr)] has [GLOB.EvacuationAuthority.flags_scuttle & FLAGS_SELF_DESTRUCT_DENY ? "forbidden" : "allowed"] the self-destruct system.")
...
//Regular jobs
	...
	//JOBBAN'S INNARDS
		...
		message_staff("[key_name_admin(usr)] set the forced secret mode as [GLOB.secret_force_mode].")
		Game() // updates the main game menu
