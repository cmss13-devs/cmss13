...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if(href_list["nukedeny"])
		...
		if(!istype(ref_person))
			return FALSE
		marine_announcement("USCM High Command has denied the request for Nuclear Ordnance.", "NUCLEAR ORDNANCE DENIED", 'sound/misc/notice2.ogg', logging = ARES_LOG_MAIN)
