	...
	switch(href_list["ahelp"])
		...
		if("autoresponse") // new verb on the Ahelp.  Will tell the person their message was received, and they probably won't get a response
			...
			switch(choice)
				...
				if("L: Current Map")
					msgplayer = SPAN_NOTICE("<b>NOTICE: <font color=red>[key_name_admin(usr, 0)]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. If you need a map to the current game, you can (usually) find them on the front page of our wiki in the 'Maps' section. <a href='[URL_WIKI_LANDING]'>Check it out here.</a> If the map is not listed, it's a new or rare map and the overview hasn't been finished yet.</b>")
				...
				if("J: Job bans")
					msgplayer = SPAN_NOTICE("<b>NOTICE: <font color=red>[key_name_admin(usr, 0)]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. All job bans, including xeno bans, are permanent until appealed. You can appeal your ban at the <a href='[URL_FORUM_APPEALS]'>Appeals forum</a>.</b>")
