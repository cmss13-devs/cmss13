...
/client/proc/SDQL_query(query_text as message)
	...
	if(query_list.len < 2)
		if(query_list.len > 0)
			to_chat(usr, SPAN_DANGER("SDQL: Too few discrete tokens in query \"[query_text]\". Please check your syntax and try again."))
		return

	if(!(lowertext(query_list[1]) in list("select", "delete", "update")))
		to_chat(usr, SPAN_DANGER("SDQL: Unknown query type: \"[query_list[1]]\" in query \"[query_text]\". Please check your syntax and try again."))
		return
	...
	if(lowertext(query_list[1]) == "update")
		if(i <= query_list.len && lowertext(query_list[i]) == "set")
			for(i++; i <= query_list.len; i++)
				if(i + 2 <= query_list.len && query_list[i + 1] == "=")
					...
					to_chat(usr, SPAN_DANGER("SDQL: Invalid set parameter in query \"[query_text]\". Please check your syntax and try again."))
					return
		...
		if(set_vars.len < 1)
			to_chat(usr, SPAN_DANGER("SDQL: Invalid or missing set in query \"[query_text]\". Please check your syntax and try again."))
			return
	...
	for(var/datum/t in objs)
		...
		for(i = 1, i - 1 < where.len, i++)
			...
			if(!(compare_op in list("==", "=", "<>", "<", ">", "<=", ">=", "!=")))
				to_chat(usr, SPAN_DANGER("SDQL: Unknown comparison operator [compare_op] in where clause following [v] in query \"[query_text]\". Please check your syntax and try again."))
				return
...
/proc/SDQL_evaluate(datum/object, list/equation)
	if(equation.len == 0)
		...
		to_chat(usr, SPAN_DANGER("SDQL: Sorry, equations not yet supported :("))
		return null
...
/proc/SDQL_tokenize(query_text)
	...
	for(var/i = 1, i <= len, i++)
		...
		if(char in whitespace)
			...
			if(word != "")
				to_chat(usr, SPAN_DANGER("SDQL: You have an error in your SDQL syntax, unexpected ' in query: \<font color=gray>[query_text]</font>\" following \"<font color=gray>[word]</font>\". Please check your syntax, and try again."))
				return null
