...
/client/proc/cmd_admin_object_narrate(obj/selected in view(src))
	...
	switch(type)
		if(NARRATION_METHOD_SAY)
			...
			selected.langchat_send_message(message, LANGCHAT_IMAGE_IGNORE_LANG, heard)
			selected.visible_message("<b>[selected]</b> says, \"[message]\"")
		if(NARRATION_METHOD_ME)
			selected.AddComponent(/datum/component/langchat_image)
			selected.langchat_send_message(message, LANGCHAT_IMAGE_IGNORE_LANG | LANGCHAT_IMAGE_IS_EMOTE, heard, LANGCHAT_FAST_POP, additional_styles = list("langchat_small"))
