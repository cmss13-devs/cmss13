...
/proc/high_command_announce(text , mob/Sender , iamessage)
	...
	msg = "<b>[SPAN_NOTICE("<font color=orange>USCM[iamessage ?  "IA" : ""]:</font>")][key_name(Sender, 1)] [CC_MARK(Sender)] [ADMIN_PP(Sender)] [ADMIN_VV(Sender)] [ADMIN_SM(Sender)] [ADMIN_JMP_USER(Sender)] (<A HREF='?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];CentcommReply=\ref[Sender]'>RPLY</A>):</b> [msg]"
	log_admin(msg)
	for(var/client/C in GLOB.admins)
