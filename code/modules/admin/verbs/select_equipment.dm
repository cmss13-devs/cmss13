...
/client/proc/cmd_admin_dress_human(mob/living/carbon/human/M in GLOB.human_mob_list, var/datum/equipment_preset/dresscode, var/no_logs = 0, var/count_participant = FALSE)
	if (!no_logs)
...
/proc/arm_equipment(mob/living/carbon/human/M, dresscode, randomise = FALSE, var/count_participant = FALSE, var/client/mob_client)
	if(ispath(dresscode))
