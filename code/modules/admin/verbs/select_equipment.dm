...
/proc/arm_equipment(var/mob/living/carbon/human/M, var/dresscode, var/randomise = FALSE, var/count_participant = FALSE, var/client/mob_client, var/show_job_gear = TRUE, var/show_vanity_items = TRUE)
	if(ispath(dresscode))
		...
		GLOB.gear_path_presets_list[dresscode].load_preset(M, randomise, count_participant, mob_client, show_job_gear, show_vanity_items)
	else
		if(!GLOB.gear_name_presets_list)
			CRASH("arm_equipment !gear_path_presets_list")
		if(!GLOB.gear_name_presets_list[dresscode])
			CRASH("arm_equipment !gear_path_presets_list[dresscode]")
		GLOB.gear_name_presets_list[dresscode].load_preset(M, randomise, count_participant, mob_client, show_job_gear, show_vanity_items)
