/client/proc/xooc(msg as text)
	...
	for(var/mob/living/carbon/M in GLOB.alive_mob_list)
		if(M.client && M.hivenumber && !check_client_rights(M.client, R_MOD, FALSE)) // Send to xenos who are non-staff
			to_chat(M, SPAN_XOOC("XOOC: [key]([player_data.admin_holder.admin_rank.rank]): [msg]"))

	for(var/mob/dead/observer/M in GLOB.observer_list)
		if(M.client && !M.client.player_data.admin_holder) // Send to observers who are non-staff
			to_chat(M, SPAN_XOOC("XOOC: [key]([player_data.admin_holder.admin_rank.rank]): [msg]"))
	...
	for(var/client/C in GLOB.admins) // Send to staff
		...
		to_chat_spaced(C, margin_top = 0.5, margin_bottom = 0.5, html = SPAN_XOOC("XOOC: [key]([player_data.admin_holder.admin_rank.rank]): [msg]"))
