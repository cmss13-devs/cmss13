	...
	if(check_rights(R_VAREDIT))
		if(target_index)
			...
			if(href_list[VV_HK_LIST_REMOVE])
				...
				message_staff("[key_name_admin(src)] modified list's contents: REMOVED=[variable]")
		...
		if(href_list[VV_HK_LIST_ERASE_DUPES])
			...
			message_staff("[key_name_admin(src)] modified list's contents: CLEAR DUPES")
		if(href_list[VV_HK_LIST_ERASE_NULLS])
			...
			message_staff("[key_name_admin(src)] modified list's contents: CLEAR NULLS")
		if(href_list[VV_HK_LIST_SET_LENGTH])
			...
			message_staff("[key_name_admin(src)] modified list's len: [target.len]")
		if(href_list[VV_HK_LIST_SHUFFLE])
			...
			message_staff("[key_name_admin(src)] modified list's contents: SHUFFLE")
