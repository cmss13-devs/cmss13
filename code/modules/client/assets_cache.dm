/*
CREDITS AND LICENSE WHATEVER TO STONEDMC
...
//When sending mutiple assets, how many before we give the client a quaint little sending assets message
...
/proc/send_asset_list(var/client/client, var/list/asset_list, var/verify = TRUE)
	...
	if(length(unreceived) >= ASSET_CACHE_TELL_CLIENT_AMOUNT)
		to_chat(client, "Sending you the required assets...")
...
/proc/get_files_slot(var/client/client, var/list/files, var/register_asset = TRUE)
...
#define SPRITE_INDEX 2
#define SPRITESHEET_COUNT 1
#define SPRITESHEET_ICON 2
#define SPRITESHEET_STRIPPED 3
...
/datum/asset/spritesheet/register()
	...
	for(var/size_id in sizes)
		...
		register_asset("[name]_[size_id].png", size[SPRITESHEET_STRIPPED])
...
/datum/asset/spritesheet/proc/ensure_stripped(sizes_to_strip = sizes)
	for(var/size_id in sizes_to_strip)
		var/size = sizes[size_id]
		if(size[SPRITESHEET_STRIPPED])
			continue

		// save flattened version
		var/fname = "data/spritesheets/[name]_[size_id].png"
		fcopy(size[SPRITESHEET_ICON], fname)
		size[SPRITESHEET_STRIPPED] = icon(fname)
		fdel(fname)
...
/datum/asset/spritesheet/proc/generate_css()
	...
	for(var/size_id in sizes)
		var/size = sizes[size_id]
		var/icon/tiny = size[SPRITESHEET_ICON]
	...
	for(var/sprite_id in sprites)
		...
		var/size_id = sprite[SPRITE_SIZE]
		var/idx = sprite[SPRITE_INDEX]
		var/size = sizes[size_id]

		var/icon/tiny = size[SPRITESHEET_ICON]
		var/icon/big = size[SPRITESHEET_STRIPPED]
...
/datum/asset/spritesheet/proc/Insert(sprite_name, icon/I, icon_state="", dir=SOUTH, frame=1, moving=FALSE)
	...
	if(size)
		var/position = size[SPRITESHEET_COUNT]++
		var/icon/sheet = size[SPRITESHEET_ICON]
		size[SPRITESHEET_STRIPPED] = null
...
/datum/asset/spritesheet/proc/insert_all(prefix, icon/I, list/directions)
	if(length(prefix))
		prefix = "[prefix]-"
...
#undef SPRITE_SIZE
#undef SPRITE_INDEX
#undef SPRITESHEET_COUNT
#undef SPRITESHEET_ICON
#undef SPRITESHEET_STRIPPED
