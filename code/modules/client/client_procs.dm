...
/client/proc/add_to_access_log(client/user, address, cid, byond_version, byond_build)
	var/datum/entity/connection/connect = DB_ENTITY(/datum/entity/connection)
	connect.ckey = user.key
	connect.ip = address
	connect.cid = cid
	connect.byond_version = "[byond_version].[byond_build]"
	connect.time = time2text(world.timeofday, "YYYY-MM-DD hh:mm:ss")
	connect.save()
	connect.detach()
