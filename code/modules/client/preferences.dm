...
/datum/preferences
	...
	var/list/volume_preferences = list(1, 0.5, 1, 0.6, 0.7, 0.25) // Game, music, admin midis, lobby music, human voices, xeno voices (this is also set in sanitize_volume_preferences() call)
...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_menu)
		...
		if(MENU_SETTINGS)
			...
			dat += "<b>Hear Human Speech Sounds:</b> <a href='byond://?_src_=prefs;preference=hear_humans'><b>[(toggles_sound & SOUND_HUMAN_SPEAKING) ? "No" : "Yes"]</b></a><br>"
	...
	switch (href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("hear_humans")
					toggles_sound ^= SOUND_HUMAN_SPEAKING
