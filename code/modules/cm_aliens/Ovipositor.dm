...
/obj/ovipositor/proc/do_decay()
	...
	if (T)
		T.overlays += image('icons/mob/hostiles/Ovipositor.dmi', "ovipositor_molted", ATMOS_DEVICE_LAYER) //ATMOS_DEVICE_LAYER so that the ovi is above weeds, blood, and resin weed nodes.
...
/obj/ovipositor/proc/explode()
	...
	var/turf/T = get_turf(src)
	if (T)
		T.overlays += image('icons/mob/hostiles/Ovipositor.dmi', "ovipositor_gibbed", ATMOS_DEVICE_LAYER)
