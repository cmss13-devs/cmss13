...
/obj/effect/alien/resin/special/pylon/endgame/proc/comms_relay_connection()
	marine_announcement("ALERT.\n\nIrregular build up of energy around communication relays at [get_area(src)].", "[MAIN_AI_SYSTEM] Biological Scanner.<br><br>DANGER: Relay growth is generating new xenomorph entities, advise urgent termination of pylon by ground forces.")
	...
	for(var/hivenumber in GLOB.hive_datum)
		...
		if(checked_hive == linked_hive)
			xeno_announcement(SPAN_XENOANNOUNCE("We have harnessed the tall's communication relay at [get_area(src)]. We will now grow more of our number from this pylon. Hold it!"), hivenumber, XENO_GENERAL_ANNOUNCE)
		else
