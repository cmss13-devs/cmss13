...
/obj/effect/alien/resin/special/pylon/core/attack_alien(mob/living/carbon/xenomorph/M)
	if(M.a_intent != INTENT_HELP && M.can_destroy_special() && M.hivenumber == linked_hive.hivenumber)
		if(!hardcore && last_attempt + 6 SECONDS > world.time)
			...
			if((alert(M, "Are we sure that you want to remove the hive core? No cooldown will be applied.", , "Yes", "No") != "Yes"))
				return XENO_NO_DELAY_ACTION
...
/obj/effect/alien/resin/special/pylon/core/Destroy()
	if(linked_hive)
		...
		if(hardcore)
			xeno_message(SPAN_XENOANNOUNCE("We can no longer gain new sisters or another Queen. Additionally, we are unable to heal if your Queen is dead"), 2, linked_hive.hivenumber)
