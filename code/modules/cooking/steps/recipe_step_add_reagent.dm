...
/datum/cooking/recipe_step/add_reagent/check_conditions_met(obj/used_item, datum/cooking/recipe_tracker/tracker)
	...
	if(!tracker)
		if(!(used_item.flags_atom & OPENCONTAINER))
			return PCWJ_CHECK_INVALID

		var/obj/item/reagent_container/reagent_container = used_item
	...
	// shouldn't matter.)
	if(istype(used_item, /obj/structure/machinery/cooking))
	...
	if(!istype(used_item, /obj/item/reagent_container))
		return PCWJ_CHECK_INVALID
	if(!(used_item.flags_atom & OPENCONTAINER))
		return PCWJ_CHECK_INVALID
	...
	if(reagent_container.reagents.total_volume)
		...
		var/incoming_amount = max(0, min(reagent_container.amount_per_transfer_from_this, reagent_container.reagents.total_volume, (container.reagents.maximum_volume - container.reagents.total_volume)))
...
/datum/cooking/recipe_step/add_reagent/follow_step(obj/used_item, datum/cooking/recipe_tracker/tracker)
	var/obj/item/reagent_container/our_item = used_item
	if(!istype(our_item))
		return list()
