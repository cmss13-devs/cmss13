...
/datum/admin_podlauncher/proc/pre_launch() //Creates a list of acceptable items,
	...
	if (length(acceptableTurfs) && !custom_dropoff) //We don't fill the supplypod if acceptableTurfs is empty, if the pod is going in reverse (effectReverse=true), or if the pod is acting like a missile (effectMissile=true)
