...
/obj/item/device/eftpos/attackby(O as obj, user as mob)
	if(istype(O, /obj/item/card))
		if(linked_account)
			...
			to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Unable to connect to linked account.</span>")
			...
			if(!linked_account.suspended)
				if(transaction_locked && !transaction_paid)
					if(transaction_amount <= E.worth)
						...
						src.visible_message("[htmlicon(src, viewers(src))] The [src] chimes.")
						transaction_paid = 1
						...
						to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>The charge card doesn't have that much money!</span>")
			else
				to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Connected account has been suspended.</span>")
		else
			to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>EFTPOS is not connected to an account.</span>")
		...
		switch(href_list["choice"])
			if("change_code")
				...
				if(attempt_code == access_code)
					...
					to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Incorrect code entered.</span>")
			if("change_id")
			...
			if("link_account")
				...
				if(linked_account)
					if(linked_account.suspended)
						linked_account = null
						to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Account has been suspended.</span>")
				else
					to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Account not found.</span>")
			if("trans_purpose")
			...
			if("toggle_lock")
				if(transaction_locked)
					...
					to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>No account connected to send transactions to.</span>")
			if("scan_card")
				if(linked_account)
					...
					to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Unable to link accounts.</span>")
			if("reset")
				...
				if (istype(I, /obj/item/card))
					...
					if(ACCESS_MARINE_LOGISTICS in C.access)
						access_code = 0
						to_chat(usr, "[htmlicon(src, usr)]<span class='info'>Access code reset to 0.</span>")
				...
				if(!linked_account.suspended)
					...
					if(D)
						if(!D.suspended)
							...
							to_chat(usr, "[htmlicon(src, usr)]<span class='warning'>Your account has been suspended.</span>")
					else
