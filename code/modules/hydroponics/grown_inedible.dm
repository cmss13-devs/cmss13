...
/obj/item/grown/nettle/pickup(mob/living/carbon/human/user as mob)
	if(istype(user) && !user.gloves)
		to_chat(user, "<span class='danger'>The nettle burns your bare hand!</span>")
...
/obj/item/grown/nettle/death/pickup(mob/living/carbon/human/user as mob)
	...
	if(..() && prob(50))
		user.KnockOut(5)
		to_chat(user, "<span class='danger'>You are stunned by the deathnettle when you try picking it up!</span>")
...
/obj/item/grown/nettle/death/attack(mob/living/carbon/M as mob, mob/user as mob)
	...
	if(istype(M, /mob/living))
		...
		msg_admin_attack("[user.name] ([user.ckey]) used the [src.name] on [M.name] ([M.ckey]) (<A HREF='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
