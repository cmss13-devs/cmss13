...
/obj/structure/machinery/portable_atmospherics/hydroponics/get_examine_text(mob/user)
	. = ..()
	var/hyrdro_info = show_hydro_info(user)
	if(hyrdro_info)
		. += hyrdro_info
...
/obj/structure/machinery/portable_atmospherics/hydroponics/proc/show_hydro_info(mob/user as mob)
	var/info = ""
	if(seed && !dead)
		info += "[src] has \blue [seed.display_name] \black planted.\n"
	...
	if(weedlevel >= 5)
		info += "[src] is \red filled with weeds!\n"
	if(pestlevel >= 5)
		info += "[src] is \red filled with tiny worms!\n"
