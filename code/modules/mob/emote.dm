...
/mob/proc/custom_emote(var/m_type=1,var/message = null, player_caused, var/nolog = 0)
 ...
 // Maybe some people are okay with that.
		...
		if(m_type & 1)
			...
			langchat_speech(input, viewers, GLOB.all_languages, skip_language_check = TRUE, additional_styles = list("emote"))
			...
			for (var/mob/O in hearers(get_turf(src), null))
				...
				if(O.status_flags & PASSEMOTES)
					...
					for(var/mob/living/M in O.contents)
						...
						hearers.Add(M)
				O.show_message(message, m_type)
			langchat_speech(input, hearers, GLOB.all_languages, skip_language_check = TRUE, additional_styles = list("emote"))
