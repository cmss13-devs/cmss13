		...
		if(INTENT_DISARM)
			...
			if(!skillcheck(attacking_mob, SKILL_CQC, SKILL_CQC_SKILLED))
				if (isgun(r_hand) || isgun(l_hand))
					...
					if (prob(chance))
						visible_message(SPAN_DANGER("[attacking_mob] accidentally discharges [src]'s [held_weapon.name] during the struggle!"), SPAN_DANGER("You accidentally discharge your [held_weapon.name] during the struggle!"), null, 5)
						var/list/turfs = list()
