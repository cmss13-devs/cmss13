...
/mob/living/carbon/human/attack_hand(mob/living/carbon/human/attacking_mob)
	...
	switch(attacking_mob.a_intent)
		...
		if(INTENT_DISARM)
			...
			var/attacker_skill_level = attacking_mob.skills ? attacking_mob.skills.get_skill_level(SKILL_CQC) : SKILL_CQC_DEFAULT // No skills, so assume default
			var/defender_skill_level = skills ? skills.get_skill_level(SKILL_CQC) : SKILL_CQC_DEFAULT // No skills, so assume default
