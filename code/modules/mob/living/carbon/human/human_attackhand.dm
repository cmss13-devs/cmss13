...
/mob/living/carbon/human/attack_hand(mob/living/carbon/human/attacking_mob)
	...
	switch(attacking_mob.a_intent)
		...
		if(INTENT_DISARM)
			...
			var/attacker_skill_level = attacking_mob.skills && skills ? attacking_mob.skills.get_skill_level(SKILL_CQC) : SKILL_CQC_MAX // No skills, so assume max
			var/defender_skill_level = skills && attacking_mob.skills ? skills.get_skill_level(SKILL_CQC) : SKILL_CQC_MAX // No skills, so assume max
