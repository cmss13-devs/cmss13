...
/datum/strippable_item/mob_item_slot/mask/alternate_action(atom/source, mob/user)
	...
	if(user.action_busy || user.is_mob_incapacitated() || !source.Adjacent(user))
		return
	if(MODE_HAS_MODIFIER(/datum/gamemode_modifier/disable_stripdrag_enemy) && (sourcehuman.stat == DEAD || sourcehuman.health < sourcehuman.human_health_threshold_crit) && !sourcehuman.get_target_lock(user.faction_group))
...
/datum/strippable_item/mob_item_slot/jumpsuit/alternate_action(atom/source, mob/user)
	...
	if(user.action_busy || user.is_mob_incapacitated() || !source.Adjacent(user))
		return
	if(MODE_HAS_MODIFIER(/datum/gamemode_modifier/disable_stripdrag_enemy) && (sourcemob.stat == DEAD || sourcemob.health < sourcemob.human_health_threshold_crit) && !sourcemob.get_target_lock(user.faction_group))
