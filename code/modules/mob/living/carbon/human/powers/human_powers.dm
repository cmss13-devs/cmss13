...
/mob/living/proc/set_resting(new_resting, silent = TRUE, instant = FALSE)
	...
	if(!COOLDOWN_FINISHED(src, rest_cooldown))
		to_chat(src, SPAN_WARNING("[ishuman(src) ? "You" : "We"] can't 'rest' that fast. Take a breather!"))
		return
	...
	if(new_resting)
		if(body_position == LYING_DOWN)
			if(!silent)
				to_chat(src, SPAN_NOTICE("[ishuman(src) ? "You" : "We"] will now try to stay lying down on the floor."))
		else if(HAS_TRAIT(src, TRAIT_FORCED_STANDING) || (buckled && buckled.buckle_lying != NO_BUCKLE_LYING))
			if(!silent)
				to_chat(src, SPAN_NOTICE("[ishuman(src) ? "You" : "We"] will now lay down as soon as [ishuman(src) ? "You" : "We"] are able to."))
		else
			if(!silent)
				to_chat(src, SPAN_NOTICE("[ishuman(src) ? "You" : "We"] lay down."))
			set_lying_down()
	else
		if(body_position == STANDING_UP)
			if(!silent)
				to_chat(src, SPAN_NOTICE("[ishuman(src) ? "You" : "We"] will now try to remain standing up."))
		else if(HAS_TRAIT(src, TRAIT_FLOORED) || (buckled && buckled.buckle_lying != NO_BUCKLE_LYING))
			if(!silent)
				to_chat(src, SPAN_NOTICE("[ishuman(src) ? "You" : "We"] will now stand up as soon as you are able to."))
		else
			if(!silent)
				to_chat(src, SPAN_NOTICE("[ishuman(src) ? "You" : "We"] stand up."))
			get_up(instant)
