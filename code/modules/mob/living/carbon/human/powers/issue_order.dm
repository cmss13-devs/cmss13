	...
	if(src.client?.prefs && (src.client?.prefs?.toggle_prefs & TOGGLE_LEADERSHIP_SPOKEN_ORDERS))
		...
		switch(order)
			if(COMMAND_ORDER_MOVE)
				spoken_order = pick("*GET MOVING!*", "*GO, GO, GO!*", "*WE ARE ON THE MOVE!*", "*MOVE IT!*", "*DOUBLE TIME!*", "*KEEP UP LADIES!*", "*PACK IT UP PEOPLE!*", "*MOVE, MOVE, MOVE!*", "*ADVANCE!*", "*KICK YOUR FEET, TWINKLETOES!*", "*ON YOUR FEET SOLDIER, WE ARE LEAVING!*", "*WE ARE OSCAR MIKE!*", "*FORWARD!*", "*GET UP THERE!*", "*GET ON ME!*", "*CHARGE, FORWARD!*", "*ON ME MEN, LET'S MOVE!*", "*STEPPING OFF!*", "*STEP IT UP, LET'S ROLL!*", "*LET'S ROLL!*", "*MOVEMENT ORDER!*")
