	...
	if(src.client?.prefs?.toggle_prefs & TOGGLE_LEADERSHIP_SPOKEN_ORDERS)
		...
		switch(order)
			if(COMMAND_ORDER_MOVE)
				spoken_order = pick("*GET MOVING!*", "*GO, GO, GO!*", "*WE ARE ON THE MOVE!*", "*MOVE IT!*", "*DOUBLE TIME!*", "*KEEP UP LADIES!*", "*PACK IT UP PEOPLE!*", "*MOVE, MOVE, MOVE!*", "*ADVANCE!*", "*KICK YOUR FEET, TWINKLETOES!*", "*ON YOUR FEET SOLDIER, WE ARE LEAVING!*", "*WE ARE OSCAR MIKE!*", "*FORWARD!*", "*GET UP THERE!*", "*GET ON ME!*", "*CHARGE, FORWARD!*", "*ON ME MEN, LET'S MOVE!*", "*STEPPING OFF!*", "*STEP IT UP, LET'S ROLL!*", "*LET'S ROLL!*", "*STACK UP ON ME!*", "*FALL IN!*", "*MOVEMENT ORDER!*")
			if(COMMAND_ORDER_HOLD)
				spoken_order = pick("*DUCK AND COVER!*", "*HOLD THE LINE!*", "*HOLD POSITION!*", "*STAND YOUR GROUND!*", "*STAND AND FIGHT!*", "*HOLD YOUR GROUND!*", "*HUNKER!*", "*HUNKER DOWN!*", "*HOLD HERE!*", "*HOLD WITH ME!*", "*TO ME, HOLD!*", "*DEFENSIVE POSITIONS!*", "*TAKE COVER AND FIRE!*", "*STAND FAST!*", "*NOT ONE STEP BACK!*", "*HOLD FAST!*", "*DIG IN AND HOLD!*", "*BRACE!*", "*TO THE LAST MAN!*", "*KEEP YOUR NERVE, HOLD STRONG!*", "*NO RETREAT, STAND FIRM!*", "*WE HOLD THIS GROUND IN THE NAME OF GOD!*", "*HOLD ORDER!*")
			if(COMMAND_ORDER_FOCUS)
				spoken_order = pick("*FOCUS FIRE!*", "*PICK YOUR TARGETS!*", "*CENTER MASS!*", "*SHORT-CONTROLLED BURSTS!*", "*AIM YOUR SHOTS!*", "*ON MY MARK!*", "*AIMED SHOTS!*", "*AIM FOR THE KILL!*", "*SHOOT EM' DEAD!*", "*KILL THEM ALL!*", "*SUPPRESSIVE FIRE!*", "*TIGHTEN YOUR SHOTS!*", "*LOCK AND LOAD!*", "*TARGET THEIR WEAK SPOTS!*", "*ZERO IN ON THE ENEMY!*", "*EYES ON THE PRIZE!*", "*LET'S ROCK!*", "*HIT 'EM WHERE IT HURTS!*", "*SLAUGHTER 'EM!*", "*BRING THE PAIN!*", "*STEADY YOUR AIM!*", "*EYES UP AND FIRE!*", "*FIRING LANES!*", "*CROSSHAIRS, PEOPLE!*", "*STRIKE!*", "*STRIFE!*", "*FOCUS ORDER!*")
