...
/datum/species/synthetic/colonial/hazard_joe
...
/datum/species/synthetic/colonial/hazard_joe/handle_post_spawn(mob/living/carbon/human/joe)
	. = ..()
	give_action(joe, /datum/action/hazard_joe_emote_panel)


/// Open the WJ's emote panel, which allows them to use voicelines
/datum/species/synthetic/colonial/hazard_joe/proc/open_emote_panel()
	var/datum/hazard_joe_emote_panel/ui = new(usr)
	ui.ui_interact(usr)


/datum/action/hazard_joe_emote_panel
	name = "Open Voice Synthesizer"
	action_icon_state = "looc_toggle"


/datum/action/hazard_joe_emote_panel/can_use_action()
	. = ..()
	if(!.)
		return FALSE
...
/datum/action/hazard_joe_emote_panel/action_activate()
	if(!can_use_action())
		return

	var/mob/living/carbon/human/human_owner = owner
	var/datum/species/synthetic/colonial/hazard_joe/joe_species = human_owner.species
	joe_species.open_emote_panel()


/datum/hazard_joe_emote_panel
...
/datum/hazard_joe_emote_panel/New()
	if(!length(hj_emotes))
		var/list/emotes_to_add = list()
		for(var/datum/emote/living/carbon/human/synthetic/hazard_joe/emote as anything in subtypesof(/datum/emote/living/carbon/human/synthetic/hazard_joe))
...
/datum/hazard_joe_emote_panel/proc/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "JoeEmotes")
		ui.open()
...
/datum/hazard_joe_emote_panel/ui_state(mob/user)
	return GLOB.conscious_state


/datum/hazard_joe_emote_panel/ui_data(mob/user)
	var/list/data = list()
...
/datum/hazard_joe_emote_panel/ui_static_data(mob/user)
	var/list/data = list()

	data["categories"] = hj_categories
	data["emotes"] = list()

	for(var/datum/emote/living/carbon/human/synthetic/hazard_joe/emote as anything in hj_emotes)
		data["emotes"] += list(list(
			"id" = initial(emote.key),
			"text" = (initial(emote.override_say) || initial(emote.say_message)),
			"category" = initial(emote.category),
...
/datum/hazard_joe_emote_panel/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
	...
	switch(action)
		if("emote")
			var/datum/emote/living/carbon/human/synthetic/hazard_joe/path
			if(!params["emotePath"])
				return
			...
			if(!(path in subtypesof(/datum/emote/living/carbon/human/synthetic/hazard_joe)))
				return
