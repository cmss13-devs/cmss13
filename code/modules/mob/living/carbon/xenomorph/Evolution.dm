...
/mob/living/carbon/xenomorph/proc/do_evolve()
	...
	log_game("EVOLVE: [key_name(src)] evolved into [new_xeno]. (Location: [loc.x], [loc.y], [loc.z])")
	if(mind)
...
/mob/living/carbon/xenomorph/verb/Deevolve()
	...
	if(new_xeno)
		log_game("EVOLVE: [key_name(src)] de-evolved into [new_xeno]. (Location: [loc.x], [loc.y], [loc.z])")
