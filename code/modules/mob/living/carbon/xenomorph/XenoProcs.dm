...
/mob/living/carbon/xenomorph/proc/pounced_mob(mob/living/L)
    ...
    if(pounceAction.can_be_shield_blocked)
        if(ishuman(M) && (M.dir in reverse_nearby_direction(dir)))
            ...
            if(H.check_shields("the pounce", get_dir(H, src), attack_type = SHIELD_ATTACK_POUNCE, custom_response = TRUE)) //Human shield block.
                ...
                if(!ishuman(H)) target_fake = "[H]" // Fallback if non-human blocks?

                for(var/mob/M_view in viewers(src))
                    if(!M_view.client) continue
                    if(M_view == src)
                        to_chat(M_view, SPAN_XENODANGER("We slam into [target_fake]!"))
                    else if(isxeno(M_view))
                        to_chat(M_view, SPAN_DANGER("[src] slams into [target_fake]!"))
                    else
                        to_chat(M_view, SPAN_DANGER("[xeno_fake] slams into [H]!"))
            ...
            if(isyautja(H) && prob(75))//Body slam the fuck out of xenos jumping at your front.
                ...
                var/x_desc = GLOB.xeno_caste_descriptors[caste_type] || "strange"
                var/xeno_fake = "a [x_desc] alien"
                var/attacker_fake = "a tall host"
                if(!ishuman(H)) attacker_fake = "[H]"

                for(var/mob/M_view in viewers(src))
                    if(!M_view.client) continue
                    if(M_view == src)
                        to_chat(M_view, SPAN_XENODANGER("[attacker_fake] body slams us!"))
                    else if(isxeno(M_view))
                        to_chat(M_view, SPAN_DANGER("[attacker_fake] body slams [src]!"))
                    else
                        to_chat(M_view, SPAN_DANGER("[H] body slams [xeno_fake]!"))
            ...
            if(iscolonysynthetic(H) && prob(60))
                ...
                for(var/mob/M_view in viewers(src))
                    if(!M_view.client) continue
                    if(M_view == src)
                        to_chat(M_view, SPAN_XENODANGER("[attacker_fake] throws us down after withstanding the pounce!"))
                    else if(isxeno(M_view))
                        to_chat(M_view, SPAN_DANGER("[attacker_fake] withstands being pounced and slams down [src]!"))
                    else
                        to_chat(M_view, SPAN_DANGER("[H] withstands being pounced and slams down [xeno_fake]!"))
    ...
    var/target_fake = "[M]"
    if(ishuman(M))
        var/mob/living/carbon/human/H_targ = M
        var/h_desc = GLOB.human_gender_descriptors[H_targ.gender] || "strange"
        target_fake = "a [h_desc] tall host"

    for(var/mob/M_view in viewers(src))
        if(!M_view.client) continue
        if(M_view == src)
            to_chat(M_view, SPAN_XENODANGER("We [pounceAction.action_text] onto [target_fake]!"))
        else if(isxeno(M_view))
            to_chat(M_view, SPAN_DANGER("[src] [pounceAction.action_text] onto [target_fake]!"))
        else
            to_chat(M_view, SPAN_DANGER("[xeno_fake] [pounceAction.action_text] onto [M]!"))
    ...
    if (pounceAction.should_destroy_objects)
        if(istype(O, /obj/structure/surface/table) || istype(O, /obj/structure/surface/rack) || istype(O, /obj/structure/window_frame))
            ...
            for(var/mob/M_view in viewers(src))
                if(!M_view.client) continue
                if(M_view == src)
                    to_chat(M_view, SPAN_DANGER("We plow straight through [S]!"))
                else if(isxeno(M_view))
                    to_chat(M_view, SPAN_DANGER("[src] plows straight through [S]!"))
                else
                    to_chat(M_view, SPAN_DANGER("[xeno_fake] plows straight through [S]!"))
...
/mob/living/carbon/xenomorph/proc/haul(mob/living/carbon/human/victim)
    ...
    var/x_desc = GLOB.xeno_caste_descriptors[caste_type] || "strange"
    var/xeno_fake = "a [x_desc] alien"
    var/target_fake = "[victim]"
    if(ishuman(victim))
        var/h_desc = GLOB.human_gender_descriptors[victim.gender] || "strange"
        target_fake = "a [h_desc] tall host"

    for(var/mob/M_view in viewers(src))
        if(!M_view.client) continue
        if(M_view == src)
            to_chat(M_view, SPAN_WARNING("We fully restrain [target_fake] and start hauling them!"))
        else if(isxeno(M_view))
            to_chat(M_view, SPAN_WARNING("[src] restrains [target_fake], hauling them effortlessly!"))
        else
            to_chat(M_view, SPAN_WARNING("[xeno_fake] restrains [victim], hauling them effortlessly!"))
...
/mob/living/carbon/xenomorph/proc/about_to_release_hauled()
	...
	var/target_fake = "[user]"
	if(ishuman(user))
		var/h_desc = GLOB.human_gender_descriptors[user.gender] || "strange"
		target_fake = "a [h_desc] tall host"

	to_chat(src, SPAN_XENOWARNING("We feel our grip loosen on [target_fake], we will have to release them soon."))
...
/mob/living/carbon/xenomorph/proc/release_dead_haul()
    ...
    to_chat(src, SPAN_XENOWARNING("[target_fake] is dead. No more use for them now."))
...
/mob/living/carbon/xenomorph/proc/release_haul(stuns = FALSE)
    ...
    for(var/mob/M_view in viewers(src))
        if(!M_view.client) continue
        if(M_view == src)
            to_chat(M_view, SPAN_XENOWARNING("We release [target_fake] from our grip!"))
        else if(isxeno(M_view))
            to_chat(M_view, SPAN_XENOWARNING("[src] releases [target_fake] from their grip!"))
        else
            to_chat(M_view, SPAN_XENOWARNING("[xeno_fake] releases [user] from their grip!"))
