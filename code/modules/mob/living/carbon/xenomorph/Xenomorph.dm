...
/mob/living/carbon/xenomorph/handle_blood_splatter(splatter_dir, splatter_angle, duration)
	var/color_override
	if(special_blood)
		var/datum/reagent/D = GLOB.chemical_reagents_list[special_blood]
		if(D)
			color_override = D.color
	new /obj/effect/temp_visual/dir_setting/bloodsplatter/xenosplatter(loc, splatter_dir, splatter_angle, duration, color_override)
