...
/datum/action/xeno_action/onclick/release_haul/use_ability(atom/A)
	...
	if(!isturf(xeno.loc))
		to_chat(xeno, SPAN_WARNING("You cannot release whatever you are hauling here."))
		return

	if(xeno.haul_contents.len)
		for(var/mob/living/current_haul in xeno.haul_contents)
			// Also has good reason to be a proc on all Xenos
			xeno.release_haul(current_haul, TRUE)
