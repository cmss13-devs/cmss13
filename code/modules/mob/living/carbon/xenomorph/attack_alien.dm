	...
	switch(intent)
		...
		if(INTENT_HELP)
			if(on_fire)
				...
				var/x_desc = GLOB.xeno_caste_descriptors[attacking_xeno.caste_type] || "strange"
				var/xeno_fake = "a [x_desc] alien"
				var/target_fake = "[src]"
				if(ishuman(src))
					var/h_desc = GLOB.human_gender_descriptors[src.gender] || "strange"
					target_fake = "a [h_desc] tall host"

				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_NOTICE("We caress [target_fake] with our claws."))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_NOTICE("[attacking_xeno] caresses [target_fake] with its claws."))
					else
						to_chat(M_view, SPAN_NOTICE("[xeno_fake] caresses [src] with its claws."))
		...
		if(INTENT_GRAB)
			...
			if(!unblockable && check_shields(attacking_xeno.name, get_dir(src, attacking_xeno), custom_response = TRUE)) // Blocking check
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_DANGER("Our grab was blocked by [target_fake]'s shield!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[attacking_xeno]'s grab is blocked by [target_fake]'s shield!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake]'s grab is blocked by [src]'s shield!"))
		...
		if(INTENT_HARM)
			...
			if(attacking_xeno.claw_restrained())
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_XENONOTICE("We try to strike [target_fake] but fail due to our restraints!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_NOTICE("[attacking_xeno] tries to strike [target_fake]"))
					else
						to_chat(M_view, SPAN_NOTICE("[xeno_fake] tries to strike [src]"))
				return XENO_ATTACK_ACTION
			...
			if(attacking_xeno.can_not_harm(src, check_hive_flags=FALSE)) // We manually check hive_flags later
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_XENONOTICE("We nibble [target_fake]"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_NOTICE("[attacking_xeno] nibbles [target_fake]"))
					else
						to_chat(M_view, SPAN_NOTICE("[xeno_fake] nibbles [src]"))
				return XENO_ATTACK_ACTION
			...
			if(stat == DEAD)
				to_chat(attacking_xeno, SPAN_WARNING("[target_fake] is dead, why would we want to touch it?"))
				return XENO_NO_DELAY_ACTION
			...
			if(attacking_xeno.caste && !attacking_xeno.caste.is_intelligent)
				...
				if(embryo_allied)
					if(HAS_TRAIT(src, TRAIT_NESTED))
						...
						for(var/mob/M_view in viewers(src))
							if(!M_view.client) continue
							if(M_view == attacking_xeno)
								to_chat(M_view, SPAN_XENONOTICE("We nibble [target_fake], as it has a sister inside we should not harm."))
							else if(isxeno(M_view))
								to_chat(M_view, SPAN_NOTICE("[attacking_xeno] nibbles [target_fake]"))
							else
								to_chat(M_view, SPAN_NOTICE("[xeno_fake] nibbles [src]"))
					...
					if(!HAS_FLAG(attacking_xeno.hive.hive_flags, XENO_SLASH_INFECTED))
						...
						for(var/mob/M_view in viewers(src))
							if(!M_view.client) continue
							if(M_view == attacking_xeno)
								to_chat(M_view, SPAN_XENONOTICE("We nibble [target_fake], as queen forbade slashing of infected hosts!"))
							else if(isxeno(M_view))
								to_chat(M_view, SPAN_NOTICE("[attacking_xeno] nibbles [target_fake]"))
							else
								to_chat(M_view, SPAN_NOTICE("[xeno_fake] nibbles [src]"))
				...
				if(!HAS_FLAG(attacking_xeno.hive.hive_flags, XENO_SLASH_NORMAL))
					...
					for(var/mob/M_view in viewers(src))
						if(!M_view.client) continue
						if(M_view == attacking_xeno)
							to_chat(M_view, SPAN_XENONOTICE("We nibble [target_fake], as queen forbade slashing!"))
						else if(isxeno(M_view))
							to_chat(M_view, SPAN_NOTICE("[attacking_xeno] nibbles [target_fake]"))
						else
							to_chat(M_view, SPAN_NOTICE("[xeno_fake] nibbles [src]"))
			...
			if(!unblockable && check_shields(attacking_xeno.name, get_dir(src, attacking_xeno), custom_response = TRUE)) // Blocking check
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_DANGER("Our slash is blocked by [target_fake]'s shield!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[attacking_xeno]'s slash is blocked by [target_fake]'s shield!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake]'s slash is blocked by [src]'s shield!"))
				return XENO_ATTACK_ACTION
			...
			if(!damage)
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_DANGER("We lunge at [target_fake]!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[attacking_xeno] lunges at [target_fake]!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake] lunges at [src]!"))
				return XENO_ATTACK_ACTION
				...
				if(istype(wear_mask, /obj/item/clothing/mask/gas/yautja))
					...
					if(prob(knock_chance))
						...
						for(var/mob/M_view in viewers(src))
							if(!M_view.client) continue
							if(M_view == attacking_xeno)
								to_chat(M_view, SPAN_DANGER("We smash off [target_fake]'s [wear_mask.name]!"))
							else if(isxeno(M_view))
								to_chat(M_view, SPAN_DANGER("[attacking_xeno] smashes off [target_fake]'s [wear_mask.name]!"))
							else
								to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes off [src]'s [wear_mask.name]!"))
			...
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(M_view == attacking_xeno)
					to_chat(M_view, SPAN_DANGER("We [attacking_xeno.slash_verb] [target_fake]!"))
				else if(isxeno(M_view))
					to_chat(M_view, SPAN_DANGER("[attacking_xeno] [attacking_xeno.slashes_verb] [target_fake]!"))
				else
					to_chat(M_view, SPAN_DANGER("[xeno_fake] [attacking_xeno.slashes_verb] [src]!"))
		...
		if(INTENT_DISARM)
			...
			if(!unblockable && check_shields(attacking_xeno.name, get_dir(src, attacking_xeno), custom_response = TRUE)) // Blocking check
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_DANGER("We tackle is blocked by [target_fake]'s shield!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[attacking_xeno]'s tackle is blocked by [target_fake]'s shield!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake]'s tackle is blocked by [src]'s shield!"))
				return XENO_ATTACK_ACTION
			...
			if(attacking_xeno.attempt_tackle(src, tackle_mult, tackle_min_offset, tackle_max_offset))
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_DANGER("We tackle down [target_fake]!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[attacking_xeno] tackles down [target_fake]!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake] tackles down [src]!"))
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == attacking_xeno)
						to_chat(M_view, SPAN_DANGER("We try to tackle [target_fake]!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[attacking_xeno] tries to tackle [target_fake]!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake] tries to tackle [src]!"))
				knocked_down = FALSE
...
/mob/living/attack_alien(mob/living/carbon/xenomorph/M)
	...
	switch(M.a_intent)
		if(INTENT_HELP)
			var/x_desc = GLOB.xeno_caste_descriptors[M.caste_type] || "strange"
			var/xeno_fake = "a [x_desc] alien"
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(isxeno(M_view))
					to_chat(M_view, SPAN_NOTICE("[M] caresses [src] with its claws."))
				else
					to_chat(M_view, SPAN_NOTICE("[xeno_fake] caresses [src] with its claws."))
		...
		if(INTENT_HARM)
			...
			if(!damage)
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == M)
						to_chat(M_view, SPAN_DANGER("We lunge at [src]!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[M] lunges at [src]!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake] lunges at [src]!"))
				return XENO_ATTACK_ACTION
			...
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(M_view == M)
					to_chat(M_view, SPAN_DANGER("We [M.slash_verb] [src]!"))
				else if(isxeno(M_view))
					to_chat(M_view, SPAN_DANGER("[M] [M.slashes_verb] [src]!"))
				else
					to_chat(M_view, SPAN_DANGER("[xeno_fake] [M.slashes_verb] [src]!"))
		...
		if(INTENT_DISARM)
			...
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(M_view == M)
					to_chat(M_view, SPAN_WARNING("We shove [src]!"))
				else if(isxeno(M_view))
					to_chat(M_view, SPAN_WARNING("[M] shoves [src]!"))
				else
					to_chat(M_view, SPAN_WARNING("[xeno_fake] shoves [src]!"))
...
/mob/living/attack_larva(mob/living/carbon/xenomorph/larva/M)
	M.animation_attack_on(src)
	var/x_desc = "squirming"
	var/xeno_fake = "a [x_desc] alien larva"
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_DANGER("We nudge our head against [src]."))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[M] nudges its head against [src]."))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] nudges its head against [src]."))
	...
	if(breakable)
		...
		if(health <= 0)
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(M_view == M)
					to_chat(M_view, SPAN_DANGER("We slice [src] apart!"))
				else if(isxeno(M_view))
					to_chat(M_view, SPAN_DANGER("[M] slices [src] apart!"))
				else
					to_chat(M_view, SPAN_DANGER("[xeno_fake] slices [src] apart!"))
			deconstruct(FALSE)
		else
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
...
/obj/structure/surface/table/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	if(health <= 0)
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("We destroy [src] with our tail!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] destroys [src] with its tail!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] destroys [src] with its tail!"))
		deconstruct(FALSE)
	else
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("We strike [src] with our tail!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] strikes [src] with its tail!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] strikes [src] with its tail!"))
...
/obj/structure/barricade/attack_alien(mob/living/carbon/xenomorph/M)
	...
	if(is_wired)
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == M)
				to_chat(M_view, SPAN_DANGER("The barbed wire slices into us!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("The barbed wire slices into [M]!"))
			else
				to_chat(M_view, SPAN_DANGER("The barbed wire slices into [xeno_fake]!"))
...
/obj/structure/barricade/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	if(health <= 0)
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("We destroy \the [src] with our tail!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] destroys \the [src] with its tail!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] destroys \the [src] with its tail!"))
	else
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("We strike \the [src] with our tail!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] strikes \the [src] with its tail!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] strikes \the [src] with its tail!"))
	if(is_wired)
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("The barbed wire slices into our tail!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("The barbed wire slices into \the [xeno]'s tail!"))
			else
				to_chat(M_view, SPAN_DANGER("The barbed wire slices into \the [xeno_fake]'s tail!"))
...
/obj/structure/bed/medevac_stretcher/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == xeno)
			to_chat(M_view, SPAN_DANGER("We smack [src] with our tail!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[xeno] smacks [src] with its tail!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] smacks [src] with its tail!"))
	xeno.tail_stab_animation(src, blunt_stab)
...
/obj/structure/machinery/light/attack_alien(mob/living/carbon/xenomorph/M)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_DANGER("We smash [src]!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[M] smashes [src]!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes [src]!"))
...
/obj/structure/window/attack_alien(mob/living/carbon/xenomorph/M)
	if(M.a_intent == INTENT_HELP)
		...
		// Variables removed here because the human message is just a sound description
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == M)
				to_chat(M_view, SPAN_WARNING("We creepily tap on [src]."))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_WARNING("[M] creepily taps on [src] with its huge claw."))
			else
				to_chat(M_view, SPAN_WARNING("You hear a glass tapping sound."))
	else
...
/obj/structure/window/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == xeno)
			to_chat(M_view, SPAN_DANGER("We smash [src] with our tail!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[xeno] smashes [src] with its tail!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes [src] with its tail!"))
...
/obj/structure/machinery/door/window/attack_alien(mob/living/carbon/xenomorph/M)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_DANGER("We smash against [src]!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[M] smashes against [src]!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes against [src]!"))
...
/obj/structure/machinery/door/window/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	if(health <= 0)
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("We shatter [src] with our tail!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] shatters [src] with its tail!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] shatters [src] with its tail!"))
	else
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
...
/obj/structure/grille/attack_alien(mob/living/carbon/xenomorph/M)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_DANGER("We mangle [src]!"))
			to_chat(M_view, SPAN_DANGER("You hear twisting metal!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[M] mangles [src]!"))
			to_chat(M_view, SPAN_DANGER("You hear twisting metal!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] mangles [src]!"))
			to_chat(M_view, SPAN_DANGER("You hear twisting metal!"))
	...
	if(shock(M, 70))
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == M)
				to_chat(M_view, SPAN_DANGER("ZAP! You twitch and dance like a monkey on hyperzine!"))
				to_chat(M_view, SPAN_DANGER("You hear a sharp ZAP and a smell of ozone."))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("ZAP! [M] spazzes wildly amongst a smell of burnt ozone."))
			else
				to_chat(M_view, SPAN_DANGER("ZAP! [xeno_fake] spazzes wildly amongst a smell of burnt ozone."))
...
/obj/structure/grille/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == xeno)
			to_chat(M_view, SPAN_DANGER("We mangle [src] with our tail!"))
			to_chat(M_view, SPAN_DANGER("You hear twisting metal!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[xeno] mangles [src] with its tail!"))
			to_chat(M_view, SPAN_DANGER("You hear twisting metal!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] mangles [src] with its tail!"))
			to_chat(M_view, SPAN_DANGER("You hear twisting metal!"))
	...
	if(shock(xeno, 70))
		for(var/mob/M_view in viewers(src))
			...
			if(M_view == xeno)
				...
				to_chat(M_view, SPAN_DANGER("ZAP! [xeno] spazzes wildly amongst a smell of burnt ozone."))
			else
				to_chat(M_view, SPAN_DANGER("ZAP! [xeno_fake] spazzes wildly amongst a smell of burnt ozone."))
...
/obj/structure/fence/attack_alien(mob/living/carbon/xenomorph/M)
	...
	for(var/mob/M_view in viewers(src))
		...
		if(M_view == M)
			to_chat(M_view, SPAN_DANGER("We mangle [src]!"))
			to_chat(M_view, SPAN_DANGER("We hear twisting metal!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[M] mangles [src]!"))
			to_chat(M_view, SPAN_DANGER("We hear twisting metal!"))
		else
...
/obj/structure/mirror/attack_alien(mob/living/carbon/xenomorph/M)
	...
	if(M.a_intent == INTENT_HELP)
		...
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == M)
				to_chat(M_view, SPAN_WARNING("We ogle our own reflection in [src]."))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_WARNING("[M] ogles its own reflection in [src]."))
			else
				to_chat(M_view, SPAN_WARNING("[xeno_fake] ogles its own reflection in [src]."))
		return XENO_NONCOMBAT_ACTION
	else
		var/x_desc = GLOB.xeno_caste_descriptors[M.caste_type] || "strange"
		var/xeno_fake = "a [x_desc] alien"
...
/obj/structure/airlock_assembly/attack_alien(mob/living/carbon/xenomorph/xeno)
	...
	if(health <= 0)
		for(var/mob/M_view in viewers(src))
			...
			if(M_view == xeno)
				...
				to_chat(M_view, SPAN_DANGER("[xeno] slices [src] apart!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] slices [src] apart!"))
		if(!unacidable)
			qdel(src)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == xeno)
			to_chat(M_view, SPAN_DANGER("We [xeno.slash_verb] [src]!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[xeno] [xeno.slashes_verb] [src]!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] [xeno.slashes_verb] [src]!"))
...
/obj/structure/machinery/door/airlock/attack_alien(mob/living/carbon/xenomorph/M)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_WARNING("We dig into [src] and begin to pry it open."))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_WARNING("[M] digs into [src] and begins to pry it open."))
		else
			to_chat(M_view, SPAN_WARNING("[xeno_fake] digs into [src] and begins to pry it open."))
		...
		if(density) //Make sure it's still closed
			spawn(0)
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == M)
						to_chat(M_view, SPAN_DANGER("We pry [src] open."))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[M] pries [src] open."))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake] pries [src] open."))
	return XENO_NO_DELAY_ACTION
	...
	if(M.a_intent != INTENT_HARM)
		...
		if(islarva(M))
			...
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(M_view == M)
					to_chat(M_view, SPAN_XENONOTICE("We claw [src]."))
				else if(isxeno(M_view))
					to_chat(M_view, SPAN_XENONOTICE("[M] claws [src]!"))
				else
					to_chat(M_view, SPAN_XENONOTICE("[xeno_fake] claws [src]!"))
...
/obj/structure/machinery/attack_alien(mob/living/carbon/xenomorph/M)
	...
	if(health <= 0)
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == M)
				to_chat(M_view, SPAN_DANGER("We slice \the [src] apart!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[M] slices \the [src] apart!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] slices \the [src] apart!"))
		if(!unacidable)
			qdel(src)
	else
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == M)
				to_chat(M_view, SPAN_DANGER("We [M.slash_verb] \the [src]!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[M] [M.slashes_verb] \the [src]!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] [M.slashes_verb] \the [src]!"))
	return XENO_ATTACK_ACTION
...
/obj/structure/morgue/attack_alien(mob/living/carbon/xenomorph/alien)
	...
	var/x_desc = GLOB.xeno_caste_descriptors[alien.caste_type] || "strange"
	var/xeno_fake = "a [x_desc] alien"
	...
	if(health <= 0)
		for(var/mob/M_view in viewers(src))
			...
			if(M_view == alien)
				...
				to_chat(M_view, SPAN_DANGER("[alien] slices \the [src] apart!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] slices \the [src] apart!"))
		...
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == alien)
				to_chat(M_view, SPAN_DANGER("We [alien.slash_verb] \the [src]!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[alien] [alien.slashes_verb] \the [src]!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] [alien.slashes_verb] \the [src]!"))
	return XENO_ATTACK_ACTION
...
/obj/structure/machinery/colony_floodlight/attack_alien(mob/living/carbon/xenomorph/xeno)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == xeno)
			to_chat(M_view, SPAN_DANGER("We slash and claw at the bright light!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[xeno] slashes away at [src]!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] slashes away at [src]!"))
...
/obj/structure/machinery/colony_floodlight/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == xeno)
			to_chat(M_view, SPAN_DANGER("We smash at the bright light with our tail!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[xeno] smashes [src] with its tail!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes [src] with its tail!"))
...
/obj/structure/machinery/colony_floodlight/attack_larva(mob/living/carbon/xenomorph/larva/M)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_DANGER("In a rage, we start biting [src], but with no effect!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[M] starts biting [src]!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] starts biting [src]!"))
...
/turf/open/snow/attack_alien(mob/living/carbon/xenomorph/M)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_NOTICE("We start \the clearing out [src]..."))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_NOTICE("[M] starts clearing out \the [src]..."))
		else
			to_chat(M_view, SPAN_NOTICE("[xeno_fake] starts clearing out \the [src]..."))
		...
		if(!opened)
			...
			if(prob(difficulty))
				...
				for(var/mob/M_view in viewers(src))
					if(!M_view.client) continue
					if(M_view == M)
						to_chat(M_view, SPAN_DANGER("We smash \the [src] open!"))
					else if(isxeno(M_view))
						to_chat(M_view, SPAN_DANGER("[M] smashes \the [src] open!"))
					else
						to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes \the [src] open!"))
		else
			var/x_desc = GLOB.xeno_caste_descriptors[M.caste_type] || "strange"
			var/xeno_fake = "a [x_desc] alien"
			for(var/mob/M_view in viewers(src))
	...
	if(!opened)
		...
		if(prob(difficulty))
			...
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(M_view == xeno)
					to_chat(M_view, SPAN_DANGER("We smash [src] open with our tail!"))
				else if(isxeno(M_view))
					to_chat(M_view, SPAN_DANGER("[xeno] smashes [src] open with its tail!"))
				else
					to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes [src] open with its tail!"))
	else
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
...
/obj/structure/machinery/vending/attack_alien(mob/living/carbon/xenomorph/M)
	...
	if(M.a_intent == INTENT_HARM)
		...
		if(prob(M.melee_damage_lower))
			...
			for(var/mob/M_view in viewers(src))
				if(!M_view.client) continue
				if(M_view == M)
					to_chat(M_view, SPAN_DANGER("We enter a frenzy and smash [src] apart!"))
				else if(isxeno(M_view))
					to_chat(M_view, SPAN_DANGER("[M] smashes [src] beyond recognition!"))
				else
					to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes [src] beyond recognition!"))
			malfunction()
			tip_over()
		else
			for(var/mob/M_view in viewers(src))
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == M)
			to_chat(M_view, SPAN_WARNING("We begin to lean against [src]."))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_WARNING("[M] begins to lean against [src]."))
		else
			to_chat(M_view, SPAN_WARNING("[xeno_fake] begins to lean against [src]."))
	...
	if(do_after(M, shove_time, INTERRUPT_ALL, BUSY_ICON_HOSTILE))
		...
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == M)
				to_chat(M_view, SPAN_DANGER("We knock [src] down!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[M] knocks [src] down!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] knocks [src] down!"))
		tip_over()
	return XENO_NO_DELAY_ACTION
...
/obj/structure/machinery/vending/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	if(prob(xeno.melee_damage_upper))
		...
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("You enter a frenzy and smash [src] with your tail apart!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] smashes [src] with its tail beyond recognition!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] smashes [src] with its tail beyond recognition!"))
		malfunction()
		tip_over()
	else
		for(var/mob/M_view in viewers(src))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("You slash [src] with your tail!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] slashes [src] with its tail!"))
			else
				to_chat(M_view, SPAN_DANGER("[xeno_fake] slashes [src] with its tail!"))
		playsound(loc, 'sound/effects/metalhit.ogg', 25, 1)
	xeno.tail_stab_animation(src, blunt_stab)
	return TAILSTAB_COOLDOWN_NORMAL
...
/obj/structure/inflatable/handle_tail_stab(mob/living/carbon/xenomorph/xeno, blunt_stab)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == xeno)
			to_chat(M_view, SPAN_DANGER("We puncture [src] with our tail!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[xeno] punctures [src] with its tail!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] punctures [src] with its tail!"))
...
/obj/structure/prop/invuln/joey/attack_alien(mob/living/carbon/xenomorph/alien)
	...
	for(var/mob/M_view in viewers(src))
		if(!M_view.client) continue
		if(M_view == alien)
			to_chat(M_view, SPAN_DANGER("We [alien.slash_verb] [src]!"))
		else if(isxeno(M_view))
			to_chat(M_view, SPAN_DANGER("[alien] [alien.slashes_verb] [src]!"))
		else
			to_chat(M_view, SPAN_DANGER("[xeno_fake] [alien.slashes_verb] [src]!"))
