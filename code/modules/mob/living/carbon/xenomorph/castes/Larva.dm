...
/mob/living/carbon/Xenomorph/Larva/update_progression()
	...
	if(amount_grown >= max_grown)	// to avoid spam
		if(age < 0)
			to_chat(src, SPAN_XENODANGER("Strength ripples through your small form. You are ready to be shaped to the King's will. <a href='?src=\ref[src];evolve=1;'>Evolve</a>"))
