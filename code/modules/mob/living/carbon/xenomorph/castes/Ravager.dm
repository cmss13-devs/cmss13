...
/datum/action/xeno_action/onclick/empower/use_ability(atom/target)
	var/mob/living/carbon/xenomorph/empowering_rav = owner
	if(!empowering_rav.check_state())
		return
	...
	if(!activated_once)
		...
		empowering_rav.visible_message(SPAN_XENODANGER("[empowering_rav] starts empowering!"), SPAN_XENODANGER("We start empowering ourself!"))
		activated_once = TRUE
		...
		actual_empower(empowering_rav)
		return TRUE

/datum/action/xeno_action/onclick/empower/proc/actual_empower(mob/living/carbon/xenomorph/empowering_rav)
	var/datum/behavior_delegate/ravager_base/behavior = empowering_rav.behavior_delegate

	activated_once = FALSE
	button.icon_state = "template"
	empowering_rav.visible_message(SPAN_XENOWARNING("[empowering_rav] gets empowered by the surrounding enemies!"), SPAN_XENOWARNING("We feel a rush of power from the surrounding enemies!"))
	empowering_rav.create_empower()

	var/list/mobs_in_range = oviewers(empower_range, empowering_rav)
	// Spook patrol
	empowering_rav.emote("tail")
	...
	for(var/mob/living/mob in mobs_in_range)
		...
		if(mob.stat == DEAD || HAS_TRAIT(mob, TRAIT_NESTED))
			continue
		if(empowering_rav.can_not_harm(mob))
			continue
	...
	empowering_rav.add_xeno_shield(accumulative_health, XENO_SHIELD_SOURCE_RAVAGER)
	empowering_rav.overlay_shields()
	if(empower_targets >= behavior.super_empower_threshold) //you go in deep you reap the rewards
		super_empower(empowering_rav, empower_targets, behavior)

/datum/action/xeno_action/onclick/empower/proc/super_empower(mob/living/carbon/xenomorph/empowering_rav, empower_targets, datum/behavior_delegate/ravager_base/behavior)
	empowering_rav.visible_message(SPAN_DANGER("[empowering_rav] glows an eerie red as it empowers further with the strength of [empower_targets] hostiles!"), SPAN_XENOHIGHDANGER("We begin to glow an eerie red, empowered by the [empower_targets] enemies!"))
	empowering_rav.emote("roar")
	...
	empowering_rav.add_filter("empower_rage", 1, list("type" = "outline", "color" = color, "size" = 3))

	addtimer(CALLBACK(src, PROC_REF(weaken_superbuff), empowering_rav, behavior), 5 SECONDS)

/datum/action/xeno_action/onclick/empower/proc/weaken_superbuff(mob/living/carbon/xenomorph/empowering_rav, datum/behavior_delegate/ravager_base/behavior)

	empowering_rav.remove_filter("empower_rage")
	var/color = "#FF0000"
	var/alpha = 35
	color += num2text(alpha, 2, 16)
	empowering_rav.add_filter("empower_rage", 1, list("type" = "outline", "color" = color, "size" = 3))

	addtimer(CALLBACK(src, PROC_REF(remove_superbuff), empowering_rav, behavior), 1.5 SECONDS)

/datum/action/xeno_action/onclick/empower/proc/remove_superbuff(mob/living/carbon/xenomorph/empowering_rav, datum/behavior_delegate/ravager_base/behavior)
	behavior.empower_targets = 0

	empowering_rav.visible_message(SPAN_DANGER("[empowering_rav]'s glow slowly dims."), SPAN_XENOHIGHDANGER("Our glow fades away, the power leaving our form!"))
	empowering_rav.remove_filter("empower_rage")
...
/datum/action/xeno_action/activable/pounce/charge/additional_effects(mob/living/living)
	...
	var/mob/living/carbon/xenomorph/pouncing_rav = owner
	var/datum/behavior_delegate/ravager_base/behavior = pouncing_rav.behavior_delegate
	if(behavior.empower_targets < behavior.super_empower_threshold)
		return
	pouncing_rav.visible_message(SPAN_XENODANGER("[pouncing_rav] uses its shield to bash [human] as it charges at them!"), SPAN_XENODANGER("We use our shield to bash [human] as we charge at them!"))
	human.apply_effect(behavior.knockdown_amount, WEAKEN)
	human.attack_alien(pouncing_rav, rand(pouncing_rav.melee_damage_lower, pouncing_rav.melee_damage_upper))

	var/facing = get_dir(pouncing_rav, human)

	pouncing_rav.throw_carbon(human, facing, behavior.fling_distance, SPEED_VERY_FAST, shake_camera = FALSE, immobilize = TRUE)

/datum/action/xeno_action/activable/scissor_cut/use_ability(atom/target_atom)
	var/mob/living/carbon/xenomorph/scissors_user = owner

	if (!action_cooldown_check())
		return

	if (!scissors_user.check_state())
		return

	// Determine whether or not we should daze here
	var/should_sslow = FALSE
	var/datum/behavior_delegate/ravager_base/ravager_delegate = scissors_user.behavior_delegate
	...
	var/facing = Get_Compass_Dir(scissors_user, target_atom)
	var/turf/turf = scissors_user.loc
	var/turf/temp = scissors_user.loc
	var/list/telegraph_atom_list = list()
	...
	if (length(target_turfs) >= 2)
		scissors_user.animation_attack_on(target_turfs[length(target_turfs)], 15)

	// Hmm today I will kill a marine while looking away from them
	scissors_user.face_atom(target_atom)
	scissors_user.emote("roar")
	scissors_user.visible_message(SPAN_XENODANGER("[scissors_user] sweeps its claws through the area in front of it!"), SPAN_XENODANGER("We sweep our claws through the area in front of us!"))
	...
	for (var/turf/target_turf in target_turfs)
		for (var/mob/living/carbon/carbon_target in target_turf)
			...
			if(scissors_user.can_not_harm(carbon_target))
				continue
			scissors_user.flick_attack_overlay(carbon_target, "slash")
			carbon_target.apply_armoured_damage(damage, ARMOR_MELEE, BRUTE)
			playsound(get_turf(carbon_target), "alien_claw_flesh", 30, TRUE)

			if(should_sslow)
				new /datum/effects/xeno_slow/superslow(carbon_target, scissors_user, ttl = superslow_duration)
