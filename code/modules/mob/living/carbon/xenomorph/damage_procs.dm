/mob/living/carbon/Xenomorph/attackby(obj/item/I, mob/user)
	if(user.a_intent == INTENT_HELP)
		if(HAS_TRAIT(I, TRAIT_TOOL_MULTITOOL) && ishuman(user))
			...
			return
		if(I.type in SURGERY_TOOLS_PINCH)
			if(!iff_tag)
			...
			if(!do_after(user, 5 SECONDS * SURGERY_TOOLS_PINCH[I.type], INTERRUPT_ALL, BUSY_ICON_GENERIC, src, INTERRUPT_DIFF_LOC, BUSY_ICON_GENERIC))
				return
			if(!iff_tag)
