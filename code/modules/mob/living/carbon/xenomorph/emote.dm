	...
	switch(act)
		...
		if("growl")
			if(isXenoPredalien(src))
				emote_audio_helper("<B>The [name]</B> growls.", 'sound/voice/predalien_growl.ogg', 25)
				return

			emote_audio_helper("<B>The [name]</B> growls.", "alien_growl")
			return

		if("hiss")
			if(isXenoPredalien(src))
				emote_audio_helper("<B>The [name]</B> hisses.", 'sound/voice/predalien_hiss.ogg', 25)
				return

			emote_audio_helper("<B>The [name]</B> hisses.", "alien_hiss")
			return

		if("needhelp")
			emote_audio_helper("<B>The [name]</B> needs help!", "alien_help", 25)
			return		

		if("roar")
			if(isXenoPredalien(src))
				emote_audio_helper("<B>The [name]</B> roars!", 'sound/voice/predalien_roar.ogg', 40)
				return

			emote_audio_helper("<B>The [name]</B> roars!", "alien_roar", 40)
			return	

		if("tail")
			emote_audio_helper("<B>The [name]</B> swipes its tail.", "alien_tail_swipe", 40)
			return
...
/mob/living/carbon/Xenomorph/proc/emote_audio_helper(var/message, var/sound_to_play, var/volume = 15)
