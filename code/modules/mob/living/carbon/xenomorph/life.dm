	...
	if(!stat)
			...
			if(current_aura && plasma_stored > 5)
				if(caste_type == XENO_CASTE_QUEEN && anchored) //stationary queen's pheromone apply around the observed xeno.
					...
					if(!phero_center || !phero_center.loc)
						return
					if(phero_center.loc.z == Q.loc.z || SSmapping.same_z_traits(phero_center.loc.z, Q.loc.z))//Only same Z-level
						use_current_aura = TRUE
						aura_center = phero_center
				else
					use_current_aura = TRUE

		if(leader_current_aura && hive && hive.living_xeno_queen && (hive.living_xeno_queen.loc.z == loc.z || SSmapping.same_z_traits(hive.living_xeno_queen.loc.z, loc.z))) //Same Z-level as the Queen!
		...
		if(use_current_aura || use_leader_aura)
			for(var/mob/living/carbon/xenomorph/Z as anything in GLOB.living_xeno_list)
				if(Z.ignores_pheromones || Z.ignore_aura == current_aura || Z.ignore_aura == leader_current_aura || (Z.z != z && SSmapping.same_z_traits(Z.z, z)) || get_dist(aura_center, Z) > floor(6 + aura_strength * 2) || !HIVE_ALLIED_TO_HIVE(Z.hivenumber, hivenumber))
					continue
				if(use_leader_aura)
