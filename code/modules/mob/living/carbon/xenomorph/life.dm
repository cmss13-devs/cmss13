...
/mob/living/carbon/Xenomorph/proc/heal_wounds(m, recov)
	...
	value = L["healing"]
	apply_damage(-((maxHealth / 70) + 0.5 + (maxHealth / 70) * recov/2)*(m) + heal_penalty, BRUTE)
	apply_damage(-(maxHealth / 60 + 0.5 + (maxHealth / 60) * recov/2)*(m) + heal_penalty, BURN)
	apply_damage(-(maxHealth * 0.1 + 0.5 + (maxHealth * 0.1) * recov/2)*(m) + heal_penalty, OXY)
	apply_damage(-(maxHealth / 5 + 0.5 + (maxHealth / 5) * recov/2)*(m) + heal_penalty, TOX)
	updatehealth()
