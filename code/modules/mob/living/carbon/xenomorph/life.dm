...
/mob/living/carbon/xenomorph/handle_regular_status_updates(regular_update = TRUE)
	if(regular_update && health <= 0 && (!caste || (fire_immunity & FIRE_IMMUNITY_NO_IGNITE | FIRE_IMMUNITY_COMPLETE) || !on_fire)) //Sleeping Xenos are also unconscious, but all crit Xenos are under 0 HP. Go figure
...
/mob/living/carbon/xenomorph/proc/handle_environment()
	...
	var/recoveryActual = (!caste || (fire_immunity & FIRE_IMMUNITY_NO_IGNITE | FIRE_IMMUNITY_COMPLETE) || !on_fire) ? recovery_aura : 0
