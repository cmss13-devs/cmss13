...
/mob/living/carbon/xenomorph/proc/handle_pheromones()
	...
	if(!stat)
		...
		if(leader_current_aura && hive && hive.living_xeno_queen)
		...
		if(use_current_aura || use_leader_aura)
			for(var/mob/living/carbon/xenomorph/Z as anything in GLOB.living_xeno_list)
				if(Z.ignores_pheromones || Z.ignore_aura == current_aura || Z.ignore_aura == leader_current_aura || get_dist(aura_center, Z) > floor(6 + aura_strength * 2) || !HIVE_ALLIED_TO_HIVE(Z.hivenumber, hivenumber))
					continue
				if(use_leader_aura)
