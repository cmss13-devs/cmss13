...
/datum/mark_menu_ui/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("choose_mark")
			...
			to_chat(X, SPAN_NOTICE("Теперь вы будете объявлять '<b>[x.name]</b>!' при маркировке смолы.")) // SS220 EDIT ADDICTION
		...
		if("watch")
			...
			if(!mark_to_watch)
				return
			to_chat(X, SPAN_XENONOTICE("Вы наблюдаете за смоляной меткой [mark_to_watch.mark_meaning.name] около «[get_area_name(mark_to_watch)]».")) // SS220 EDIT ADDICTION
		...
		if("destroy")
			...
			if(mark_to_destroy.createdby == X.nicknumber)
				to_chat(X, SPAN_XENONOTICE("Вы приказываете уничтожить смоляную метку [mark_to_destroy.mark_meaning.name].")) // SS220 EDIT ADDICTION
				...
				to_chat(mark_to_destroy_owner, SPAN_XENONOTICE("Королева [X.name] отдала приказ об уничтожении вашей смоляной метки [mark_to_destroy.mark_meaning.name].")) // SS220 EDIT ADDICTION
				qdel(mark_to_destroy)
				update_all_data()
				. = TRUE
				return
			to_chat(X, SPAN_XENONOTICE("У вас нет разрешения на это."))
			return
		if("force")
			...
			if(selected_xeno == FunkTownOhyea)
				for(var/mob/living/carbon/xenomorph/forced_xeno in X.hive.totalXenos)
					...
					to_chat(forced_xeno, SPAN_XENOANNOUNCE("Улей! Ваша королева приказывает: [mark_to_force.mark_meaning.desc] около «[get_area_name(mark_to_force)]». (<a href='byond://?src=\ref[X];overwatch=1;target=\ref[mark_to_force]'>Посмотреть</a>) (<a href='byond://?src=\ref[X];track=1;target=\ref[mark_to_force]'>Отслеживать</a>)")) // SS220 EDIT ADDICTION
			...
			if (!selected_xeno || QDELETED(selected_xeno) || selected_xeno.stat == DEAD || should_block_game_interaction(selected_xeno) || !X.check_state(1))
				...
				to_chat(selected_xeno, SPAN_XENOBOLDNOTICE("Ваша королева приказывает вам следовать за: [mark_to_force.mark_meaning.desc] около «[get_area_name(mark_to_force)]». (<a href='byond://?src=\ref[X];overwatch=1;target=\ref[mark_to_force]'>Посмотреть</a>) (<a href='byond://?src=\ref[X];track=1;target=\ref[mark_to_force]'>Отслеживать</a>)")) // SS220 EDIT ADDICTION
