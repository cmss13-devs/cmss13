/datum/xeno_strain/dancer
	...
	description = "We lose all acid-based abilities and a small amount of our armor in exchange for increased movement speed, evasion, and unparalleled agility. This strain excels at rapid repositioning, bullet dodging, and phasing effortlessly through enemies and allies alike. Slashing enemies applies a red tag, altering how our tail abilities function. Tagged enemies cause Impale to strike twice and transform Tail Trip into a powerful knockdown instead of a brief stun. Our new Tail Stab adapts to our intent. When used in Disarm mode, it becomes a Blunt, armor-piercing strike. When enemies are brought close to death, yellow tags will spread to nearby foes. Slashing yellow-tagged enemies reduces the cooldown of our tail abilities, and using a tail trip or impale ability on a yellow-tagged target grants no cooldown penalty."
...
/datum/behavior_delegate/praetorian_dancer/proc/try_spread_tags_from(mob/living/carbon/human/source)
	...
	if(spread_count >= 0)
		to_chat(bound_xeno, SPAN_XENOHIGHDANGER("Fear spreads among the prey, their weakness fuels our instincts to strike them down!"))
...
/datum/action/xeno_action/activable/prae_tail_trip/use_ability(atom/target_atom)
	...
	if(isxeno(target_carbon))
		...
		if(Xeno.mob_size >= MOB_SIZE_BIG)
			...
			dancer_user.visible_message(SPAN_XENODANGER("[dancer_user] smashes [Xeno] with it's tail!"), SPAN_XENODANGER("We smash [Xeno] with our tail!"))
			to_chat(Xeno, SPAN_XENOHIGHDANGER("We feel dizzy as [dancer_user] smashes you with their tail!"))
