...
/datum/action/xeno_action/activable/valkyrie_rage/use_ability(atom/target_atom)
	var/mob/living/carbon/xenomorph/xeno = owner
	var/mob/living/carbon/xenomorph/buffing_target = target_atom
	...
	if(!buffing_target.ally_of_hivenumber(xeno.hivenumber))
		to_chat(xeno, SPAN_XENOWARNING("Why would we help our enemies?!"))
		return

	if(buffing_target.is_dead())
		to_chat(xeno, SPAN_XENOWARNING("No amount of anger can bring our sister back."))
		return

	if(istype(buffing_target.strain, /datum/xeno_strain/valkyrie))
		to_chat(xeno, SPAN_XENOWARNING("We can't order another valkyrie with our rage."))
		return

	if(HAS_TRAIT(buffing_target, TRAIT_VALKYRIE_ARMORED))
		to_chat(xeno, SPAN_XENOWARNING("[buffing_target] is already enraged!"))
		return

	if(get_dist(xeno, buffing_target) > max_range) // an edge case where you watch somebody from hive status, and move while observing somebody so the buff applies since theres no actual range check.
		to_chat(xeno, SPAN_XENOWARNING("We need to be closer."))
		return
	...
	playsound(get_turf(xeno), "alien_roar", 40)
	to_chat(xeno, SPAN_XENODANGER("Our rage drives us forward, our healing and armor is increased."))
	xeno.create_custom_empower(icolor = "#a31010", ialpha = 200, small_xeno = TRUE)
	xeno.add_filter("raging", 1, list("type" = "outline", "color" = "#a31010", "size" = 1))
	xeno.balloon_alert(xeno, "we feel an overwhelming rage", text_color = "#93ec78")
	xeno.armor_modifier += armor_buff
	ADD_TRAIT(xeno, TRAIT_VALKYRIE_ARMORED, TRAIT_SOURCE_ABILITY("Tantrum"))
	xeno.recalculate_armor()
	RegisterSignal(xeno, list(COMSIG_XENO_PRE_APPLY_ARMOURED_DAMAGE, COMSIG_XENO_PRE_CALCULATE_ARMOURED_DAMAGE_PROJECTILE), PROC_REF(calculate_damage_mitigation_self))
...
/datum/action/xeno_action/activable/valkyrie_rage/proc/remove_rage()
	var/mob/living/carbon/xenomorph/xeno = owner
	var/datum/behavior_delegate/praetorian_valkyrie/behavior = xeno.behavior_delegate
	xeno.remove_filter("raging")
	xeno.armor_modifier -= armor_buff
	armor_buffs_active = FALSE
	behavior.raging = FALSE
	REMOVE_TRAIT(xeno, TRAIT_VALKYRIE_ARMORED, TRAIT_SOURCE_ABILITY("Tantrum"))
	xeno.recalculate_armor()
	UnregisterSignal(xeno, list(COMSIG_XENO_PRE_APPLY_ARMOURED_DAMAGE, COMSIG_XENO_PRE_CALCULATE_ARMOURED_DAMAGE_PROJECTILE))
	to_chat(xeno, SPAN_XENOHIGHDANGER("We feel ourselves calm down."))



/datum/action/xeno_action/activable/valkyrie_rage/proc/remove_target_speed()
	var/mob/living/carbon/xenomorph/xeno = focus_rage.resolve()
	if(xeno) //if the target was qdeleted it would be null so you need to check for it
...
/datum/action/xeno_action/activable/valkyrie_rage/proc/remove_target_rage()
	var/mob/living/carbon/xenomorph/xeno = focus_rage.resolve()
	if(xeno) //if the target was qdeleted it would be null so you need to check for it
		xeno.armor_modifier -= target_armor_buff
		xeno.remove_filter("raging")
		REMOVE_TRAIT(xeno, TRAIT_VALKYRIE_ARMORED, TRAIT_SOURCE_ABILITY("Tantrum"))
		xeno.recalculate_armor()
		UnregisterSignal(xeno, list(COMSIG_XENO_PRE_APPLY_ARMOURED_DAMAGE, COMSIG_XENO_PRE_CALCULATE_ARMOURED_DAMAGE_PROJECTILE))
...
/datum/action/xeno_action/activable/high_gallop/use_ability(atom/target_atom)
	...
	xeno.visible_message(SPAN_XENODANGER("[xeno] stomps its feet furiously, breaking the ground underneath!"), SPAN_XENODANGER("We send a shockwave through the ground, breaking the balance of anyone in front of us!"))
	xeno.emote("roar")
	playsound(xeno, 'sound/effects/alien_footstep_charge3.ogg', 35, 0)

	for(var/turf/range in target_turfs)
		for(var/mob/living/carbon/target_carbon in range)
			if(target_carbon.stat == DEAD)
				continue

			if(!isxeno_human(target_carbon) || xeno.can_not_harm(target_carbon))
				continue
			...
			new /datum/effects/xeno_slow(target_carbon, xeno, ttl = get_xeno_stun_duration(target, 25))

		for(var/obj/item/explosive/grenade/grenades in range) // sends back grenades
			var/direction = get_dir(xeno, grenades)
...
/datum/action/xeno_action/onclick/fight_or_flight/use_ability(atom/target_atom)
	var/mob/living/carbon/xenomorph/xeno = owner
	...
	playsound(xeno, 'sound/voice/xenos_roaring.ogg', 125)
	for(var/mob/living/carbon/xenomorph/allied_xenomorphs in range(range, xeno))
		if(!allied_xenomorphs.ally_of_hivenumber(xeno.hivenumber))
			continue
		to_chat(allied_xenomorphs, SPAN_XENOWARNING("Every single inch in our body moves on its own to fight."))
		xeno.create_shriekwave(3)
...
/datum/action/xeno_action/activable/tail_stab/tail_fountain/use_ability(atom/target_atom)
	var/mob/living/carbon/xenomorph/xeno = owner
	var/mob/living/carbon/xenomorph/target_xeno = target_atom


	var/distance = get_dist(xeno, target_xeno)

	if(distance > 2)
		to_chat(xeno, SPAN_XENOWARNING("We need to be closer to our target."))
		return

	if(target_atom	== xeno)
		to_chat(xeno, SPAN_XENOWARNING("We can't extinguish ourselves."))
		return

	if(!iscarbon(target_atom))
		to_chat(xeno, SPAN_XENOWARNING("We need to target something."))
		return

	XENO_ACTION_CHECK_USE_PLASMA(xeno)

	playsound(xeno, 'sound/effects/splat.ogg', 40, FALSE)
	target_xeno.ExtinguishMob() // This can both help your allies, or help caps that are on fire.
	apply_cooldown()
	xeno.visible_message(SPAN_XENODANGER("[xeno] pours acid all over [target_xeno] using its tail."), SPAN_XENOHIGHDANGER("We use our tail to pour acid over [target_xeno]"))
...
/datum/action/xeno_action/activable/prae_retrieve/use_ability(atom/target_atom)
	...
	for(var/x in 0 to max_distance)
		...
		for(var/obj/structure/target_structure in temp)
			if(target_structure.opacity || ((istype(target_structure, /obj/structure/barricade) || istype(target_structure, /obj/structure/girder)  && target_structure.density|| istype(target_structure, /obj/structure/machinery/door)) && target_structure.density))
				blocked = TRUE
				break
		if(blocked)
		...
		if(target_turf in turflist)
			break

		turflist += target_turf
		facing = get_dir(target_turf, target_atom)
	...
	for(var/turf/choosed_turf in turflist)
		if(target_xeno in choosed_turf)
