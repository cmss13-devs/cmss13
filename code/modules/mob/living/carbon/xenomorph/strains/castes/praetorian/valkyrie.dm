...
/datum/action/xeno_action/activable/valkyrie_rage/use_ability(atom/target_atom)
	...
	if(raging_valkyrie.action_busy)
		return
	...
	XENO_ACTION_CHECK(raging_valkyrie)
...
/datum/action/xeno_action/activable/high_gallop/use_ability(atom/target_atom)
	...
	if(!target_atom || target_atom.layer >= FLY_LAYER || !isturf(valkyrie.loc))
		return
	...
	var/facing = Get_Compass_Dir(valkyrie, target_atom)
	...
	for(var/turf/range in target_turfs)
		for(var/mob/living/carbon/target_carbon in range)
			...
			if(!isxeno_human(target_carbon) || valkyrie.can_not_harm(target_carbon))
				continue
			...
			new /datum/effects/xeno_slow(target_carbon, valkyrie, ttl = get_xeno_stun_duration(target, 25))
...
/datum/action/xeno_action/onclick/fight_or_flight/use_ability(atom/target_atom)
	...
	XENO_ACTION_CHECK_USE_PLASMA(valkyrie_flight)
...
/datum/action/xeno_action/activable/tail_stab/tail_fountain/use_ability(atom/target_atom)
	...
	var/distance = get_dist(extinguisher_tail, target_xeno)

	if(distance > 2)
		to_chat(extinguisher_tail, SPAN_XENOWARNING("We need to be closer to our target."))
		return

	if(target_atom	== extinguisher_tail)
	...
	XENO_ACTION_CHECK_USE_PLASMA(extinguisher_tail)
	...
	apply_cooldown()
	extinguisher_tail.visible_message(SPAN_XENODANGER("[extinguisher_tail] pours acid all over [target_xeno] using its tail."), SPAN_XENOHIGHDANGER("We use our tail to pour acid over [target_xeno]"))
