...
/mob/living/proc/_handle_tank_edge_move(NewLoc, direct)
	...
	if(isturf(NewLoc) && !(NewLoc in tank.locs))
		...
		if(!edge || edge.z != z || (edge in tank.locs))
			return FALSE
		if(tank._blocked_except_mobs(edge, src, here))
			return TRUE
 ...
 *
 * This proc checks the tank this mob's vehicle_rider component (if any) points to.
 ...
 * * FALSE   - If the mob has no vehicle_rider component, if the Z-levels of the mob and Tank mistmach, if the tile Tu is invalid,
	...
	if(is_on_tank_hull())
		...
		if(isturf(NewLoc))
			...
			if(tank && !(NewLoc in tank.locs))
				// clear_on_top() runs for every mob forced off the hull. Humans additionally get punished.
				if(ishuman(src))
...
/mob/living/Collide(atom/movable/moving_atom)
	...
	if(src_on_tank != target_on_tank)
		if(!src_on_tank && target_on_tank)
			// Reroute to the tank's own Collided() so src can climb up and share the tile with living_mob.
			var/obj/vehicle/multitile/tank = living_mob.get_tank_on_top_of()
			if(tank)
				now_pushing = FALSE
				tank.Collided(src)
				return
...
/mob/living/proc/update_layer()
	...
	else if(is_on_tank_hull())
		// Standing, not being carried, on the tank hull.
