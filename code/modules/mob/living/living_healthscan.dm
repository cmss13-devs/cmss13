	...
	if(ishuman(target_mob))
		...
		if(!permadead)
			...
			if(human_target_mob.stat == DEAD)
				if((human_target_mob.health + 20) > human_target_mob.human_health_threshold_dead)
					...
					if(((human_target_mob.health + 50) < human_target_mob.human_health_threshold_dead) && !issynth(human_target_mob))
						advice += list(list(
