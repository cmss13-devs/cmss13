		...
		spawn(0)
			if(do_after(usr,(breakout_time*MINUTES_1), FALSE))
				...
				if(istype(usr.loc, /obj/structure/closet/secure_closet))
					...
					SC.locked = 0
					SC.update_icon()
					to_chat(usr, "<span class='danger'>You successfully break out!</span>")
					for(var/mob/O in viewers(L.loc))
						O.show_message("<span class='danger'><B>\the [usr] successfully broke out of \the [SC]!</B></span>", 1)
					...
					to_chat(usr, "<span class='danger'>You successfully break out!</span>")
					for(var/mob/O in viewers(L.loc))
						O.show_message("<span class='danger'><B>\the [usr] successfully broke out of \the [C]!</B></span>", 1)
	...
	//breaking out of handcuffs & putting out fires
		...
		if(CM.handcuffed && CM.canmove && (CM.last_special <= world.time))
			...
			if(can_break_cuffs) //Don't want to do a lot of logic gating here.
				...
				for(var/mob/O in viewers(CM))
					O.show_message( "<span class='danger'><B>[usr] attempts to remove [LC]!</B></span>", 1)
				spawn(0)
