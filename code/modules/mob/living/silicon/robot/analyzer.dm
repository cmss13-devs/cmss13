...
/obj/item/device/robotanalyzer/attack(mob/living/M as mob, mob/living/user as mob)
	...
	user.show_message("\t Key: <font color='#FFA500'>Electronics</font>/<span class='caution'>Brute</span>", 1)
	user.show_message("\t Damage Specifics: <font color='#FFA500'>[BU]</font> - <span class='caution'>[BR]</span>")
	...
	if (istype(M, /mob/living/silicon/robot))
		...
		if(length(damaged)>0)
			for(var/datum/robot_component/org in damaged)
				...
				(org.toggled)	?	"Toggled ON"	:	"<span class='caution'>Toggled OFF</span>",\
				(org.powered)	?	"Power ON"		:	"<span class='caution'>Power OFF</span>"),1)
		else
