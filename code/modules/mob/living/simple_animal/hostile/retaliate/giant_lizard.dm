...
/mob/living/simple_animal/hostile/retaliate/giant_lizard
	...
	///Cooldown to stop pet_emotes spam.
	COOLDOWN_DECLARE(pet_emotes_cooldown)
	...
	var/list/acceptable_foods = list(/obj/item/reagent_container/food/snacks/meat, /obj/item/reagent_container/food/snacks/resin_fruit)
...
///Proc for growling.
/mob/living/simple_animal/hostile/retaliate/giant_lizard/proc/growl(target_mob)
	if(target_mob)
...
/mob/living/simple_animal/hostile/retaliate/giant_lizard/get_examine_text(mob/user)
	if(user.faction in faction_group)
...
/mob/living/simple_animal/hostile/retaliate/giant_lizard/attack_hand(mob/living/carbon/human/attacking_mob)
	...
	if(attacking_mob.a_intent == INTENT_HELP && (attacking_mob.faction in faction_group))
		if(!resting)
			chance_to_rest += 15
		if(COOLDOWN_FINISHED(src, pet_emotes_cooldown) && resting)
			chance_to_rest = 0
			manual_emote(pick(pet_emotes))
			COOLDOWN_START(src, pet_emotes_cooldown, rand(5, 8) SECONDS)
...
/mob/living/simple_animal/hostile/retaliate/giant_lizard/proc/MoveTo(target, distance = 1)
