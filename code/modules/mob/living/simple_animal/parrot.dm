	...
	for(var/mob/living/carbon/C in view(1,src))
		...
		if(stolen_item)
			if(C.drop_inv_item_to_loc(stolen_item, src))
				...
				visible_message("[src] grabs \the [held_item] out of [C]'s hand!", SPAN_NOTICE("You grab \the [held_item] out of [C]'s hand!"), "You hear the sounds of wings flapping furiously.")
				return held_item
