...
/mob/proc/point_to_atom(atom/A, turf/T)
    ...
    var/src_xeno_view = "[src]"
    var/src_human_view = "[src]"
    if(isxeno(src))
        var/mob/living/carbon/xenomorph/X = src
        var/x_desc = GLOB.xeno_caste_descriptors[X.caste_type] || "strange"
        src_human_view = "<b>a [x_desc] alien</b>"
    else if(ishuman(src))
        src_xeno_view = "<b>a tall host</b>"

    var/target_xeno_view = "[A]"
    var/target_human_view = "[A]"
    if(ishuman(A))
    ...
    for(var/mob/M_view in viewers(src))
        ...
        if(isxeno(M_view))
            to_chat(M_view, "[src_xeno_view] points to [target_xeno_view]")
        else
            to_chat(M_view, "[src_human_view] points to [target_human_view]")
...
/mob/living/start_pulling(atom/movable/clone/AM, lunge, no_msg)
    ...
    if(!QDELETED(AM.pulledby) && !QDELETED(M))
        ...
        if(isxeno(src))
            var/mob/living/carbon/xenomorph/X = src
            var/x_desc = GLOB.xeno_caste_descriptors[X.caste_type] || "strange"
            src_human_view = "a [x_desc] alien"
        else if(ishuman(src))
            src_xeno_view = "a tall host"

        var/grabber_xeno_view = "[AM.pulledby]"
        var/grabber_human_view = "[AM.pulledby]"
        if(ishuman(AM.pulledby))
            grabber_xeno_view = "a tall host"
        else if(isxeno(AM.pulledby))
            var/mob/living/carbon/xenomorph/G = AM.pulledby
            var/g_desc = GLOB.xeno_caste_descriptors[G.caste_type] || "strange"
            grabber_human_view = "a [g_desc] alien"
        ...
        for(var/mob/M_view in viewers(src))
            if(!M_view.client) continue
            if(isxeno(M_view))
                to_chat(M_view, SPAN_WARNING("[src_xeno_view] has broken [grabber_xeno_view]'s grip on [victim_xeno_view]!"))
            else
                to_chat(M_view, SPAN_WARNING("[src_human_view] has broken [grabber_human_view]'s grip on [victim_human_view]!"))
    ...
    if(!QDELETED(M))
        ...
        if(!no_msg)
            ...
            for(var/mob/M_view in viewers(src))
                if(!M_view.client) continue
                if(isxeno(M_view))
                    to_chat(M_view, SPAN_WARNING("[src_xeno_view] has grabbed [victim_xeno_view] passively!"))
                else
                    to_chat(M_view, SPAN_WARNING("[src_human_view] has grabbed [victim_human_view] passively!"))
...
/mob/proc/yank_out_object()
    ...
    if(self)
        ...
        for(var/mob/M_view in viewers(src))
            if(!M_view.client) continue
            if(M_view == src)
                to_chat(M_view, SPAN_WARNING("<b>You rip [selection] out of your body.</b>"))
            else if(isxeno(M_view))
                to_chat(M_view, SPAN_WARNING("<b>[src_xeno_view] rips [selection] out of their body.</b>"))
            else
                to_chat(M_view, SPAN_WARNING("<b>[src_human_view] rips [selection] out of their body.</b>"))
    else
        var/usr_xeno_view = "[usr]"
        var/usr_human_view = "[usr]"
        if(isxeno(usr))
            var/mob/living/carbon/xenomorph/X = usr
            var/x_desc = GLOB.xeno_caste_descriptors[X.caste_type] || "strange"
            usr_human_view = "a [x_desc] alien"
        else if(ishuman(usr))
            usr_xeno_view = "a tall host"
        ...
        for(var/mob/M_view in viewers(src))
            if(!M_view.client) continue
            if(M_view == usr) // The one yanking
                to_chat(M_view, SPAN_WARNING("<b>You rip [selection] out of [src]'s body.</b>"))
            else if(M_view == src) // The victim
                to_chat(M_view, SPAN_WARNING("<b>[usr] rips [selection] out of your body.</b>"))
            else if(isxeno(M_view))
                to_chat(M_view, SPAN_WARNING("<b>[usr_xeno_view] rips [selection] out of [src_xeno_view]'s body.</b>"))
            else
                to_chat(M_view, SPAN_WARNING("<b>[usr_human_view] rips [selection] out of [src_human_view]'s body.</b>"))
