...
/mob/proc/show_message(msg, type, alt_msg, alt_type, message_flags = CHAT_TYPE_OTHER)
	...
	// Type 0b01 : You can't see, but you can hear
	if((type & 1) && ((sdisabilities & DISABILITY_BLIND) || blinded))
		msg = alt_msg
		type = alt_type
		alt_msg = null // If you can't hear either, there's nothing left

	// Type 0b10 : You can't hear, but you can see
	...
	else if(message_flags & CHAT_TYPE_ALL_COMBAT)
		chat_type = MESSAGE_TYPE_COMBAT // Pre-tag these so client-side tgchat doesn't have to regex for type
...
*/
/mob/visible_message(message, self_message, blind_message, max_distance, message_flags = CHAT_TYPE_OTHER, list/mob/blacklist = list())
	blacklist += src
	// If this action targets us, then others see it as targeting someone else
	if(message_flags & CHAT_TYPE_TARGETS_ME)
		message_flags &= CHAT_TYPE_TARGETS_ME
		message_flags |= CHAT_TYPE_BEING_HIT
	if(self_message)
		show_message(self_message, 1, blind_message, 2, message_flags)
	else
		show_message(message, 1, blind_message, 2, message_flags)
...
/mob/proc/affected_message(mob/affected, message_mob, message_affected, message_viewer)
	var/list/mob/blacklist
	if(src != affected && message_affected)
		affected?.show_message(message_affected, 1)
	if(affected)
		blacklist = list(affected)
	visible_message(message_viewer, message_mob, blacklist = blacklist)
