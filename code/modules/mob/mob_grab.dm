...
/obj/item/grab/proc/progress_passive(mob/living/carbon/human/user, mob/living/victim)
	...
	for(var/mob/M_view in viewers(user))
		...
		if(isxeno(M_view))
			to_chat(M_view, SPAN_WARNING("[user_fake] has grabbed [victim_fake] aggressively!"))
		else
...
/obj/item/grab/proc/progress_aggressive(mob/living/carbon/human/user, mob/living/victim)
	...
	for(var/mob/M_view in viewers(user))
		if(!M_view.client) continue
		if(isxeno(M_view))
			to_chat(M_view, SPAN_WARNING("[user_fake] holds [victim_fake] by the neck and starts choking them!"))
		else
...
/obj/item/grab/attack(mob/living/dragged_mob, mob/living/user)
	if(dragged_mob == grabbed_thing)
		...
		for(var/mob/M_view in viewers(xeno))
			if(!M_view.client) continue
			if(M_view == xeno)
				to_chat(M_view, SPAN_DANGER("We start restraining [target_fake]!"))
			else if(isxeno(M_view))
				to_chat(M_view, SPAN_DANGER("[xeno] starts to restrain [target_fake]!"))
			else
