...
/mob/new_player/proc/AttemptLateSpawn(rank)
	...
	if(!GLOB.enter_allowed)
		to_chat(usr, SPAN_WARNING("There is an administrative lock on entering the game! (The dropship likely crashed into the Almayer. This should take at most 20 minutes.)"))
		return
	if(!GLOB.RoleAuthority.assign_role(src, GLOB.RoleAuthority.roles_for_mode[rank], 1))
	...
	GLOB.RoleAuthority.equip_role(character, GLOB.RoleAuthority.roles_for_mode[rank], late_join = TRUE)
	EquipCustomItems(character)

	if(GLOB.security_level > SEC_LEVEL_BLUE || GLOB.EvacuationAuthority.evac_status)
		to_chat(character, SPAN_HIGHDANGER("As you stagger out of hypersleep, the sleep bay blares: '[GLOB.EvacuationAuthority.evac_status ? "VESSEL UNDERGOING EVACUATION PROCEDURES, SELF DEFENSE KIT PROVIDED" : "VESSEL IN HEIGHTENED ALERT STATUS, SELF DEFENSE KIT PROVIDED"]'."))
		character.put_in_hands(new /obj/item/storage/box/kit/cryo_self_defense(character.loc))

	GLOB.data_core.manifest_inject(character)
	SSticker.minds += character.mind//Cyborgs and AIs handle this in the transform proc.	//TODO!!!!! ~Carn
	SSticker.mode.latejoin_tally += GLOB.RoleAuthority.calculate_role_weight(GLOB.RoleAuthority.roles_for_mode[rank])
