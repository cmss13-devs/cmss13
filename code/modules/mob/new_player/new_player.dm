...
/mob/new_player/proc/AttemptLateSpawn(rank)
	...
	if(character.mind && character.mind.player_entity)
		...
		if(player.get_playtime(STATISTIC_HUMAN) == 0 && player.get_playtime(STATISTIC_XENO) == 0)
			msg_admin_niche("NEW JOIN: <b>[key_name(character, 1, 1, 0)] [ADMIN_INFO(character)]</b>. IP: [character.lastKnownIP], CID: [character.computer_id]")
		if(character.client)
			var/client/C = character.client
			if(C.player_data && C.player_data.playtime_loaded && length(C.player_data.playtimes) == 0)
				msg_admin_niche("NEW PLAYER: <b>[key_name(character, 1, 1, 0)] [ADMIN_INFO(C)]</b>. IP: [character.lastKnownIP], CID: [character.computer_id]")
