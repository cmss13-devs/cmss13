...
/obj/item/device/camera/proc/camera_get_icon(list/turfs, turf/center, psize_x = 96, psize_y = 96, datum/turf_reservation/clone_area, size_x, size_y, total_x, total_y)
	...
	var/mutable_appearance/backdrop = mutable_appearance('icons/mob/hud/screen1.dmi', "flash")
	...
	if(!skip_normal) //these are not clones
		...
		for(var/X in sorted) //these are clones
			...
			var/xo = (clone.x - center.x) * 32 + clone.pixel_x + xcomp + clone.step_x
			// Center of the image in Y
			var/yo = (clone.y - center.y) * 32 + clone.pixel_y + ycomp + clone.step_y
