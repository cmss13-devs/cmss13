...
/obj/structure/machinery/power/apc/proc/toggle_breaker(mob/user)
	operating = !operating
	update()
	msg_admin_niche("[user] turned [operating ? "on" : "off"] \the [src] in (<A HREF='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[src.loc.x];Y=[src.loc.y];Z=[src.loc.z]'>JMP</a>).")
	update_icon()
