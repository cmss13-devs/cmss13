...
/obj/machinery/particle_accelerator/control_box/Topic(href, href_list)
	...
	if(href_list["togglep"])
		...
		if (active)
			message_admins("PA Control Computer turned ON by [key_name(usr, usr.client)](<A HREF='?_src_=admin_holder;adminmoreinfo=\ref[usr]'>?</A>) in ([x],[y],[z] - <A HREF='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)",0,1)
		...
		if(strength > 2)
			...
			message_admins("PA Control Computer increased to [strength] by [key_name(usr, usr.client)](<A HREF='?_src_=admin_holder;adminmoreinfo=\ref[usr]'>?</A>) in ([x],[y],[z] - <A HREF='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)",0,1)
