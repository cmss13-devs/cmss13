...
/obj/machinery/particle_accelerator/control_box/Topic(href, href_list)
	...
	if(href_list["togglep"])
		src.toggle_power()
		investigate_log("turned [active?"<span class='caution'>ON</span>":"<font color='green'>OFF</font>"] by [usr.key]","singulo")
		if (active)
		...
		if(strength > 2)
			...
			investigate_log("increased to <span class='caution'>[strength]</span> by [usr.key]","singulo")
		for(var/obj/structure/particle_accelerator/part in connected_parts)
...
/obj/machinery/particle_accelerator/control_box/process()
	if(src.active)
		...
		if( length(connected_parts) < 6 )
			investigate_log("lost a connected part; It <span class='caution'>powered down</span>.","singulo")
			src.toggle_power()
			return
		//emit some particles
