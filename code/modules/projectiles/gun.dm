...
/obj/item/weapon/gun/proc/set_gun_user(mob/to_set)
	if(to_set == gun_user)
		if(gun_user)
			RegisterSignal(gun_user, COMSIG_MOB_MOUSEDOWN, PROC_REF(start_fire), override=TRUE)
			RegisterSignal(gun_user, COMSIG_MOB_MOUSEDRAG, PROC_REF(change_target), override=TRUE)
			RegisterSignal(gun_user, COMSIG_MOB_MOUSEUP, PROC_REF(stop_fire), override=TRUE)
		return
	if(gun_user)
