...
/obj/item/weapon/gun
	...
	*/
	...
	/// IFF stuff, if enabled, range in which we check targets and firedelay if there are friendlies
	COOLDOWN_DECLARE(iff_halt_cooldown)
	var/iff_range = 7
	var/iff_cooldown = 0.5 SECONDS
...
/obj/item/weapon/gun/proc/check_firing_lane(obj/projectile/projectile_to_fire, atom/target, mob/living/user)
	SIGNAL_HANDLER

	var/angle = get_angle(user, target)

	var/range_to_check = iff_range
	var/cooldown = iff_cooldown
	...
	var/list/checked_turfs = getline2(extended_target_turf, user_turf)
