...
/mob/living/carbon/xenomorph/bullet_act(obj/item/projectile/P)
	...
	if(damage > 0 && !(ammo_flags & AMMO_IGNORE_ARMOR))
		...
		var/list/damagedata = list(
			...
			"armor_break_pr_pen" = P.ammo.pen_armor_punch,
			"armor_break_flat" = P.ammo.damage_armor_punch,
			"armor_integrity" = armor_integrity,
			"direction" = P.dir,
		)
		SEND_SIGNAL(src, COMSIG_XENO_PRE_CALCULATE_ARMORED_DAMAGE_PROJECTILE, damagedata)
