...
/datum/reagents/proc/trans_to(atom/target, amount=1, var/multiplier=1, var/preserve_data=1, var/reaction = TRUE)
...
/datum/reagents/proc/trans_to_datum(datum/reagents/target, amount=1, var/multiplier=1, var/preserve_data=1, var/reaction = TRUE)//if preserve_data=0, the reagents data will be lost. Usefull if you use data for some strange stuff and don't want it to be transferred.
...
/datum/reagents/proc/trans_to_ingest(atom/movable/target, amount=1, var/multiplier=1, var/preserve_data=1) //For items ingested. A delay is added between ingestion and addition of the reagents
...
/datum/reagents/proc/copy_to(obj/target, amount=1, var/multiplier=1, var/preserve_data=1, var/safety = 0)
	if(!target)
		return
	if(!target.reagents || total_volume<=0)
		return
...
/datum/reagents/proc/trans_id_to(obj/target, reagent, amount=1, var/preserve_data=1)//Not sure why this proc didn't exist before. It does now! /N
	if(!target)
		return
...
/datum/reagents/proc/reaction(atom/A, method=TOUCH, var/volume_modifier=0)
...
/datum/reagents/proc/remove_all_type(reagent_type, amount, strict = 0, var/safety = 1) // Removes all reagent of X type. @strict set to 1 determines whether the childs of the type are included.
	if(!isnum(amount))
		return TRUE
...
/datum/reagents/proc/combust(turf/sourceturf, radius, intensity, duration, supplemented, firecolor, smokerad)
	if(!sourceturf)
		return
	if(sourceturf.chemexploded)
		return // Has recently exploded, so no explosion this time. Prevents instagib satchel charges.
