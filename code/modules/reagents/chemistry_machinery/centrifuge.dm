...
/obj/structure/machinery/centrifuge/attackby(obj/item/thing, mob/living/user)
	...
	if(!skillcheck(user, SKILL_RESEARCH, SKILL_RESEARCH_TRAINED))
		...
		return
	if(thing.is_open_container() || istype(thing, /obj/item/reagent_container/blood))
		if(input_container)
			to_chat(user, SPAN_WARNING("A container is already loaded into [src]."))
			return
		if(input_source == INPUT_TURING)
			to_chat(user, SPAN_WARNING("[src] is expecting its input from the Turing. Toggle input back to container if you want to centrifuge this."))
			return
		if(user.drop_inv_item_to_loc(thing, src))
			input_container = thing
			if(output_container)
				icon_state = "centrifuge_on_closed"
			else
				icon_state = "centrifuge_on_open"
	else if(istype(thing, /obj/item/storage/fancy/vials))
		if(output_container)
			to_chat(user, SPAN_WARNING("A vial box is already loaded into [src]."))
			return
		if(user.drop_inv_item_to_loc(thing, src))
			output_container = thing
			if(input_container)
				icon_state = "centrifuge_on_closed"
			else
				icon_state = "centrifuge_empty_closed"
	else
		to_chat(user, SPAN_WARNING("[thing] doesn't fit in [src]."))
		return
	if(((input_container && input_source == INPUT_CONTAINER) || turing_ready()) && output_container)
		to_chat(user, SPAN_NOTICE("You insert [thing] and start configuring [src]."))
		updateUsrDialog()
...
/obj/structure/machinery/centrifuge/attack_hand(mob/user as mob)
	...
	if(!input_container && !output_container)
		...
		return
	if(output_container)
		to_chat(user, SPAN_NOTICE("You remove [output_container] from [src]."))
		...
		to_chat(user, SPAN_NOTICE("You remove [input_container] from [src]."))
