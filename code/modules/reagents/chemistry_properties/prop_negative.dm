...
/datum/chem_property/negative/disrupting/process(mob/living/M, potency = 1)
	to_chat(M, SPAN_NOTICE("Your mind feels oddly... quiet."))

/datum/chem_property/negative/disrupting/process_overdose(mob/living/M, potency = 1)
	M.apply_internal_damage(potency, "brain")

/datum/chem_property/negative/disrupting/process_critical(mob/living/M, potency = 1)
	M.apply_effect(potency, PARALYZE)

/datum/chem_property/negative/disrupting/reaction_mob(mob/M, method=TOUCH, volume, potency)
	if(!isxeno(M))
		return
	var/mob/living/carbon/xenomorph/xeno = M
	xeno.AddComponent(/datum/component/status_effect/interference, volume * potency, 90)

/datum/chem_property/negative/neutralizing
...
/datum/chem_property/negative/neutralizing/process(mob/living/M, potency = 1)
	M.apply_damages(0, potency, potency * POTENCY_MULTIPLIER_LOW)

/datum/chem_property/negative/neutralizing/process_overdose(mob/living/M, potency = 1)
	M.apply_damages(0, POTENCY_MULTIPLIER_MEDIUM * potency, potency)

/datum/chem_property/negative/neutralizing/process_critical(mob/living/M, potency = 1)
	M.apply_internal_damage(potency, "liver")

/datum/chem_property/negative/neutralizing/reaction_mob(mob/M, method=TOUCH, volume, potency)
	if(!isliving(M))
		return
	var/mob/living/L = M
...
/datum/chem_property/negative/neutralizing/reaction_turf(turf/T, volume, potency)
	if(!istype(T))
		return
