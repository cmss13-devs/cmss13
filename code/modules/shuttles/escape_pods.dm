...
/datum/shuttle/ferry/marine/evacuation_pod/proc/prepare_for_launch()
	...
	spawn()
		var/list/turf/Surrounding_turfs_to_D = list(get_step(D, NORTH), get_step(D, SOUTH), get_step(D, EAST), get_step(D, WEST))
		for(var/i=1;i<=length(Surrounding_turfs_to_D);i++)
			for(var/obj/structure/machinery/door/poddoor/almayer/open/evac/Evac_Poddoor in Surrounding_turfs_to_D[i].contents)
				Evac_Poddoor.close()
		D.unlock()
		D.close()
		D.lock()
