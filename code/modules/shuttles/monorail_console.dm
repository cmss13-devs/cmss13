...
/obj/structure/machinery/computer/shuttle_control/monorail/Topic(href, href_list)
	...
	if(href_list["move"])
		...
		if(!skip_time_lock && world.time < SHUTTLE_TIME_LOCK && istype(shuttle, /datum/shuttle/ferry/marine))
			to_chat(usr, SPAN_WARNING("The monorail is still charging and cannot depart yet. Please wait another [round((SHUTTLE_TIME_LOCK-world.time)/600)] minutes before trying again."))
			return
