...
/datum/shuttle/ferry/supply/short_jump(var/area/origin,var/area/destination)
	...
	spawn(warmup_time)
		...
		if (origin != away_area)
			playsound_spacial(locate(SupplyElevator_x,SupplyElevator_y,SupplyElevator_z), 'sound/machines/asrs_lowering.ogg', 50, 14 SECONDS)
			move(origin, away_area)
			lower_elevator_effect()
			start_gears(SOUTH)
			sleep(91)
		else
			playsound_spacial(locate(SupplyElevator_x,SupplyElevator_y,SupplyElevator_z), 'sound/machines/asrs_raising.ogg', 50, 17 SECONDS)
			start_gears(NORTH)
			sleep(70)
