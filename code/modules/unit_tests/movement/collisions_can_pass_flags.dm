 ...
 * with a mover's pass flags will not block movement
 */
/datum/unit_test/can_pass_blocker/Run()
	movement_test(
 ...
 * with a mover's pass flags will not block movement
 */
/datum/unit_test/can_pass_front_directional_blocker/Run()
	movement_test(
		target_direction = SOUTH,
		blocker_positions = list(movement_helper_constants::TARGET_TURF),
		blocker_parameters_entries = list(
 ...
 * with a mover's pass flags will not block movement
 */
/datum/unit_test/can_pass_behind_directional_blocker/Run()
	movement_test(
		target_direction = SOUTH,
