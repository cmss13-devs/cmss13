...
/obj/vehicle/multitile/root/cm_armored/verb/switch_active_hp()
	...
	var/obj/item/hardpoint/HP = hardpoints[slot]
	if(!HP)
		to_chat(usr, SPAN_WARNING("There's nothing installed on that hardpoint."))
...
/obj/vehicle/multitile/root/cm_armored/bullet_act(var/obj/item/projectile/P)
	...
	take_damage_type(damage * (0.25 + penetration/100), dam_type, firer)
...
/obj/vehicle/multitile/root/cm_armored/proc/handle_hardpoint_repair(var/obj/item/O, var/mob/user)
	...
	if(!damaged_hps.len)
		to_chat(user, SPAN_NOTICE("All of the hardpoints are in working order."))
		return
	...
	if(old) //If so, fuck you get it outta here
		to_chat(user, SPAN_WARNING("Please remove the attached hardpoint module first."))
		return
	...
	user.visible_message(SPAN_NOTICE("[user] repairs the [slot] slot on [src]."),
...
/obj/vehicle/multitile/root/cm_armored/proc/install_hardpoint(var/obj/item/hardpoint/HP, var/mob/user)
	...
	if(damaged_hps.Find(HP.slot))
		to_chat(user, SPAN_WARNING("You need to fix the hardpoint first."))
		return
	...
	user.visible_message(SPAN_NOTICE("[user] begins installing [HP] on the [HP.slot] hardpoint slot on [src]."),
		SPAN_NOTICE("You begin installing [HP] on the [HP.slot] hardpoint slot on [src]."))
