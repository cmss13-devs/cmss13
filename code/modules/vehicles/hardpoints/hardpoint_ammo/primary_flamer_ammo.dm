...
/**
 * Refuels this tank from a fuel source, mirroring the handheld flamer's own fuel type rules.
 * Also accepts the infantry M240's fuel tank as a source/destination. No fuel mixing either way.
 * Rejects specialist fuels, but allows research-generated/custom fuels through.
 */
/obj/item/ammo_magazine/hardpoint/primary_flamer/afterattack(obj/target, mob/user, flag)
	if(get_dist(user, target) > 1)
		return ..()
	if(!istype(target, /obj/structure/reagent_dispensers/tank/fuel) && !istype(target, /obj/item/tool/weldpack) && !istype(target, /obj/item/storage/backpack/marine/engineerpack) && !istype(target, /obj/item/ammo_magazine/hardpoint/primary_flamer) && !istype(target, /obj/item/ammo_magazine/flamer_tank))
		return ..()
	...
	if(to_add.flags & REAGENT_TYPE_SPECIALIST)
		to_chat(user, SPAN_WARNING("[src] cannot accept specialist fuels!"))
		return

	if(rejects_weak_fuel(to_add))
...
/// Whether afterattack() rejects to_add as not potent enough. /smoke overrides to accept anything.
/obj/item/ammo_magazine/hardpoint/primary_flamer/proc/rejects_weak_fuel(datum/reagent/to_add)
	return !to_add.intensityfire && to_add.id != "stablefoam"
...
/**
 * DRG-N equivalent of the infantry M240's smoke tank. Starts empty and accepts any chemical.
 * Disperses smoke further and longer than the LZR-N's version.
 */
/obj/item/ammo_magazine/hardpoint/primary_flamer/smoke
	name = "DRG-N Offensive Flamer Unit Smoke Tank"
	desc = "A tank fitted to be accepted by the DRG-N Offensive Flamer, holding powdered smoke that expands when exposed to an open flame and carries any chemicals along with it. Its larger reservoir and wider nozzle let it lay down a thicker, longer-reaching smokescreen than the LZR-N's own smoke tank."
	caliber = "Powdered Smoke"
	flamer_chem = null

/obj/item/ammo_magazine/hardpoint/primary_flamer/smoke/rejects_weak_fuel(datum/reagent/to_add)
	return FALSE
