...
/obj/item/ammo_magazine/hardpoint/secondary_flamer/afterattack(obj/target, mob/user, flag)
	if(get_dist(user, target) > 1)
		return ..()
	if(!istype(target, /obj/structure/reagent_dispensers/tank/fuel) && !istype(target, /obj/item/tool/weldpack) && !istype(target, /obj/item/storage/backpack/marine/engineerpack) && !istype(target, /obj/item/ammo_magazine/hardpoint/secondary_flamer) && !istype(target, /obj/item/ammo_magazine/flamer_tank))
		return ..()
...
/obj/item/ammo_magazine/hardpoint/secondary_flamer/proc/rejects_weak_fuel(datum/reagent/to_add)
...
/// LZR-N equivalent of the infantry M240's smoke tank. Starts empty and accepts any chemical.
/obj/item/ammo_magazine/hardpoint/secondary_flamer/smoke
	name = "LZR-N Flamer Unit Smoke Tank"
	desc = "A tank fitted to be accepted by the LZR-N Flamer, holding powdered smoke that expands when exposed to an open flame and carries any chemicals along with it."
	caliber = "Powdered Smoke"
	flamer_chem = null

/obj/item/ammo_magazine/hardpoint/secondary_flamer/smoke/rejects_weak_fuel(datum/reagent/to_add)
	return FALSE
