...
/obj/item/hardpoint/primary/cannon/generate_bullet(mob/user, turf/origin_turf)
	...
	// Carries a shell's engraved message onto its fired projectile.
	var/obj/item/ammo_magazine/hardpoint/ltb_cannon/shell = ammo
	if(istype(shell) && shell.engraved_message)
		fired_projectile.name += " \"[shell.engraved_message]\""
...
/**
 * Rebuilds the reticle/direction overlays and the beam using the AMR's focused-fire beam.
 * The reticle shows one of 5 discrete stages tracking how close the charge is to completing.
 * Also refreshes the direction arrow's facing and ramps the beam's opacity with charge time.
 */
/obj/item/hardpoint/primary/cannon/proc/apply_beam_visuals(atom/target, use_intense)
	...
	lockon_icon = image(icon = 'icons/effects/Targeted.dmi', icon_state = "tank_ltb_[get_aim_stage()]")
	lockon_icon.pixel_x = lockon_pixel_x
	lockon_icon.pixel_y = lockon_pixel_y
	target.overlays += lockon_icon

	lockon_direction_icon = image(icon = 'icons/effects/Targeted.dmi', icon_state = "sniper_lockon_direction", dir = get_cardinal_dir(target, origin_turf))
...
/// Which of the 5 reticle stages the charge is currently on.
/obj/item/hardpoint/primary/cannon/proc/get_aim_stage()
	var/elapsed = world.time - charge_start_time
	var/progress = clamp(elapsed / f_aiming_time, 0, 1)
	return clamp(ceil(progress * 5), 1, 5)

/// Flickers the beam between red and blue about twice a second while charging.
