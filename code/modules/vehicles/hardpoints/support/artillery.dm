...
/obj/item/hardpoint/support/artillery_module/proc/toggle_optics(mob/living/user)
	if(!is_functional())
		to_chat(user, SPAN_WARNING("\The [src] is [health <= 0 ? "broken" : "malfunctioning"]!"))
		return
	if(!user?.client)
		return
...
/obj/item/hardpoint/support/artillery_module/proc/enable_optics(mob/living/user)
	if(!user?.client || (user in optics_users))
		return
	if(user.observed_atom)
		to_chat(user, SPAN_WARNING("You cannot use the optics while looking up."))
		return
	optics_users += user
...
/obj/item/hardpoint/support/artillery_module/proc/reset_view(mob/living/user)
	if(!user || !user.client)
		return
	user.client.change_view(VEHICLE_SEAT_VIEW_RADIUS, owner)
	user.client.set_pixel_x(0)
	user.client.set_pixel_y(0)
...
/obj/item/hardpoint/support/artillery_module/proc/enable_nvg(mob/living/carbon/human/user)
	...
	playsound_client(user.client, 'sound/handling/toggle_nv1.ogg', null, 75)
...
/obj/item/hardpoint/support/artillery_module/proc/disable_nvg(mob/living/carbon/human/user)
	...
	playsound_client(user.client, 'sound/handling/toggle_nv2.ogg', null, 75)
...
/**
 * Kicks out anyone using the optics/NVG when this goes non-functional, and refreshes
 * crew action buttons so they show up or disappear to match.
 */
/obj/item/hardpoint/support/artillery_module/refresh_functional_state(obj/vehicle/multitile/vehicle = owner, ignore_battery = FALSE)
	. = ..()
