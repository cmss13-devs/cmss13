...
/obj/structure/weapons_loader/attackby(obj/item/I, mob/user)
	// Hardpoints that load star shells/star shell packets directly (the tank turret, the APC's flare
	// launcher) aren't loaded via a swappable magazine, so they shouldn't be picked up by the generic
	// magazine-matching loop below. Delegate loose star shells and star shell packets straight to
	// whichever hardpoint accepts them.
	if(istype(I, /obj/item/explosive/grenade/high_explosive/airburst/starshell) || istype(I, /obj/item/storage/box/packet/flare))
		if(!skillcheck(user, SKILL_VEHICLE, SKILL_VEHICLE_LARGE))
			...
			return
		var/obj/item/hardpoint/starshell_hardpoint = vehicle?.get_starshell_hardpoint()
		if(!starshell_hardpoint)
			return ..()
		return starshell_hardpoint.attackby(I, user)
