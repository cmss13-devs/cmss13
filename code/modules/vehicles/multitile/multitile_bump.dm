...
/mob/living/carbon/human/handle_vehicle_bump(var/obj/vehicle/multitile/V)
	...
	if(dmg)
		...
		log_attack("[key_name(src)] was rammed by [key_name(driver)] with [V].")
		if(faction == driver.faction)
			msg_admin_ff("[key_name(driver)] rammed and damaged member of allied faction [key_name(src)] with \the [V] in [get_area(src)] (<A HREF='?_src_=admin_holder;adminplayerobservecoodjump=1;X=[driver.x];Y=[driver.y];Z=[driver.z]'>JMP</a>) ([driver.client ? "<a href='?priv_msg=[driver.client.ckey]'>PM</a>" : "NO CLIENT"])")
	else
