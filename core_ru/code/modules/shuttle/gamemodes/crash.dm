...
/obj/structure/machinery/computer/shuttle/shuttle_control/uss_crash/Topic(href, href_list)
	...
	var/obj/docking_port/mobile/shuttle = SSshuttle.getShuttle(DROPSHIP_CRASH)
	if(!(shuttle.shuttle_flags & GAMEMODE_IMMUNE) && world.time < SSticker.round_start_time + SSticker.mode.lz_selection_timer)
	...
	addtimer(VARSET_CALLBACK(C, marines_evac, CRASH_EVAC_INPROGRESS), shuttle.ignitionTime + 1 SECONDS)
