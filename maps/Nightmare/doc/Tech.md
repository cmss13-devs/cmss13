 ...
 * a `nmnode` is instantiated with only its own JSON info
 ...
 * a `nmtask`must be synchronous and independent from context and nodes
...
This is the chosen method as of current. Basically, we assume that every task is executed synchronously and will be done by the time the call returns. This mean that any sleeping within the task will also sleep the caller thread. This does not allow for parallelism / juggling tasks. It also requires a global lock to monitor completion: in current implementation, this is done via SSnightmare.stat monitored by SSticker loop.
