...
/**
 * ## DigitalGauge
 * The DigitalGauge component provides a visual representation of a single metric, as well as being capable of showing
 ...
 *
 */
export function DigitalGauge(props: Props) {
  const {
    alertAfter,
    alertBefore,
    className,
        ...
        <svg viewBox="0 0 100 50">
          {(alertAfter || alertBefore) && (
            <g
              className={classes([
                'DigitalGauge__alert',
                alertColor ? `active DigitalGauge__alert--${alertColor}` : '',
              ])}
            >
          ...
          <g>
            <circle
              className="DigitalGauge__ringTrack"
              cx="50"
              cy="50"
              r="45"
            />
          ...
          <g>
            {Object.keys(scaledRanges).map((x, i) => {
              ...
              return (
                <circle
                  className={`DigitalGauge__ringFill DigitalGauge--color--${x}`}
                  key={i}
                  style={{
          ...
          <g
            className="DigitalGauge__needle"
            transform={`rotate(${clampedValue * 180 - 90} 50 50)`}
          >
            <polygon
              className="DigitalGauge__needleLine"
              points="46,50 50,0 54,50"
            />
            <circle
              className="DigitalGauge__needleMiddle"
              cx="50"
              cy="50"
              r="8"
            />
