...
export const STUI = (props, context) => {
  ...
  const [logsfontnumber, setLogsFontSize]
  = useLocalState(context, "logsfontnumber", 7);
          ...
          <Flex.Item mt={1} grow={1} basis={0}>
            <Section fill scrollable >
              {logs.map(log => {
                ...
                return (
                  <Box fontSize={logsfontsize} key={log}>{log}</Box>
                );
              })}
              {logs.length > 1 && (
                <Box align="center" fontSize={bigfontsize}>--- End of Logs --</Box>
              ) || (
                <Box align="center" fontSize={bigfontsize}>--- No Logs --</Box>
              )}
            </Section>
          </Flex.Item>
        </Flex>
